...
async def cb_view_schedule(query: CallbackQuery):
    async with AsyncSessionLocal() as session:
        ...
        header_sc = {"tr": f"📅 <b>{escape_html(cls_name)} Sınıfı Ders Programı:</b>", "ru": f"📅 <b>Расписание уроков {escape_html(cls_name)} класса:</b>", "uz": f"📅 <b>{escape_html(cls_html(cls_name) if 'cls_html' in locals() else escape_html(cls_name))} sinfi dars jadvali:</b>", "en": f"📅 <b>Timetable for Class {escape_html(cls_name)}:</b>"}.get(lang, f"📅 <b>{escape_html(cls_name)} Timetable:</b>")
