...
class GlobalMenuButtonMiddleware(BaseMiddleware):
    async def __call__(
        ...
        if isinstance(event, Message) and event.from_user:
            m_text = event.text or event.caption or ""
            act_type = "message"
            act_desc = f"Mesaj: {m_text[:35]}" if m_text else "Mesaj Gönderdi"
            if event.voice:
                act_type = "voice"
                act_desc = "Sesli Not Gönderdi"
            elif event.photo:
                act_type = "photo"
                act_desc = "Fotoğraf Gönderdi"
            elif event.document:
                act_type = "document"
                fname = getattr(event.document, "file_name", "Belge") or "Belge"
                act_desc = f"Belge Gönderdi ({fname[:25]})"
            elif event.video:
                act_type = "video"
                act_desc = "Video Gönderdi"
            elif event.location:
                act_type = "location"
                act_desc = "Konum Paylaştı"
            elif event.contact:
                act_type = "contact"
                act_desc = "Kişi Kartı Paylaştı"
...
ENABLE_PROFILE_TELEMETRY = os.getenv("ENABLE_PROFILE_TELEMETRY", "false").strip().lower() in {"1", "true", "yes", "on"}
APP_VERSION = "30.1-hardened"
...
def format_telegram_html(text: str) -> str:
    ...
    t = t.replace("''", "")
    # Code blocks must be converted before removing stray backticks.
    t = re.sub(r"```(.*?)```", lambda m: f"<pre>{html.escape(m.group(1), quote=False)}</pre>", t, flags=re.DOTALL)
...
LOCALES = {
    ...
    'ru': {
        ...
        'btn_behavior': '⭐ Пове����е��ие и баллы',
        ...
        'contact_req_id': 'Пожалуйста, напишите администрации ��колы.',
        ...
        'select_target_teacher': '👨‍🏫 Кому передать класс <b>{class_name}</b>? ��ы��ерите учителя:',
        ...
        'timezone_updated': 'Часовой пояс уст��новле�� на UTC+{offset}.',
...
# Safety net for legacy keys referenced by handlers but absent from the main locale catalog.
FALLBACK_TEXTS = {
    "unreg_welcome": {
        "tr": "Oturum kapatıldı. Devam etmek için giriş kodunuzu girin.",
        "ru": "Сеанс завершён. Введите код доступа, чтобы продолжить.",
        "uz": "Seans yakunlandi. Davom etish uchun kirish kodini kiriting.",
        "en": "You have been logged out. Enter your access code to continue.",
    },
    "temporary_admin_title": {
        "tr": "Geçici Yönetici", "ru": "Временный администратор",
        "uz": "Vaqtinchalik administrator", "en": "Temporary Administrator",
    },
    "record_not_found": {
        "tr": "Kayıt bulunamadı.", "ru": "Запись не найдена.",
        "uz": "Yozuv topilmadi.", "en": "Record not found.",
    },
}
...
def get_text(key: str, lang: str = "tr", **kwargs) -> str:
    ...
    template = (
        LOCALES.get(selected_lang, {}).get(key)
        or LOCALES.get("tr", {}).get(key)
        or FALLBACK_TEXTS.get(key, {}).get(selected_lang)
        or FALLBACK_TEXTS.get(key, {}).get("tr")
        or f"[{key}]"
    )
    if kwargs:
        try:
...
def is_admin_user(user: User | None, user_id: int) -> bool:
    """Centralized administrator check, including blacklist and temporary expiry."""
    ...
    if not user or user.role != "admin":
        return False
    if user.admin_type == "temporary":
        if not user.admin_until or user.admin_until <= datetime.utcnow():
            return False
    return True


def ensure_role_authorized(user: User | None, allowed_roles: list[str]) -> bool:
    """Central role authorization filter used by all privileged flows."""
    if not user or user.is_blacklisted:
        return False
    if "admin" in allowed_roles and is_admin_user(user, user.telegram_id):
        return True
    return user.role in allowed_roles and user.role != "admin"


def generate_secure_code(prefix: str) -> str:
    """Generate a cryptographically strong six-digit enrollment code."""
    num_part = str(secrets.randbelow(900000) + 100000)
    return f"{prefix}-{num_part}"


ADMIN_CALLBACK_PREFIXES = ("adm:", "pinkey:", "chgpin:")
ADMIN_FSM_STATE_NAMES = {
    "waiting_school_name", "waiting_admin_tg_id", "waiting_admin_name",
    "waiting_search_teacher_query", "sched_update_text", "waiting_search_user_query",
    "waiting_admin_dm_text", "rename_class_name", "add_class_name",
    "waiting_admin_prop_title", "waiting_admin_prop_content", "waiting_admin_decision_note",
    "waiting_force_alert_text", "add_student_name", "add_student_class", "add_student_no",
    "add_teacher_name", "add_teacher_subject", "waiting_search_query",
    "waiting_broadcast_text", "menu_update_text", "edit_student_val",
    "waiting_exam_subject", "waiting_exam_date", "waiting_emergency_text",
    "waiting_teacher_excel", "waiting_restore_excel", "appr_st_class", "appr_st_no",
}


class RoleAuthorizationMiddleware(BaseMiddleware):
    """Reject forged admin callbacks and stale privileged FSM sessions centrally."""
    ...
    async def __call__(
        ...
        if isinstance(event, CallbackQuery):
            ...
            requires_admin = callback_data.startswith(ADMIN_CALLBACK_PREFIXES)
        elif isinstance(event, Message):
            state_ctx = data.get("state")
            if state_ctx:
                try:
                    raw_state = await state_ctx.get_state()
                    state_name = str(raw_state or "").rsplit(":", 1)[-1]
                    requires_admin = state_name in ADMIN_FSM_STATE_NAMES
                except Exception:
                    logger.exception("FSM authorization state could not be read")
                    requires_admin = True
        ...
        if user_id and is_admin_user(user, user_id):
            return await handler(event, data)
        ...
        if state_ctx:
            try:
                ...
                logger.exception("Unauthorized FSM state could not be cleared")

        lang = user.language if user else "tr"
        warning = get_text("unauthorized_action", lang)
        if isinstance(event, CallbackQuery):
...
def sanitize_workbook_formulas(workbook) -> None:
    """Neutralize spreadsheet formulas originating from user-controlled text."""
...
async def process_student_excel(file_bytes: bytes) -> tuple[int, io.BytesIO]:
    ...
    sanitize_workbook_formulas(wb_out)
...
async def get_dashboard_card_text(user: User) -> str:
    ...
    if user.role == "admin":
        ...
        return format_telegram_html(raw)
...
async def cb_admin_manage_teacher_classes(query: CallbackQuery):
    ...
    async with AsyncSessionLocal() as session:
        ...
        if not all_classes:
            buttons = [
                [InlineKeyboardButton(text="➕ " + ("Sınıf Ekle" if lang=="tr" else ("Доб��вить класс" if lang=="ru" else ("Sinf qo'shish" if lang=="uz" else "Add Class"))), callback_data="adm:add_class")],
    ...
    async with AsyncSessionLocal() as session:
        ...
        if total_students == 0:
            ...
            tap_hint = {
                ...
                "ru": "Нажмите на ученика для просмотр�� данных или кодов:",
...
async def cb_admin_export_audit_excel(query: CallbackQuery):
    ...
    caption_txt = {
        ...
        "ru": f"📑 <b>Журнал аудита безопасности (Excel)</b>\nВыгружено <b>{(len(logs) if isinstance(logs, (list, tuple)) else 0)}</b> записей дей��твий.",
...
async def cb_cat_staff(event: Message | CallbackQuery, state: FSMContext | None = None):
    ...
    async with AsyncSessionLocal() as session:
        ...
        buttons = [
            ...
            [InlineKeyboardButton(text="🎓 " + {"tr": "Mezunlar Arşivi", "ru": "Архив вып��скников", "uz": "Bitiruvchilar arxivi", "en": "Alumni Archive"}.get(lang, "Alumni"), callback_data="adm:graduates:0")]
        ]
...
async def cb_admin_user_360(query: CallbackQuery):
    ...
    async with AsyncSessionLocal() as session:
        ...
        buttons = [
            [InlineKeyboardButton(text=btn_refresh, callback_data=f"adm:user_360:{tg_id}:{page}:ref")],
    ...
    async with AsyncSessionLocal() as session:
        ...
        report_card = {
            ...
            "ru": (
                ...
                f"👥 <b>Всего получателе��:</b> {total_recipients}\n"
...
async def cb_admin_bot_block_monitor(query: CallbackQuery):
    ...
    async with AsyncSessionLocal() as session:
        ...
        bc_bb = {"tr": "🏠 Ana Menü ➔ 👥 Kadro ��� 🚫 Bot İletişim Takip", "ru": "🏠 Г��авное меню ➔ 👥 Ученики ➔ 🚫 Контроль блокировок", "uz": "🏠 Asosiy menyu ➔ 👥 Kadro ➔ 🚫 Bot bloklarini nazorat", "en": "🏠 Main Menu ➔ 👥 Staff ➔ 🚫 Bot Block Tracking"}.get(lang, "🚫 Bot Block Tracking")
...
async def process_force_audio_alert_text(message: Message, state: FSMContext):
    ...
    async with AsyncSessionLocal() as session:
        ...
        for tid in target_ids:
            ...
            hdr_fa = {
                "tr": "🚨🔊 <b>OKUL YÖNETİMİ ACİL BİLDİR��Mİ</b>",
...
async def cb_admin_proposal_decide_init(query: CallbackQuery, state: FSMContext):
    ...
    prompt_d = {
        ...
        "ru": (
            f"⚖️ <b>Администрати��ное решение: #{prop_id} ({cur_dec_lbl})</b>\n"
            "────────────────────\n"
...
dp.callback_query.outer_middleware(RoleAuthorizationMiddleware())
dp.message.outer_middleware(GlobalMenuButtonMiddleware())
dp.message.outer_middleware(RoleAuthorizationMiddleware())
...
async def lifespan(app: FastAPI):
    ...
    t7 = asyncio.create_task(background_profile_telemetry_poller(bot)) if ENABLE_PROFILE_TELEMETRY else None
    ...
    print("=" * 60)
    yield
    tasks = [polling_task, t1, t2, t3, t4, t5, t6, t7]
...
@app.get("/")
async def root():
    return {"status": "ok", "service": "OkulYonetimBot", "version": APP_VERSION, "uptime": True}
...
async def bot_status_view():
    try:
        ...
        logger.exception("Bot status check failed")
        return JSONResponse(
            status_code=status.HTTP_503_SERVICE_UNAVAILABLE,
            content={"status": "error", "message": "Bot status is temporarily unavailable"}
        )
...
async def telegram_webhook(request: Request):
    ...
    try:
        ...
        logger.exception("Telegram update processing timed out: update_id=%s", data.get("update_id"))
        return JSONResponse(status_code=status.HTTP_503_SERVICE_UNAVAILABLE, content={"error": "Temporary processing timeout"})
    except Exception:
        logger.exception("Telegram update processing failed: update_id=%s", data.get("update_id"))
        return JSONResponse(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, content={"error": "Update processing failed"})
...
# Attach the fully configured router only after all handlers are declared.
...
if __name__ == "__main__":
    ...
    # Uvicorn is used in both webhook and polling modes so the same lifespan,
    # database initialization, background workers and graceful shutdown run.
