# -*- coding: utf-8 -*-
# ======================================================================
# OKUL BOTU ENTERPRISE - RELEASE V45.0 (CLASS-BY-CLASS PROMOTION & FULL I18N)
...
async def generate_user_forensic_dossier_pdf(target_user_id: int, lang: str = "tr") -> io.BytesIO:
    ...
    story = [
        Paragraph("<b>RESMİ ADLİ İSTİHBARAT VE GÜVENLİK DOSYASI</b>", t_style),
        Paragraph("<font size=7 color='#718096'>T.C. MİLLİ EĞİTİM BAKANLIĞI • KURUMSAL BİLGİ VE TELEMETRİ SİSTEMİ</font>", c_center),
        Spacer(1, 10)
    ]

    date_str = (datetime.utcnow() + timedelta(hours=TIMEZONE_OFFSET)).strftime('%d.%m.%Y %H:%M')
    status_disp = "YASAKLI" if u.is_blacklisted else ("BOTU SİLDİ" if u.is_bot_blocked else "AKTİF")
    meta = [
        [Paragraph("<b>Ad Soyad:</b>", c_bold), Paragraph(escape_html(u.full_name or "Bilinmeyen"), c_norm), Paragraph("<b>Tarih / Saat:</b>", c_bold), Paragraph(date_str, c_norm)],
        [Paragraph("<b>Telegram ID:</b>", c_bold), Paragraph(str(u.telegram_id), c_norm), Paragraph("<b>Kullanıcı Adı:</b>", c_bold), Paragraph(f"@{u.username}" if u.username else "Yok", c_norm)],
        [Paragraph("<b>Sistem Rolü:</b>", c_bold), Paragraph(u.role.upper(), c_norm), Paragraph("<b>İdari Statü:</b>", c_bold), Paragraph(u.admin_type.upper(), c_norm)],
        [Paragraph("<b>Hesap Durumu:</b>", c_bold), Paragraph(status_disp, c_norm), Paragraph("<b>Gece Aktivitesi:</b>", c_bold), Paragraph(f"{u.night_activity_count} işlem", c_norm)],
        [Paragraph("<b>İndirilen Dosyalar:</b>", c_bold), Paragraph(f"{u.download_count} adet", c_norm), Paragraph("<b>Son İşlem:</b>", c_bold), Paragraph(escape_html(u.last_action_desc or "Kayıt yok"), c_norm)],
    ]
    ...
    story.append(Paragraph("<b>ETKİLEŞİM VE İLGİ DAĞILIMI (TYPOLOGY PERSONA)</b>", c_bold))
    story.append(Spacer(1, 4))
    p_rows = [[Paragraph("<b>Kategori</b>", c_center_b), Paragraph("<b>İşlem Adedi</b>", c_center_b)]]
    if not p_counts:
        p_rows.append([Paragraph("Genel Etkileşim", c_norm), Paragraph("0", c_center)])
    else:
    ...
    story.append(Paragraph("<b>KİMLİK VE KULLANICI ADI DEĞİŞİM GEÇMİŞİ (IDENTITY MORPHING LOG)</b>", c_bold))
    story.append(Spacer(1, 4))
...
async def cb_student_card(query: CallbackQuery):
    ...
    async with AsyncSessionLocal() as session:
        ...
        parent_names = [f"{p.full_name or p_label_def} (<code>{p.telegram_id}</code>)" for p in parents]
        p_info = ", ".join(parent_names) if parent_names else no_parent_txt
...
async def cb_admin_forensic_pdf(query: CallbackQuery):
    ...
    async with AsyncSessionLocal() as session:
        ...
        pdf_buf.close()
    await query.answer("📑 Adli Rapor Hazırlandı!", show_alert=False)
...
async def cb_admin_class_promotion_init(query: CallbackQuery, state: FSMContext | None = None):
    ...
    async with AsyncSessionLocal() as session:
        ...
        desc_pr = {
            "tr": (
                f"📌 <b>Eğitim Dönemi Sonu Terfi İşlemleri</b>\n"
                f"• Aktif Sınıf Sayısı: <b>{total_cls}</b>\n"
                f"• Aktif Öğrenci Sayısı: <b>{total_st}</b>\n\n"
                f"Lütfen uygulamak istediğiniz terfi yöntemini seçiniz:\n\n"
                f"• <b>🌐 Toplu Terfi (Tüm Okul):</b> Okuldaki tüm sınıfları bir üst seviyeye taşır, son sınıf öğrencilerini otomatik olarak <i>Mezunlar Arşivi</i>ne kaydeder.\n"
                f"• <b>🏫 Sınıfa Göre Terfi:</b> Belirleyeceğiniz tek bir sınıfı bir üst kademeye taşır veya mezun eder."
            ),
            "ru": (
                f"📌 <b>Перевод классов по итогам учебного периода</b>\n"
                f"• Активных классов: <b>{total_cls}</b>\n"
                f"• Учеников в классах: <b>{total_st}</b>\n\n"
                f"Выберите способ перевода:\n\n"
                f"• <b>🌐 Массовый перевод (Вся школа):</b> Переводит все классы школы на ступень выше, выпускники сохраняются в <i>Архиве выпускников</i>.\n"
                f"• <b>🏫 Поклассовый перевод:</b> Позволяет выбрать конкретный класс для перевода в следующий класс или выпуска."
            ),
            "uz": (
                f"📌 <b>O'quv davri yakunida sinflarni ko'chirish</b>\n"
                f"• Faol sinflar soni: <b>{total_cls}</b>\n"
                f"• Faol o'quvchilar soni: <b>{total_st}</b>\n\n"
                f"Ko'chirish usulini tanlang:\n\n"
                f"• <b>🌐 Ommaviy ko'chirish (Butun maktab):</b> Maktabdagi barcha sinflarni bir pog'ona yuqoriga o'tkazadi, bitiruvchilarni <i>Bitiruvchilar arxivi</i>ga saqlaydi.\n"
                f"• <b>🏫 Sinf bo'yicha ko'chirish:</b> Tanlangan sinfni bir pog'ona yuqoriga o'tkazadi yoki bitiruvchi qiladi."
            ),
            "en": (
                f"📌 <b>End-of-Term Class Promotion Operations</b>\n"
                f"• Active Classes: <b>{total_cls}</b>\n"
                f"• Active Students: <b>{total_st}</b>\n\n"
                f"Select your preferred promotion method:\n\n"
                f"• <b>🌐 Bulk Promotion (All School):</b> Promotes all classes across the school and automatically archives seniors into the <i>Alumni Archive</i>.\n"
                f"• <b>🏫 Class-by-Class Promotion:</b> Promotes or graduates a specific single class of your choice."
            )
        }.get(lang, "Select promotion method:")
