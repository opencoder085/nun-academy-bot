# -*- coding: utf-8 -*-
# ======================================================================
# OKUL BOTU ENTERPRISE - RELEASE V43.0 (TEACHER CANCEL TRIPLE-GUARDED & VERIFIED)
...
# --- SINIF ATLATMA (DÖNEM SONU TERFİ) - DOĞRUDAN ONAYLI & OTOMATİK EXCEL YEDEKLEMELİ ---
...
async def cb_admin_class_promotion_init(query: CallbackQuery, state: FSMContext | None = None):
    async with AsyncSessionLocal() as session:
        ...
        buttons = [
            [InlineKeyboardButton(text={"tr": "✅ Evet, Sınıfları Yükselt", "ru": "✅ Да, перевести классы", "uz": "✅ Ha, sinflarni ko'chirish", "en": "✅ Yes, Promote Classes"}.get(lang, "✅ Promote Classes"), callback_data="adm:class_promotion_confirm")]
        ]
        buttons.append(get_nav_buttons(lang, back_callback="adm:cat_staff"))
...
async def cb_grade_save_final(query: CallbackQuery, state: FSMContext):
    ...
    async with AsyncSessionLocal() as session:
        ...
        for p_id in parents:
            p_u = await session.get(User, p_id)
            p_lang = p_u.language if p_u else "tr"
            alert_text = get_text("grade_parent_notification", p_lang, name=escape_html(st.full_name), subject=escape_html(subject_name), exam_type=escape_html(exam_type_val), score=score_val, badge=badge_val)
...
async def cb_view_report_card(query: CallbackQuery, state: FSMContext):
    ...
    async with AsyncSessionLocal() as session:
        ...
        for s_name, scores in subj_map.items():
            ...
            subj_lines.append(f"📚 <b>{escape_html(s_name)}:</b> {scores_str} • <b>Ort:</b> <code>{s_avg}</code> {badge}")
...
async def cb_admin_risk_radar(query: CallbackQuery):
    ...
    async with AsyncSessionLocal() as session:
        ...
        for s in students:
            ...
            if absent_cnt >= 8:
                risk_score += 50
                reasons.append(f"{absent_cnt} gün devamsızlık")
            elif absent_cnt >= 5:
                risk_score += 30
                reasons.append(f"{absent_cnt} gün devamsızlık")
            ...
            if avg_gpa < 50:
                risk_score += 40
                reasons.append(f"Düşük ortalama ({avg_gpa:.1f})")
            elif avg_gpa < 65:
                risk_score += 25
                reasons.append(f"Riskli ortalama ({avg_gpa:.1f})")

            if not parents:
                risk_score += 20
                reasons.append("Bağlı veli yok")
            elif all((not p.last_seen_at or (datetime.utcnow() - p.last_seen_at).days > 30) for p in parents):
                risk_score += 15
                reasons.append("Veli ilgisiz (>30 gün)")
        ...
        if not risk_list:
            ...
            for s, r_score, r_reasons in risk_list[:8]:
                ...
                lines.append(f"{badge} <b>{escape_html(s.full_name)}</b> ({escape_html(s.class_name)}) • <b>Risk: %{r_score}</b>\n   ↳ <i>{r_txt}</i>")
    ...
    async with AsyncSessionLocal() as session:
        ...
        if not parents:
            ...
            for p in parents:
                ...
                k_info = ", ".join([f"{k.full_name} ({k.class_name})" for k in kids]) if kids else "Öğrenci Bağı Yok"
                days_absent = (now - p.last_seen_at).days if p.last_seen_at else "Hiç girmedi"
                lines.append(f"⚪ <b>{escape_html(p.full_name or 'Veli')}</b> (<code>{p.telegram_id}</code>)\n   ↳ Öğrenci: {escape_html(k_info)} • Pasif: {days_absent} gün")
                buttons.append([InlineKeyboardButton(text=f"👤 {p.full_name or 'Veli'} ({days_absent} gün)", callback_data=f"adm:user_card:{p.telegram_id}:0")])
            lines.append("────────────────────")
            text = "\n".join(lines)
...
async def cb_admin_activity_heatmap(query: CallbackQuery):
    ...
    async with AsyncSessionLocal() as session:
        ...
        text = (
            f"{hdr}\n"
            "────────────────────\n"
            f"{labels[0]}: <b>%{pct_s}</b>\n<code>{get_bar(bins['sabah'])}</code> ({bins['sabah']} işlem)\n\n"
            f"{labels[1]}: <b>%{pct_o}</b>\n<code>{get_bar(bins['ogle'])}</code> ({bins['ogle']} işlem)\n\n"
            f"{labels[2]}: <b>%{pct_i}</b>\n<code>{get_bar(bins['ikindi'])}</code> ({bins['ikindi']} işlem)\n\n"
            f"{labels[3]}: <b>%{pct_a}</b>\n<code>{get_bar(bins['aksam'])}</code> ({bins['aksam']} işlem)\n\n"
            f"{labels[4]}: <b>%{pct_g}</b>\n<code>{get_bar(bins['gece'])}</code> ({bins['gece']} işlem)\n"
            "────────────────────\n"
            f"👥 <b>Toplam Kayıtlı Aktivite:</b> {sum(bins.values())} işlem\n"
            "────────────────────"
        )
        buttons = [get_nav_buttons(lang, back_callback="adm:cat_tools_reports")]
        await safe_edit_or_answer(query, text, reply_markup=InlineKeyboardMarkup(inline_keyboard=buttons), parse_mode="HTML")
...
async def cb_admin_watchlist_hub(query: CallbackQuery):
    ...
    async with AsyncSessionLocal() as session:
        ...
        if not suspicious:
            ...
            for u in suspicious:
                flags = []
                if u.is_blacklisted: flags.append("Yasaklı")
                if u.is_bot_blocked: flags.append("Botu Sildi")
                if u.night_activity_count > 0: flags.append(f"{u.night_activity_count} Gece")
                if u.download_count >= 5: flags.append(f"{u.download_count} İndirme")
                f_str = ", ".join(flags)
