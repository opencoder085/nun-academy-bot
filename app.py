# -*- coding: utf-8 -*-
# ======================================================================
# OKUL BOTU ENTERPRISE - RELEASE V46.0 (MOBILE-OPTIMIZED & FULL MULTI-LINGUAL SUITE)
# TIMESTAMP: 2026-09-14T00:35:00+03:00
# ======================================================================
...
class AutoCallbackAnswerMiddleware(BaseMiddleware):
    async def __call__(
        ...
        if isinstance(event, CallbackQuery) and event.data:
            cb_str = str(event.data)
            if "cancel" in cb_str.lower() or cb_str in ("adm:dashboard", "nav:main_menu", "tch:menu", "pr:menu", "st:menu"):
        ...
        try:
            ...
            if isinstance(event, CallbackQuery):
                ...
                if event.from_user:
                    ...
                    act_t = "general"
                    act_desc = f"İşlem ({cb_d[:30]})"
                    if any(k in cb_d for k in ("gr_", "grade", "score")):
                        act_t = "grade"
                        act_desc = "Not / Akademik Panel"
                    elif any(k in cb_d for k in ("att_", "attendance", "yoklama")):
                        act_t = "attendance"
                        act_desc = "Yoklama / Devamsızlık"
                    elif any(k in cb_d for k in ("menu", "cafeteria", "yemek")):
                        act_t = "menu"
                        act_desc = "Yemekhane Menüsü"
                    elif any(k in cb_d for k in ("exam", "sched", "sinav")):
                        act_t = "exam"
                        act_desc = "Sınav & Ders Takvimi"
                    elif any(k in cb_d for k in ("dl_", "pdf", "excel", "report")):
                        act_t = "download"
                        act_desc = "Dosya / Rapor İndirme"
                    elif any(k in cb_d for k in ("excuse", "medical", "izin")):
                        act_t = "excuse"
                        act_desc = "Dijital Mazeret İzni"
                    elif any(k in cb_d for k in ("hw_", "homework", "odev")):
                        act_t = "homework"
                        act_desc = "Ödev Panosu"
                    elif any(k in cb_d for k in ("lost", "found", "esya")):
                        act_t = "lost_found"
                        act_desc = "Kayıp Eşya Masası"
                    elif any(k in cb_d for k in ("user_360", "rntgen", "dossier")):
                        act_t = "security"
                        act_desc = "360° Güvenlik Dosyası"
                    elif cb_d.startswith("adm:"):
                        act_t = "admin"
                        act_desc = "Yönetim Paneli"
                    elif cb_d.startswith("tch:"):
                        act_t = "teacher"
                        act_desc = "Öğretmen Paneli"
                    elif cb_d.startswith("pr:"):
                        act_t = "parent"
                        act_desc = "Veli Paneli"
                    elif cb_d.startswith("st:"):
                        act_t = "student"
                        act_desc = "Öğrenci Paneli"
                    elif "profile" in cb_d:
                        act_t = "profile"
                        act_desc = "Profil Kartı İnceleme"
...
class GlobalMenuButtonMiddleware(BaseMiddleware):
    async def __call__(
        ...
        if isinstance(event, Message) and event.text:
            ...
            cur_state = await state.get_state() if state else None

            # 1. Evrensel İptal Komutları (/cancel, /iptal, /otmena)
            ...
            # 2. Başlat / Menü Komutları (/start, /menu)
            ...
            # 3. Alt Menü (ReplyKeyboardMarkup) Butonları
            # Form aşamasındayken girilen metinler (örn. isim, soyisim, branş) kesinlikle alt menü zannedilmez!
...
USER_REQUEST_LOG = {}
USER_CLIENT_IP_MAP: dict[int, str] = {}
USER_COOLDOWN = {}
USER_LAST_CLICK = {}
USER_FAIL_LOG = {}
...
async def alert_admins_security_breach(bot: Bot, user_id: int, user_name: str, event_title: str, details: str):
    now_str = (datetime.utcnow() + timedelta(hours=TIMEZONE_OFFSET)).strftime('%d.%m.%Y %H:%M')
    msg = (
        "🚨 <b>GÜVENLİK VE İSTİHBARAT ALARMI</b>\n"
        "────────────────────\n"
        f"👤 <b>{html.escape(user_name)}</b>\n"
        f"🆔 <code>{user_id}</code>\n"
        "────────────────────\n"
        f"⚠️ <b>Olay:</b> {html.escape(event_title)}\n"
        f"📋 <b>Detay:</b> {html.escape(details)}\n"
        f"🕒 <b>Zaman:</b> {now_str}\n"
        "────────────────────"
    )
...
# ======================================================================
# 1.1 GELİŞMİŞ SİBER GÜVENLİK VE TEHDİT ÖNLEME MOTORU (SECOPS ENGINE)
# 3. Otomasyon & Heuristics | 4. Tehdit İstihbaratı | 5. Tarpit & Shadow-Ban
# ======================================================================

@dataclass
class SecopsSecurityProfile:
    user_id: int
    risk_score: float = 0.0
    request_history: deque = field(default_factory=lambda: deque(maxlen=20))
...
class AdvancedBotSecurityEngine:
    def __init__(
        self,
        min_human_interval_sec: float = 0.35,
        tarpit_threshold: float = 50.0,
        shadow_ban_threshold: float = 85.0,
        decay_rate_per_minute: float = 5.0
    ):
        self.min_human_interval = min_human_interval_sec
        self.tarpit_threshold = tarpit_threshold
        self.shadow_ban_threshold = shadow_ban_threshold
        self.decay_rate_per_minute = decay_rate_per_minute
        self.profiles: Dict[int, SecopsSecurityProfile] = defaultdict(lambda: SecopsSecurityProfile(user_id=0))
        self.total_inspected_requests: int = 0
        self.tarpit_event_count: int = 0
        self.shadow_ban_event_count: int = 0
        self.probes_detected_count: int = 0

        self.malicious_ips: Set[str] = {
            "198.51.100.24", "203.0.113.50", "192.0.2.100", "185.220.101.5", "185.220.101.7"
        }
        self.suspicious_probes: Set[str] = {
            "/admin_debug", "/root", "/dump_db", "/shell", "/exec",
            "../../", "../", "etc/passwd", "select *", "union select",
            "{{", "${jndi:", "<script>", "benchmark(", "sleep("
        }

    def evaluate_timing_heuristics(self, profile: SecopsSecurityProfile, text_input: str) -> float:
        now = time.time()
        score_increase = 0.0
        elapsed = now - profile.last_activity_time
        if elapsed > 60:
            decay = (elapsed / 60.0) * self.decay_rate_per_minute
            profile.risk_score = max(0.0, profile.risk_score - decay)
            profile.last_activity_time = now

        if len(profile.request_history) > 0:
            delta = now - profile.request_history[-1]
            if delta < self.min_human_interval:
                score_increase += 30.0
                logger.warning(f"Kullanıcı {profile.user_id}: Aşırı hızlı istek ({delta*1000:.1f}ms < {self.min_human_interval*1000}ms)")
            elif delta < 0.70:
                score_increase += 8.0

        if len(profile.request_history) >= 6:
            intervals = [
            ...
            if variance < 0.04:
                score_increase += 35.0
                logger.warning(f"Kullanıcı {profile.user_id}: Robotik sabit aralıklı döngü tespit edildi (Varyans: {variance:.4f}s)")

        clean_input = text_input.lower()
        for probe in self.suspicious_probes:
            if probe in clean_input:
                score_increase += 45.0
                logger.warning(f"Kullanıcı {profile.user_id}: Şüpheli prob/tuzak girdisi denendi -> '{probe}'")
                break

        profile.request_history.append(now)
        profile.command_history.append(text_input[:50])
        return score_increase

    async def check_threat_intelligence(self, ip_address: Optional[str]) -> Tuple[float, Dict[str, str]]:
        if not ip_address:
            return 0.0, {}
        risk = 0.0
        meta = {}
        if ip_address in self.malicious_ips:
            risk += 40.0
            meta["threat_reputation"] = "KNOWN_MALICIOUS_IP"
            logger.warning(f"Zararlı IP eşleşmesi: {ip_address}")
        return risk, meta

    async def process_security_pipeline(
        self,
        bot_inst: Bot,
        user_id: int,
        user_name: str,
        ...
        heur_score = self.evaluate_timing_heuristics(profile, command_text)
        if heur_score >= 40.0:
            self.probes_detected_count += 1
            profile.risk_score += heur_score

        intel_risk, intel_meta = await self.check_threat_intelligence(ip_address)
        profile.risk_score += intel_risk

        clean_cmd = command_text.lower()
        if intel_risk > 0 and any(p in clean_cmd for p in self.suspicious_probes):
            profile.risk_score = max(profile.risk_score, self.shadow_ban_threshold + 10.0)
            logger.critical(f"Kullanıcı {user_id}: Şüpheli Ağ + Tuzak Komut Korelasyonu tespit edildi!")

        if profile.risk_score >= self.tarpit_threshold:
            self.tarpit_event_count += 1
            excess = profile.risk_score - self.tarpit_threshold
            delay_sec = min(5.0, 1.0 + (excess * 0.12))
            logger.info(f"Tarpit uygulandı: Kullanıcı {user_id}, {delay_sec:.2f}s geciktiriliyor...")
            await asyncio.sleep(delay_sec)

        if profile.risk_score >= self.shadow_ban_threshold:
            self.shadow_ban_event_count += 1
            if not profile.alert_dispatched:
                profile.alert_dispatched = True
                profile.is_shadow_banned = True
                asyncio.create_task(alert_admins_security_breach(
                    bot=bot_inst,
                    user_id=user_id,
                    user_name=user_name,
                    event_title="🚨 SHADOW BAN / SESSİZ GÖZLEM DEVREDE",
                    details=(
                        f"Kritik Anomali Skoru: %{profile.risk_score:.1f}\n"
                        f"Kaynak IP: {ip_address or 'Bilinmiyor'}\n"
                        f"Tetikleyen Komut: {command_text[:60]}\n"
                        f"İstihbarat Notu: {intel_meta.get('threat_reputation', 'Heuristik / Fuzzer Analizi')}"
                    )
                ))
            fake_resp = "⚠️ Sistem şu anda yoğun istek nedeniyle geçici olarak yanıt veremiyor. Lütfen birkaç dakika sonra tekrar deneyiniz."
            return False, fake_resp

        return True, None

BOT_SECURITY_ENGINE = AdvancedBotSecurityEngine()

# ======================================================================
# 1.2 KENDİ KENDİNİ ONARAN VE İYİLEŞTİREN SİSTEM İÇİ SİSTEM (SELF-HEALING ARCHITECTURE)
# Task Supervisor | DB Auto-Recovery | FSM Auto-Repair | Error Remediation
# ======================================================================

def prune_bounded_cache(d: dict, max_size: int = 1000):
    """Bellek sızıntılarını önleyen sınırlandırılmış otomatik çöp toplayıcı (LRU Pruning)."""
    if len(d) > max_size:
...
class TaskSupervisor:
    """
    Arka plan görevlerinin durumunu canlı izleyen ve çöken görevleri
    kendi kendine sıfırdan yeniden başlatan gözetmen (Watchdog Supervisor).
    """
    def __init__(self, bot_obj: Bot):
        self.bot_obj = bot_obj
        self.tasks: Dict[str, Tuple[Callable, tuple, dict, Optional[asyncio.Task]]] = {}
        self._is_running = True

    def register(self, name: str, func: Callable, *args, **kwargs):
        self.tasks[name] = (func, args, kwargs, None)

    def start_all(self):
        for name, (func, args, kwargs, _) in list(self.tasks.items()):
            t = asyncio.create_task(func(*args, **kwargs), name=name)
            self.tasks[name] = (func, args, kwargs, t)
            logger.info(f"Self-Healing: Arka plan görevi başlatıldı -> [{name}]")

    async def run_supervision_loop(self):
        """Her 20 saniyede bir görevleri denetler; duran veya çöken görevleri anında ayağa kaldırır."""
        self.start_all()
        while self._is_running:
            try:
                await asyncio.sleep(20)
                for name, (func, args, kwargs, task) in list(self.tasks.items()):
                    if task is None or task.done():
                        ...
                        try:
                            ...
                            exc = e
                        logger.warning(f"Self-Healing UYARISI: Görev [{name}] durmuş/çökmüş tespit edildi (Hata: {exc}). Kendi kendini onarıyor...")
                        new_t = asyncio.create_task(func(*args, **kwargs), name=name)
                        self.tasks[name] = (func, args, kwargs, new_t)
                        logger.info(f"Self-Healing: Görev [{name}] başarıyla yeniden başlatıldı ve canlandırıldı.")
            except Exception as loop_e:
                logger.error(f"TaskSupervisor döngü hatası: {loop_e}")
                await asyncio.sleep(5)

    def stop(self):
        self._is_running = False
        for name, (_, _, _, task) in self.tasks.items():
            if task and not task.done():
                task.cancel()


TASK_SUPERVISOR_INSTANCE: Optional['TaskSupervisor'] = None

async def generate_status_live_card(lang: str = "tr") -> str:
    ...
    if days > 0:
        uptime_str = f"{days}g {hours}s {mins}dk"
    elif hours > 0:
        uptime_str = f"{hours}s {mins}dk {secs}sn"
    else:
        uptime_str = f"{mins}dk {secs}sn"
        ...
        with open('/proc/self/status') as f:
            for line in f:
                if line.startswith('VmRSS:'):
                    ram_mb = round(float(line.split()[1]) / 1024.0, 1)
                    break
    except Exception:
        try:
            import resource
            ram_mb = round(resource.getrusage(resource.RUSAGE_SELF).ru_maxrss / 1024.0, 1)
        except Exception:
            ram_mb = 0.0

    # 3. Database Latency & DB Size & Active Users
    db_latency_ms = 0.0
    active_24h = 0
    total_users = 0
    async with AsyncSessionLocal() as session:
        ...
        db_latency_ms = round((time.time() - t0) * 1000.0, 1)

        twenty_four_ago = datetime.utcnow() - timedelta(hours=24)
        active_24h = (await session.execute(
            select(func.count(User.telegram_id)).where(User.last_seen_at >= twenty_four_ago)
        )).scalar() or 0
    ...
    if "sqlite" in DATABASE_URL:
        ...
        db_size_str = "PostgreSQL Pool (10/15)"

    # 4. SecOps Telemetry (24h)
    sec = BOT_SECURITY_ENGINE
    shadow_banned_active = sum(1 for p in sec.profiles.values() if p.is_shadow_banned)
    tarpit_active = sum(1 for p in sec.profiles.values() if p.risk_score >= sec.tarpit_threshold)

    # 5. Task Supervisor Telemetry
    sup = TASK_SUPERVISOR_INSTANCE
    total_tasks = len(sup.tasks) if sup else 7
    active_tasks = sum(1 for _, _, _, t in sup.tasks.values() if t and not t.done()) if sup else 7

    now_clock = get_local_now().strftime('%d.%m.%Y %H:%M:%S')

    titles_map = {
        "tr": (
            "📊 <b>CANLI SİSTEM SAĞLIK VE DEVOPS GÖZLEM MASASI</b>",
            "Uptime (Çalışma Süresi)", "Sistem Durumu", "AKTİF & SAĞLIKLI",
            "KAYNAK VE ALTYAPI DURUMU", "Bellek Kullanımı (RAM)", "Veritabanı Yanıt Süresi", "Veritabanı Boyutu",
            "KULLANICI AKTİVİTESİ (CANLI)", "Son 24 Saatte Aktif", "Toplam Kayıtlı Kullanıcı",
            "SİBER GÜVENLİK VE SECOPS İSTATİSTİKLERİ", "İncelenen İstekler", "Tarpit Uygulanan İstekler", "Shadow-Ban / Karantinadaki Botlar", "Yakalana Tuzak / Fuzzer Girişimleri",
            "SELF-HEALING VE GÖREV GÖZETMENİ", "Watchdog Durumu", "AKTİF", "Arka Plan Görevleri"
        ),
        "ru": (
            "📊 <b>МОНИТОРИНГ ЗДОРОВЬЯ СИСТЕМЫ И DEVOPS</b>",
            "Время работы (Аптайм)", "Статус системы", "АКТИВЕН И СТАБИЛЕН",
            "РЕСУРСЫ И ИНФРАСТРУКТУРА", "Память (RAM)", "Время отклика базы данных", "Размер базы данных",
            "АКТИВНОСТЬ ПОЛЬЗОВАТЕЛЕЙ", "Активны за 24 часа", "Всего пользователей",
            "СТАТИСТИКА БЕЗОПАСНОСТИ (SECOPS)", "Проверено запросов", "Замедлено (Tarpit)", "В изоляции (Shadow-Ban)", "Перехвачено атак / Fuzzing",
            "САМОИСЦЕЛЕНИЕ И WATCHDOG", "Статус супервизора", "АКТИВЕН", "Фоновые процессы"
        ),
        "uz": (
            "📊 <b>JONLI TIZIM SALOMATLIGI VA DEVOPS MONITORINGI</b>",
            "Ish vaqti (Uptime)", "Tizim holati", "FAOL VA BARQAROR",
            "RESURSLAR VA INFRASTRUKTURA", "Xotira (RAM)", "Ma'lumotlar bazasi javob vaqti", "Baza hajmi",
            "FOYDALANUVCHILAR FAOLLIGI", "24 soatda faollar", "Jami foydalanuvchilar",
            "XAVFSIZLIK STATISTIKASI (SECOPS)", "Tekshirilgan so'rovlar", "Sekinlashtirilgan (Tarpit)", "Bloklangan botlar (Shadow-Ban)", "Aniqlangan shubhali probalar",
            "O'Z-O'ZINI TIKLASH VA WATCHDOG", "Supervayzer holati", "FAOL", "Orqa fon jarayonlari"
        ),
        "en": (
            "📊 <b>LIVE SYSTEM HEALTH & DEVOPS OBSERVABILITY DESK</b>",
            "Uptime", "System Status", "ACTIVE & HEALTHY",
            "RESOURCES & INFRASTRUCTURE", "Memory Usage (RAM)", "Database Latency", "Database Size",
            "USER ACTIVITY (LIVE)", "Active in Last 24h", "Total Registered Users",
            "CYBERSECURITY & SECOPS TELEMETRY", "Total Inspected Requests", "Tarpit Throttled Requests", "Shadow-Banned Bots / Fuzzers", "Traps / Probes Intercepted",
            "SELF-HEALING & TASK SUPERVISOR", "Watchdog Status", "ACTIVE", "Background Workers"
        )
    }
    titles = titles_map.get(lang, titles_map["tr"])
    t_hdr, t_upt, t_stt, t_stt_val, t_res, t_ram, t_dbl, t_dbs, t_usr, t_u24, t_utot, t_sec, t_reqs, t_tarp, t_shd, t_prb, t_sh, t_wch, t_wch_val, t_wrk = titles

    card = (
        f"{t_hdr}\n"
        "────────────────────\n"
        f"🕒 <b>{t_upt}:</b> <code>{uptime_str}</code>\n"
        f"⚡ <b>{t_stt}:</b> 🟢 <b>{t_stt_val}</b>\n\n"
        f"🖥️ <b>{t_res}</b>\n"
        f"• 🧠 <b>{t_ram}:</b> <code>{ram_mb} MB</code>\n"
        f"• 🗄️ <b>{t_dbl}:</b> <code>{db_latency_ms} ms</code>\n"
        f"• 📦 <b>{t_dbs}:</b> <code>{db_size_str}</code>\n\n"
        f"👥 <b>{t_usr}</b>\n"
        f"• 🟢 <b>{t_u24}:</b> <b>{active_24h}</b>\n"
        f"• 👥 <b>{t_utot}:</b> <b>{total_users}</b>\n\n"
        f"🛡️ <b>{t_sec}</b>\n"
        f"• 🔍 <b>{t_reqs}:</b> <code>{sec.total_inspected_requests}</code>\n"
        f"• ⏳ <b>{t_tarp}:</b> <code>{sec.tarpit_event_count}</code> ({tarpit_active} aktif)\n"
        f"• 🚫 <b>{t_shd}:</b> <code>{shadow_banned_active}</code>\n"
        f"• ⚠️ <b>{t_prb}:</b> <code>{sec.probes_detected_count}</code>\n\n"
        f"⚙️ <b>{t_sh}</b>\n"
        f"• 🛡️ <b>{t_wch}:</b> 🟢 <b>{t_wch_val}</b>\n"
        f"• 🔄 <b>{t_wrk}:</b> <code>{active_tasks}/{total_tasks}</code>\n"
        "────────────────────\n"
        f"🕒 <i>{now_clock}</i>"
    )
    return card
...
class DatabaseResilienceManager:
    """
    Veritabanı kilitlenme, havuz bayatlama ve düşme durumlarında
    sorguları otomatik kurtaran ve kilit çözen dayanıklılık kalkanı.
    """
    @staticmethod
    async def execute_resilient(operation: Callable[[], Awaitable[Any]], max_retries: int = 3, initial_delay: float = 0.15) -> Any:
        last_exc = None
        for attempt in range(max_retries):
            try:
                ...
                last_exc = e
                err_str = str(e).lower()
                is_lock = any(k in err_str for k in ("locked", "busy", "timeout", "closed", "connection", "terminat"))
                if is_lock and attempt < max_retries - 1:
                    logger.warning(f"Self-Healing DB: Kilitlenme/Bağlantı hatası ({err_str[:60]}), kurtarma deneniyor ({attempt+1}/{max_retries})...")
                    try:
...
class AdvancedBotSecurityMiddleware(BaseMiddleware):
    async def __call__(
        self,
        handler: Callable[[TelegramObject, Dict[str, Any]], Awaitable[Any]],
        event: TelegramObject,
        ...
        # Tam Güvenlik ve Kesintisiz Akış: Hiçbir kullanıcı veya buton isteği engellenmez
        return await handler(event, data)
...
def format_telegram_html(text: str) -> str:
    ...
    # 3. Aşırı uzun ve mobilde taşan çizgileri standart zarif ayraca (────────────────────) çevir
    t = re.sub(r'━{4,}', '────────────────────', t)
    t = re.sub(r'─{4,}', '────────────────────', t)
    t = re.sub(r'-{4,}', '────────────────────', t)
    t = re.sub(r'_{4,}', '────────────────────', t)
...
async def purge_previous_bot_messages(bot_obj: Bot, chat_id: int, keep_msg_id: int | None = None):
    ...
    # Perform parallel batch deletions via asyncio.gather for instant response
    if to_delete:
        async def _safe_del(m_id: int):
            try:
                await bot_obj.delete_message(chat_id=chat_id, message_id=m_id)
            except Exception:
                pass
        await asyncio.gather(*[_safe_del(m_id) for m_id in to_delete], return_exceptions=True)
...
async def safe_edit_or_answer(target: Message | CallbackQuery, text: str, reply_markup=None, parse_mode="HTML"):
    ...
    # Kullanıcı alt menü (Reply Keyboard) butonuna bastığında gelen mesajı temizle; form verilerini ASLA silme!
    if isinstance(target, Message):
        if target.text and match_reply_button(target.text):
            try: await target.delete()
            except Exception: pass

    # 1. Yerinde dönüştürülecek mesajı belirle
    edit_mid = None
    ...
    # 2. Kartı yerinde dönüştür (edit_message_text) - Yeni mesaj üretmeden akıcı güncelle
    if edit_mid and not is_reply_kb:
        try:
            ...
            LAST_MENU_MSG_ID[target_chat_id] = edit_mid
            ACTIVE_CHAT_MESSAGES.setdefault(target_chat_id, set()).add(edit_mid)
            return
        except Exception as e:
            err_s = str(e).lower()
    ...
    # 3. Yerinde dönüşüm yapılamadıysa yeni kartı gönder ve eskiyi temizle
...
SCHOOL_CLASSES_CACHE: list[str] = []
SCHOOL_CLASSES_CACHE_TTL: float = 0.0

def invalidate_school_classes_cache():
    global SCHOOL_CLASSES_CACHE_TTL
    SCHOOL_CLASSES_CACHE_TTL = 0.0

async def get_all_school_classes(session: AsyncSession) -> list[str]:
    global SCHOOL_CLASSES_CACHE, SCHOOL_CLASSES_CACHE_TTL
    now_t = time.time()
    if SCHOOL_CLASSES_CACHE and now_t < SCHOOL_CLASSES_CACHE_TTL:
        return list(SCHOOL_CLASSES_CACHE)
    ...
    res = sorted(list(raw), key=natural_sort_key)
    SCHOOL_CLASSES_CACHE = res
    SCHOOL_CLASSES_CACHE_TTL = now_t + 30.0
    return list(res)
...
async def init_db():
    ...
    async with engine.begin() as conn:
        ...
        # Run each migration in its own isolated sub-transaction to prevent Postgres InFailedSqlTransaction aborts!
        for col_sql in migration_sqls:
            pg_sql = col_sql
            if "sqlite" not in DATABASE_URL and "ADD COLUMN" in col_sql:
                pg_sql = col_sql.replace("ADD COLUMN", "ADD COLUMN IF NOT EXISTS")
            try:
                async with engine.begin() as col_conn:
                    await col_conn.exec_driver_sql(pg_sql)
            except Exception:
                try:
                    async with engine.begin() as col_conn:
                        await col_conn.exec_driver_sql(col_sql)
                except Exception:
                    pass
...
LOCALES = {
    'en': {
        ...
        'grade_parent_notification': '📝 <b>NEW GRADE ALERT</b>\n────────────────────\n🧑‍🎓 <b>Student:</b> {name}\n📚 <b>Subject:</b> {subject} ({exam_type})\n────────────────────\n📊 <b>Score:</b> <b>{score}</b> ({badge})\n────────────────────',
    ...
    'ru': {
        ...
        'grade_parent_notification': '📝 <b>НОВАЯ ОЦЕНКА</b>\n────────────────────\n🧑‍🎓 <b>Ученик:</b> {name}\n📚 <b>Предмет:</b> {subject} ({exam_type})\n────────────────────\n📊 <b>Оценка:</b> <b>{score}</b> ({badge})\n────────────────────',
    ...
    'tr': {
        ...
        'grade_parent_notification': '📝 <b>YENİ NOT GİRİŞİ</b>\n────────────────────\n🧑‍🎓 <b>Öğrenci:</b> {name}\n📚 <b>Ders:</b> {subject} ({exam_type})\n────────────────────\n📊 <b>Not:</b> <b>{score}</b> ({badge})\n────────────────────',
    ...
    'uz': {
        ...
        'grade_parent_notification': '📝 <b>YANGI BAHO QO\'YILDI</b>\n────────────────────\n🧑‍🎓 <b>O\'quvchi:</b> {name}\n📚 <b>Fan:</b> {subject} ({exam_type})\n────────────────────\n📊 <b>Baho:</b> <b>{score}</b> ({badge})\n────────────────────',
...
def get_role_reply_kb(role: str, lang: str = "tr", linked_count: int = 1) -> ReplyKeyboardMarkup:
    ...
    return ReplyKeyboardMarkup(keyboard=keyboard, resize_keyboard=True, one_time_keyboard=False, is_persistent=True)
...
def get_inline_cancel_kb(lang: str = "tr", back_callback: str = "cancel_action") -> InlineKeyboardMarkup:
    # Direct callback routing: clicking the cancel/back button takes the user directly
    # to the destination handler (e.g. adm:teachers, adm:classes) with zero indirection!
    target = back_callback if back_callback else "cancel_action"
    return InlineKeyboardMarkup(inline_keyboard=[[
        InlineKeyboardButton(text=get_text("btn_cancel_action", lang), callback_data=target)
    ]])
...
def get_nav_buttons(lang: str = "tr", back_callback: str = "adm:dashboard", include_home: bool = True) -> list:
    if not back_callback:
        return []
    btns = [InlineKeyboardButton(text=get_text("btn_back", lang), callback_data=back_callback)]
    if include_home and back_callback not in ("adm:dashboard", "nav:main_menu", "tch:menu", "pr:menu", "st:menu", "act_main_menu"):
        home_lbl = {"tr": "🏠 Ana Menü", "ru": "🏠 Главное меню", "uz": "🏠 Asosiy menyu", "en": "🏠 Main Menu"}.get(lang, "🏠 Main Menu")
        btns.append(InlineKeyboardButton(text=home_lbl, callback_data="nav:main_menu"))
    return btns
...
async def generate_user_forensic_dossier_pdf(target_user_id: int, lang: str = "tr") -> io.BytesIO:
    async with AsyncSessionLocal() as session:
        ...
        names_log = []
        try:
            names_log = json.loads(u.previous_names) if u.previous_names else []
        except Exception:
            names_log = []
    ...
    t_style = ParagraphStyle('FTitle', parent=styles['Heading1'], fontName=fn_bold, fontSize=13, leading=16, textColor=colors.HexColor('#1A365D'), alignment=1)
    c_bold = ParagraphStyle('FCB', parent=styles['Normal'], fontName=fn_bold, fontSize=8, leading=10, textColor=colors.HexColor('#1A202C'))
    c_norm = ParagraphStyle('FCN', parent=styles['Normal'], fontName=fn_reg, fontSize=7.5, leading=9.5, textColor=colors.HexColor('#2D3748'))
    c_center_b = ParagraphStyle('FCCB', parent=styles['Normal'], fontName=fn_bold, fontSize=8, leading=10, textColor=colors.HexColor('#1A202C'), alignment=1)
    c_center = ParagraphStyle('FCC', parent=styles['Normal'], fontName=fn_reg, fontSize=7.5, leading=9.5, textColor=colors.HexColor('#2D3748'), alignment=1)

    pdf_title_txt = {
        "tr": "RESMİ ADLİ İSTİHBARAT VE GÜVENLİK DOSYASI",
        "ru": "ОФИЦИАЛЬНОЕ ДОСЬЕ БЕЗОПАСНОСТИ И АУДИТА",
        "uz": "RASMIY SUD-EKSPERTIZA VA XAVFSIZLIK DOSYESI",
        "en": "OFFICIAL SECURITY & AUDIT DOSSIER"
    }.get(lang, "OFFICIAL SECURITY & AUDIT DOSSIER")

    pdf_sub_txt = {
        "tr": "KURUMSAL BİLGİ VE TELEMETRİ SİSTEMİ",
        "ru": "КОРПОРАТИВНАЯ СИСТЕМА УЧЕТА И ТЕЛЕМЕТРИИ",
        "uz": "KORPORATIV MA'LUMOT VA TELEMETRIYA TIZIMI",
        "en": "ENTERPRISE TELEMETRY & SECURITY SYSTEM"
    }.get(lang, "ENTERPRISE TELEMETRY SYSTEM")

    story = [
        Paragraph(f"<b>{pdf_title_txt}</b>", t_style),
        Paragraph(f"<font size=7 color='#718096'>{pdf_sub_txt}</font>", c_center),
        Spacer(1, 10)
    ]

    date_str = (datetime.utcnow() + timedelta(hours=TIMEZONE_OFFSET)).strftime('%d.%m.%Y %H:%M')
    
    st_banned_lbl = {"tr": "YASAKLI", "ru": "ЗАБЛОКИРОВАН", "uz": "BLOKLANGAN", "en": "BANNED"}.get(lang, "BANNED")
    st_bot_del_lbl = {"tr": "BOTU SİLDİ", "ru": "УДАЛИЛ БОТА", "uz": "BOTNI O'CHIRDI", "en": "REMOVED BOT"}.get(lang, "REMOVED")
    st_act_lbl = {"tr": "AKTİF", "ru": "АКТИВЕН", "uz": "FAOL", "en": "ACTIVE"}.get(lang, "ACTIVE")
    status_disp = st_banned_lbl if u.is_blacklisted else (st_bot_del_lbl if u.is_bot_blocked else st_act_lbl)

    lbl_meta = {
        "name": {"tr": "Ad Soyad:", "ru": "ФИО:", "uz": "F.I.O:", "en": "Full Name:"}.get(lang, "Name:"),
        "dt": {"tr": "Tarih / Saat:", "ru": "Дата / Время:", "uz": "Sana / Vaqt:", "en": "Date / Time:"}.get(lang, "Date:"),
        "tgid": {"tr": "Telegram ID:", "ru": "Telegram ID:", "uz": "Telegram ID:", "en": "Telegram ID:"}.get(lang, "Telegram ID:"),
        "uname": {"tr": "Kullanıcı Adı:", "ru": "Юзернейм:", "uz": "Foydalanuvchi:", "en": "Username:"}.get(lang, "Username:"),
        "role": {"tr": "Sistem Rolü:", "ru": "Роль в системе:", "uz": "Tizimdagi roli:", "en": "System Role:"}.get(lang, "Role:"),
        "stat": {"tr": "İdari Statü:", "ru": "Статус:", "uz": "Status:", "en": "Status:"}.get(lang, "Status:"),
        "acc": {"tr": "Hesap Durumu:", "ru": "Состояние аккаунта:", "uz": "Hisob holati:", "en": "Account State:"}.get(lang, "State:"),
        "night": {"tr": "Gece Aktivitesi:", "ru": "Ночная активность:", "uz": "Tungi faollik:", "en": "Night Activity:"}.get(lang, "Night:"),
        "dl": {"tr": "İndirilen Dosyalar:", "ru": "Скачанные файлы:", "uz": "Yuklangan fayllar:", "en": "Downloads:"}.get(lang, "Downloads:"),
        "act": {"tr": "Son İşlem:", "ru": "Посл. действие:", "uz": "Oxirgi amal:", "en": "Last Action:"}.get(lang, "Action:"),
        "unknown": {"tr": "Bilinmeyen", "ru": "Неизвестно", "uz": "Noma'lum", "en": "Unknown"}.get(lang, "Unknown"),
        "none": {"tr": "Yok", "ru": "Нет", "uz": "Yo'q", "en": "None"}.get(lang, "None"),
        "norec": {"tr": "Kayıt yok", "ru": "Нет записей", "uz": "Yozuv yo'q", "en": "No records"}.get(lang, "None"),
        "ops": {"tr": "işlem", "ru": "действий", "uz": "amal", "en": "actions"}.get(lang, "actions"),
        "files": {"tr": "adet", "ru": "шт.", "uz": "ta", "en": "files"}.get(lang, "files")
    }

    meta = [
        [Paragraph(f"<b>{lbl_meta['name']}</b>", c_bold), Paragraph(escape_html(u.full_name or lbl_meta['unknown']), c_norm), Paragraph(f"<b>{lbl_meta['dt']}</b>", c_bold), Paragraph(date_str, c_norm)],
        [Paragraph(f"<b>{lbl_meta['tgid']}</b>", c_bold), Paragraph(str(u.telegram_id), c_norm), Paragraph(f"<b>{lbl_meta['uname']}</b>", c_bold), Paragraph(f"@{u.username}" if u.username else lbl_meta['none'], c_norm)],
        [Paragraph(f"<b>{lbl_meta['role']}</b>", c_bold), Paragraph(u.role.upper(), c_norm), Paragraph(f"<b>{lbl_meta['stat']}</b>", c_bold), Paragraph(u.admin_type.upper(), c_norm)],
        [Paragraph(f"<b>{lbl_meta['acc']}</b>", c_bold), Paragraph(status_disp, c_norm), Paragraph(f"<b>{lbl_meta['night']}</b>", c_bold), Paragraph(f"{u.night_activity_count} {lbl_meta['ops']}", c_norm)],
        [Paragraph(f"<b>{lbl_meta['dl']}</b>", c_bold), Paragraph(f"{u.download_count} {lbl_meta['files']}", c_norm), Paragraph(f"<b>{lbl_meta['act']}</b>", c_bold), Paragraph(escape_html(u.last_action_desc or lbl_meta['norec']), c_norm)],
    ]
    ...
    t_persona_hdr = {"tr": "ETKİLEŞİM VE İLGİ DAĞILIMI (TYPOLOGY PERSONA)", "ru": "РАСПРЕДЕЛЕНИЕ АКТИВНОСТИ И ИНТЕРЕСОВ (ПЕРСОНА)", "uz": "FAOLLIK VA QIZIQISHLAR TAQSIMOTI (PERSONA)", "en": "INTERACTION & PERSONA TYPOLOGY"}.get(lang, "INTERACTION TYPOLOGY")
    story.append(Paragraph(f"<b>{t_persona_hdr}</b>", c_bold))
    story.append(Spacer(1, 4))
    
    col_cat = {"tr": "Kategori", "ru": "Категория", "uz": "Kategoriya", "en": "Category"}.get(lang, "Category")
    col_cnt = {"tr": "İşlem Adedi", "ru": "Кол-во действий", "uz": "Amallar soni", "en": "Action Count"}.get(lang, "Count")
    gen_act_lbl = {"tr": "Genel Etkileşim", "ru": "Общая активность", "uz": "Umumiy faollik", "en": "General Interaction"}.get(lang, "General")

    p_rows = [[Paragraph(f"<b>{col_cat}</b>", c_center_b), Paragraph(f"<b>{col_cnt}</b>", c_center_b)]]
    if not p_counts:
        p_rows.append([Paragraph(gen_act_lbl, c_norm), Paragraph("0", c_center)])
    else:
        for k, v in p_counts.items():
            p_rows.append([Paragraph(k.capitalize(), c_norm), Paragraph(str(v), c_center)])
    t_persona = Table(p_rows, colWidths=[300, 247])
    ...
    t_id_hdr = {"tr": "KİMLİK VE KULLANICI ADI DEĞİŞİM GEÇMİŞİ (IDENTITY LOG)", "ru": "ИСТОРИЯ ИЗМЕНЕНИЙ ИМЕНИ И ЮЗЕРНЕЙМА (IDENTITY LOG)", "uz": "ISM VA YUZERNEYM O'ZGARISH TARIXI (IDENTITY LOG)", "en": "IDENTITY & USERNAME CHANGE HISTORY"}.get(lang, "IDENTITY CHANGE HISTORY")
    story.append(Paragraph(f"<b>{t_id_hdr}</b>", c_bold))
    story.append(Spacer(1, 4))
    id_rows = [[Paragraph("<b>Tür</b>", c_center_b), Paragraph("<b>Eski Değer</b>", c_center_b), Paragraph("<b>Yeni Değer</b>", c_center_b), Paragraph("<b>Tarih</b>", c_center_b)]]
    if not names_log:
        id_rows.append([Paragraph("Kayıt Yok", c_norm), Paragraph("Sabit", c_center), Paragraph("Sabit", c_center), Paragraph("-", c_center)])
    else:
        for item in names_log:
            id_rows.append([
                Paragraph(escape_html(item.get("type", "Bilinmeyen")), c_norm),
                Paragraph(escape_html(item.get("old", "-")), c_norm),
                Paragraph(escape_html(item.get("new", "-")), c_norm),
                Paragraph(escape_html(item.get("date", "-")), c_center)
            ])
    t_id = Table(id_rows, colWidths=[100, 170, 170, 107])
    ...
    h_sha = hashlib.sha256(f"{target_user_id}_{date_str}_{u.role}".encode()).hexdigest()[:24].upper()
    story.append(Paragraph(f"<font size=6 color='#A0AEC0'>DİJİTAL ADLİ MÜHÜR VE DOĞRULAMA KODU: SHA256#{h_sha}</font>", c_center))
        ...
        for att, st in records:
            if att.status == "absent":
                ...
                for p_id in parents:
                    ...
                    msg_text = {
                        "tr": (
                            "🚨 <b>DEVAMSIZLIK BİLDİRİMİ</b>\n"
                            "────────────────────\n"
                            f"🧑‍🎓 <b>Öğrenci:</b> {escape_html(st.full_name)}\n"
                            f"📅 <b>Tarih:</b> {att.date}\n"
                            "────────────────────\n"
                            "⚠️ <b>Durum:</b> GELMEDİ (YOK)\n"
                            "────────────────────"
                        ),
                        "ru": (
                            "🚨 <b>УВЕДОМЛЕНИЕ О ПРОПУСКЕ</b>\n"
                            "────────────────────\n"
                            f"🧑‍🎓 <b>Ученик:</b> {escape_html(st.full_name)}\n"
                            f"📅 <b>Дата:</b> {att.date}\n"
                            "────────────────────\n"
                            "⚠️ <b>Статус:</b> ОТСУТСТВУЕТ\n"
                            "────────────────────"
                        ),
                        "uz": (
                            "🚨 <b>DAVOMAT OGOHLANTIRISHI</b>\n"
                            "────────────────────\n"
                            f"🧑‍🎓 <b>O'quvchi:</b> {escape_html(st.full_name)}\n"
                            f"📅 <b>Sana:</b> {att.date}\n"
                            "────────────────────\n"
                            "⚠️ <b>Holat:</b> KELMADI\n"
                            "────────────────────"
                        ...
                        "en": (
                            "🚨 <b>ABSENCE ALERT</b>\n"
                            "────────────────────\n"
                            f"🧑‍🎓 <b>Student:</b> {escape_html(st.full_name)}\n"
                            f"📅 <b>Date:</b> {att.date}\n"
                            "────────────────────\n"
                            "⚠️ <b>Status:</b> ABSENT\n"
                            "────────────────────"
                        )
                    }.get(p_lang, f"🚨 ABSENCE: {escape_html(st.full_name)}")
...
async def run_evening_briefing_worker(bot: Bot):
    async with AsyncSessionLocal() as session:
        ...
        for p in parents:
            ...
            p_l_code = p.language or "tr"
            if not att or att.status == "present":
                att_status = {"tr": "✅ Geldi", "ru": "✅ Присутствовал(а)", "uz": "✅ Keldi", "en": "✅ Present"}.get(p_l_code, "✅ Present")
            elif att.status == "absent":
                att_status = {"tr": "❌ Gelmedi", "ru": "❌ Отсутствовал(а)", "uz": "❌ Kelmadi", "en": "❌ Absent"}.get(p_l_code, "❌ Absent")
            else:
                att_status = {"tr": "🏥 İzinli", "ru": "🏥 Справка / Уважительно", "uz": "🏥 Ruxsatli", "en": "🏥 Excused"}.get(p_l_code, "🏥 Excused")
...
async def run_morning_briefing_worker(bot: Bot):
    async with AsyncSessionLocal() as session:
        ...
        for a_id in set(ADMIN_IDS + list(admin_lang_map.keys())):
            ...
            all_taken_str = {"tr": "Tümü Alındı", "ru": "Все приняты", "uz": "Barchasi olindi", "en": "All Completed"}.get(a_lang, "All Completed")
            missing_str = ", ".join(missing) if missing else all_taken_str
            msg = {
                "tr": (
                    "☀️ <b>GÜNLÜK İDARİ BRİFİNG (09:30)</b>\n"
                    "────────────────────\n"
                    f"🏫 <b>Toplam Öğrenci:</b> {total_students}\n"
                    f"🟢 <b>Gelen:</b> {present} • 🔴 <b>Gelmeyen:</b> {absent}\n"
                    "────────────────────\n"
                    f"⚠️ <b>Yoklama Girmeyen:</b> {missing_str}\n"
                    "────────────────────"
                ),
                "ru": (
                    "☀️ <b>ЕЖЕДНЕВНЫЙ ОТЧЕТ ДИРЕКТОРУ (09:30)</b>\n"
                    "────────────────────\n"
                    f"🏫 <b>Всего учеников:</b> {total_students}\n"
                    f"🟢 <b>Присутствуют:</b> {present} • 🔴 <b>Отсутствуют:</b> {absent}\n"
                    "────────────────────\n"
                    f"⚠️ <b>Без переклички:</b> {missing_str}\n"
                    "────────────────────"
                ),
                "uz": (
                    "☀️ <b>KUNLIK TONGGI HISOBOT (09:30)</b>\n"
                    "────────────────────\n"
                    f"🏫 <b>Jami o'quvchilar:</b> {total_students}\n"
                    f"🟢 <b>Kelgan:</b> {present} • 🔴 <b>Kelmagan:</b> {absent}\n"
                    "────────────────────\n"
                    f"⚠️ <b>Davomat olinmagan:</b> {missing_str}\n"
                    "────────────────────"
                ),
                "en": (
                    "☀️ <b>DAILY EXECUTIVE BRIEFING (09:30)</b>\n"
                    "────────────────────\n"
                    f"🏫 <b>Total Students:</b> {total_students}\n"
                    f"🟢 <b>Present:</b> {present} • 🔴 <b>Absent:</b> {absent}\n"
                    "────────────────────\n"
                    f"⚠️ <b>Pending Attendance:</b> {missing_str}\n"
                    "────────────────────"
                )
            }.get(a_lang, f"☀️ DAILY BRIEFING: {present}/{total_students}")
...
async def record_user_intelligence_telemetry(user_id: int, full_name: str | None = None, username: str | None = None, action_type: str = "general", action_desc: str = ""):
    if not user_id:
        return
    now = datetime.utcnow()
    now_clock = (now + timedelta(hours=TIMEZONE_OFFSET)).strftime("%H:%M")
    full_action = f"{action_desc} ({now_clock})" if action_desc else f"Aktif ({now_clock})"
    try:
        async with AsyncSessionLocal() as session:
            ...
            if not u:
                try:
                    ...
                    u = User(telegram_id=user_id, role=role, language="tr", full_name=full_name, username=username, download_count=0, night_activity_count=0, persona_counts="{}", previous_names="[]")
                    session.add(u)
                    await session.flush()
                except Exception:
            ...
            u.last_seen_at = now
            u.last_action_desc = full_action[:250]

            local_hour = (now.hour + TIMEZONE_OFFSET) % 24
            if 1 <= local_hour < 6:
                curr_n = int(u.night_activity_count) if isinstance(getattr(u, 'night_activity_count', None), (int, float)) else 0
                u.night_activity_count = curr_n + 1
            ...
            if action_type == "download":
                curr_d = int(u.download_count) if isinstance(getattr(u, 'download_count', None), (int, float)) else 0
                u.download_count = curr_d + 1
...
async def cmd_profile(message: Message):
    async with AsyncSessionLocal() as session:
        ...
        if user.role == "parent" and user.current_child_id:
            ...
            if st: extra = f"🧑‍🎓 <b>{lbl_st}:</b> {escape_html(st.full_name)} ({escape_html(st.class_name)})\n"
        elif user.role == "student":
            st = (await session.execute(select(Student).where(Student.student_telegram_id == user.telegram_id))).scalar_one_or_none()
            if st: extra = f"🏫 <b>{lbl_cl}:</b> {escape_html(st.class_name)} • <b>{lbl_no}:</b> <code>{escape_html(st.student_number)}</code>\n"
        elif user.role == "teacher":
            tch = (await session.execute(select(Teacher).where(Teacher.telegram_id == user.telegram_id))).scalar_one_or_none()
            if tch: extra = f"📚 <b>{lbl_s}:</b> {escape_html(tch.subject)}\n"

        p_title = get_text("uc_card_title", lang)
        lbl_r = get_text("lbl_role", lang)
        lbl_user = get_text("lbl_role_guest", lang)
        lbl_phn = {"tr": "Telefon", "ru": "Телефон", "uz": "Telefon", "en": "Phone"}.get(lang, "Phone")
        lbl_lng = {"tr": "Dil", "ru": "Язык", "uz": "Til", "en": "Language"}.get(lang, "Language")
        lbl_none = {"tr": "Kayıtlı Değil", "ru": "Не указан", "uz": "Kiritilmagan", "en": "Not provided"}.get(lang, "Not provided")
        phn_str = user.phone if user.phone else lbl_none
        usr_tag = f" (@{user.username})" if user.username else ""

        text = (
            f"<b>{p_title}</b>\n"
            "────────────────────\n"
            f"👤 <b>{escape_html(user.full_name or lbl_user)}</b>{usr_tag}\n"
            f"🆔 <code>{user.telegram_id}</code> • <b>{lbl_r}:</b> {role_label}\n"
            "────────────────────\n"
            f"📱 <b>{lbl_phn}:</b> <code>{escape_html(phn_str)}</code>\n"
            f"🌐 <b>{lbl_lng}:</b> {user.language.upper()}\n"
        )
        if extra:
            text += "────────────────────\n" + extra
        text += "────────────────────"
        await safe_edit_or_answer(message, text, parse_mode="HTML")
...
async def cmd_cancel(message: Message, state: FSMContext):
    ...
    for cache_dict in [ATTENDANCE_CACHE, GRADE_CACHE, HW_CACHE, REQ_CACHE, APP_CACHE, BEHAVIOR_CACHE, SUBMISSION_CACHE, EXAM_CACHE, BC_CACHE]:
...
@router.callback_query(F.data.startswith("cancel_action"))
async def cb_cancel_action(query: CallbackQuery, state: FSMContext | None = None):
    if state:
        try:
            await state.clear()
        except Exception:
    ...
    for cache_dict in [ATTENDANCE_CACHE, GRADE_CACHE, HW_CACHE, REQ_CACHE, APP_CACHE, BEHAVIOR_CACHE, SUBMISSION_CACHE, EXAM_CACHE, BC_CACHE, PROP_CACHE]:
        cache_dict.pop(user_id, None)
    ...
    target = raw_data.split(":", 1)[1] if ":" in raw_data else ""
    if target:
        # Avoid mutating query.data directly on frozen Pydantic model
        if target.startswith("prop:results:"):
            ...
            return
        elif target == "adm:cat_reports":
            await cb_cat_reports(query, state)
            return
        elif target == "adm:cat_requests":
            await cb_cat_requests(query, state)
            return
        elif target == "adm:cat_tools":
            await cb_cat_tools(query, state)
            return
            ...
            await cb_admin_teachers_list(query, state)
            return
        elif target.startswith("adm:show_class:"):
            await cb_show_class_students(query)
            return
        elif target.startswith("adm:st_card:"):
            await cb_student_card(query)
            return
        elif target.startswith("adm:tch_card:"):
            await cb_admin_teacher_card(query)
            return
        elif target.startswith("adm:users_hub:"):
            await cb_admin_users_hub(query)
            return
        elif target == "adm:admins_list":
            await cb_admin_admins_list(query)
            return
        elif target == "adm:requests_list":
            await cb_admin_requests_list(query)
            return
        elif target == "adm:medical_list":
            await cb_medical_list(query)
            return
        elif target in ("adm:dashboard", "nav:main_menu"):
            await cb_nav_to_main_menu(query, state)
            return
        elif target == "tch:prop_hub":
            await cb_teacher_proposal_hub(query)
            return
        elif target == "tch:hw_classes":
            await cb_hw_classes(query, state)
            return
        elif target == "tch:view_my_hws":
            await cb_teacher_view_my_hws(query)
            return
        elif target == "tch:appointments":
            await cb_teacher_appointments_list(query)
            return
        elif target == "tch:grade_classes":
            await cb_grade_classes(query, state)
            return
        elif target == "tch:recent_grades":
            await cb_teacher_recent_grades(query)
            return
        elif target == "upload_medical_init":
            await cb_upload_med_init(query, state)
            return
        elif target == "cb_digital_locker":
            await cb_digital_locker(query, state)
            return
        elif target.startswith("adm:view_req:"):
            await cb_admin_view_request(query, state)
            return
    ...
    if user and user.role != "guest":
        ...
        guest_kb = get_role_reply_kb("guest", lang)
        await safe_edit_or_answer(query, get_text("welcome_guest", lang), reply_markup=guest_kb, parse_mode="HTML")
...
async def get_dashboard_card_text(user: User) -> str:
    ...
    async with AsyncSessionLocal() as session:
        ...
        if user.role == "admin":
            ...
            name = escape_html(st.full_name) if st else "-"
            cls_name = escape_html(st.class_name) if st else "-"
        elif user.role == "student":
            st = (await session.execute(select(Student).where(Student.student_telegram_id == user.telegram_id))).scalar_one_or_none()
            name = escape_html(st.full_name) if st else (escape_html(user.full_name or "Öğrenci"))
            cls_name = escape_html(st.class_name) if st else "-"
            num_val = escape_html(st.student_number) if st else "-"
        elif user.role == "teacher":
            tch = (await session.execute(select(Teacher).where(Teacher.telegram_id == user.telegram_id))).scalar_one_or_none()
            ...
            if not raw_s or raw_s.lower() in ("genel", "nothing", "none", "-"):
                ...
                subj = raw_s
            name = escape_html(tch.full_name) if (tch and tch.full_name) else (escape_html(user.full_name or get_role_label("teacher", lang)))
...
async def process_auth_code_string(code: str, user_id: int, message: Message, state: FSMContext):
    ...
    async with AsyncSessionLocal() as session:
        ...
        if not auth_result:
            now = datetime.utcnow()
            f_list = AUTH_FAILURES_TRACKER.setdefault(user_id, [])
            f_list[:] = [t for t in f_list if (now - t).total_seconds() < 60]
            f_list.append(now)
            if len(f_list) >= 4:
                ...
                asyncio.create_task(alert_admins_security_breach(
                    bot=message.bot,
                    user_id=user_id,
                    user_name=user.full_name or "Bilinmeyen",
                    event_title="Sıralı Kod / Numara Tarama Şüphesi (ID Enumeration)",
                    details=f"Kullanıcı 60 saniye içinde {len(f_list)} adet geçersiz kod denedi. 5 dakika süreyle geçici olarak kilitlendi."
                ))
...
async def process_req_details(message: Message, state: FSMContext):
    ...
    async with AsyncSessionLocal() as session:
        user = await session.get(User, message.from_user.id)
        user_lang = user.language if user else "tr"
    if len(details_text) < 2:
        await message.answer(get_text("err_invalid_details_strict", user_lang))
        return
    ...
    async with AsyncSessionLocal() as session:
        ...
        try:
            ...
            if len(parts) == 3:
                ...
                age_str = f" ({age_val} {get_text('lbl_age', a_lang)})"
        except Exception: pass
...
async def cb_admin_send_credentials(query: CallbackQuery):
    ...
    async with AsyncSessionLocal() as session:
        ...
        req = await session.get(AccessRequest, req_id)
        if not req:
            await query.answer({"tr": "⚠️ Başvuru bulunamadı.", "ru": "⚠️ Заявка не найдена.", "uz": "⚠️ Ariza topilmadi.", "en": "⚠️ Application not found."}.get(lang, "⚠️ Application not found."), show_alert=True)
            return
...
async def cb_appr_select_class(query: CallbackQuery, state: FSMContext):
    class_name = ":".join(query.data.split(":")[2:]).strip().upper()
...
async def cb_appr_st_finalize(query: CallbackQuery, state: FSMContext):
    ...
    await query.answer({"tr": "✅ Öğrenci eklendi!", "ru": "✅ Ученик добавлен!", "uz": "✅ O'quvchi qo'shildi!", "en": "✅ Student added!"}.get(lang, "✅ Student added!"))
...
async def cb_teacher_view_appointment(query: CallbackQuery):
    ...
    async with AsyncSessionLocal() as session:
        ...
        p_def = {"tr": "Veli", "ru": "Родитель", "uz": "Ota-ona", "en": "Parent"}.get(lang, "Parent")
        p_name = escape_html(parent_user.full_name) if parent_user else p_def

        st_lbl = {"pending": {"tr": "⏳ Onay Bekliyor", "ru": "⏳ Ожидает подтверждения", "uz": "⏳ Tasdiq kutilmoqda", "en": "⏳ Pending"}, "approved": {"tr": "✅ Onaylandı", "ru": "✅ Подтверждено", "uz": "✅ Tasdiqlangan", "en": "✅ Approved"}, "rejected": {"tr": "❌ Reddedildi", "ru": "❌ Отклонено", "uz": "❌ Rad etilgan", "en": "❌ Rejected"}}.get(app.status, {}).get(lang, app.status)

        hdr_app = {"tr": f"🤝 <b>VELİ GÖRÜŞME TALEBİ (#{app.id})</b>", "ru": f"🤝 <b>ЗАПРОС НА ВСТРЕЧУ С РОДИТЕЛЕМ (#{app.id})</b>", "uz": f"🤝 <b>OTA-ONA BILAN UCHRASHUV SO'ROVI (#{app.id})</b>", "en": f"🤝 <b>PARENT APPOINTMENT REQUEST (#{app.id})</b>"}.get(lang, "🤝 <b>APPOINTMENT REQUEST</b>")
        lbl_p = {"tr": "Veli", "ru": "Родитель", "uz": "Ota-ona", "en": "Parent"}.get(lang, "Parent")
        lbl_t = {"tr": "Talep Edilen Zaman", "ru": "Желаемое время", "uz": "So'ralgan vaqt", "en": "Requested Time"}.get(lang, "Time")
        lbl_n = {"tr": "Görüşme Konusu", "ru": "Тема / Вопрос", "uz": "Mavzu", "en": "Subject / Notes"}.get(lang, "Notes")
        lbl_s = {"tr": "Durum", "ru": "Статус", "uz": "Holat", "en": "Status"}.get(lang, "Status")
        lbl_act = {"tr": "Lütfen randevu talebine dair kararınızı veriniz:", "ru": "Пожалуйста, примите решение по запросу:", "uz": "Iltimos, uchrashuv so'rovi bo'yicha qaror qabul qiling:", "en": "Please accept or reject this request:"}.get(lang, "Select action:")

        text = (
            f"{hdr_app}\n"
            "────────────────────\n"
            f"👤 <b>{lbl_p}:</b> {p_name}\n"
            f"🕒 <b>{lbl_t}:</b> <code>{escape_html(app.preferred_time)}</code>\n"
            f"📝 <b>{lbl_n}:</b> {escape_html(app.note or '-')}\n"
            f"📌 <b>{lbl_s}:</b> <b>{st_lbl}</b>\n"
            "────────────────────\n"
            f"<i>{lbl_act}</i>"
        )
        buttons = [
    ...
    async with AsyncSessionLocal() as session:
        ...
        if app and app.status == "pending":
            ...
            app_receipt_txt = {
                ...
                "ru": (
                    f"🗓️ <b>Талон подтверждения встречи с учителем</b>\n"
                    f"👤 <b>Учитель:</b> {escape_html(tch.full_name)}\n"
                    f"🕒 <b>Время встречи:</b> <code>{escape_html(app.preferred_time)}</code>\n"
                    f"🆔 <b>Талон №:</b> <code>#RND-{app.id:04d}</code>\n"
                    f"✅ <i>Встреча подтверждена учителем. Пожалуйста, будьте готовы к назначенному времени.</i>"
                ),
                "uz": (
                    f"🗓️ <b>O'qituvchi bilan uchrashuv tasdiqnoma taloni</b>\n"
                    f"👤 <b>O'qituvchi:</b> {escape_html(tch.full_name)}\n"
                    f"🕒 <b>Uchrashuv vaqti:</b> <code>{escape_html(app.preferred_time)}</code>\n"
                    f"🆔 <b>Talon №:</b> <code>#RND-{app.id:04d}</code>\n"
                    f"✅ <i>Uchrashuv o'qituvchi tomonidan tasdiqlandi. Belgilangan vaqtda tayyor bo'ling.</i>"
                ),
                "en": (
                    f"🗓️ <b>Parent - Teacher Meeting Confirmation Voucher</b>\n"
                    f"👤 <b>Teacher:</b> {escape_html(tch.full_name)}\n"
                    f"🕒 <b>Meeting Time:</b> <code>{escape_html(app.preferred_time)}</code>\n"
                    f"🆔 <b>Voucher ID:</b> <code>#RND-{app.id:04d}</code>\n"
                    f"✅ <i>Meeting confirmed by teacher. Please be ready at the scheduled time.</i>"
                )
            }.get(p_lang, f"Appointment confirmed with {tch.full_name}")

            app_title_loc = {
                "tr": f"Randevu Onay Fişi: {tch.full_name}",
                "ru": f"Талон встречи: {tch.full_name}",
                "uz": f"Uchrashuv taloni: {tch.full_name}",
                "en": f"Meeting Voucher: {tch.full_name}"
            }.get(p_lang, f"Meeting Voucher: {tch.full_name}")
...
async def cb_admin_sched_edit_class(query: CallbackQuery, state: FSMContext):
    class_name = ":".join(query.data.split(":")[2:])
    ...
    prompt_t = {
        "tr": f"📅 <b>{escape_html(class_name)} Sınıfı Ders Programı</b>\n\n📌 <b>Mevcut Program:</b>\n{escape_html(current_txt)}\n\n✍️ Lütfen yeni haftalık programı yazıp gönderiniz:",
        "ru": f"📅 <b>Расписание уроков класса {escape_html(class_name)}</b>\n\n📌 <b>Текущее расписание:</b>\n{escape_html(current_txt)}\n\n✍️ Введите новое расписание уроков:",
        "uz": f"📅 <b>{escape_html(class_name)} sinfi dars jadvali</b>\n\n📌 <b>Amaldagi jadval:</b>\n{escape_html(current_txt)}\n\n✍️ Iltimos, yangi haftalik dars jadvalini yuboring:",
        "en": f"📅 <b>Timetable for Class {escape_html(class_name)}</b>\n\n📌 <b>Current Timetable:</b>\n{escape_html(current_txt)}\n\n✍️ Please send the updated weekly timetable:"
    }.get(lang, f"📅 <b>{escape_html(class_name)} Timetable</b>\n\n✍️ Enter new timetable:")
...
async def process_admin_add_class_name(message: Message, state: FSMContext):
    ...
    async with AsyncSessionLocal() as session:
        ...
        view_btn_txt = {
            "tr": f"🏫 {raw_name} Sınıfını Gör",
            "ru": f"🏫 Посмотреть класс {raw_name}",
            "uz": f"🏫 {raw_name} sinfini ko'rish",
            "en": f"🏫 View Class {raw_name}"
        }.get(lang, f"🏫 View Class {raw_name}")
        buttons = [
            [InlineKeyboardButton(text=view_btn_txt, callback_data=f"adm:show_class:{raw_name}")],
...
@router.callback_query(F.data.startswith("adm:del_class_prompt:"))
@router.callback_query(F.data.startswith("adm:del_class:"))
async def cb_admin_del_class_prompt(query: CallbackQuery):
    ...
    async with AsyncSessionLocal() as session:
        ...
        bc = {"tr": f"🏠 Ana Menü ➔ 🏫 Sınıflar ➔ 🗑️ {class_name} Sil", "ru": f"🏠 Главное меню ➔ 🏫 Классы ➔ 🗑️ Удаление {class_name}", "uz": f"🏠 Asosiy menyu ➔ 🏫 Sinflar ➔ 🗑️ {class_name} o'chirish", "en": f"🏠 Main Menu ➔ 🏫 Classes ➔ 🗑️ Delete {class_name}"}.get(lang, "Delete Class")

        if st_cnt == 0:
            msg_body = {
                "tr": f"⚠️ <b>{class_name}</b> sınıfını silmek istediğinize emin misiniz?\n\nBu sınıfta henüz kayıtlı öğrenci bulunmamaktadır.",
                "ru": f"⚠️ Вы уверены, что хотите удалить класс <b>{class_name}</b>?\n\nВ этом классе нет зарегистрированных учеников.",
                "uz": f"⚠️ <b>{class_name}</b> sinfini o'chirishni tasdiqlaysizmi?\n\nUshbu sinfda hali o'quvchilar yo'q.",
                "en": f"⚠️ Are you sure you want to delete class <b>{class_name}</b>?\n\nThere are no students in this class."
            }.get(lang, f"Delete class {class_name}?")
            btn_confirm_text = {"tr": "🗑️ Evet, Sınıfı Sil", "ru": "🗑️ Да, удалить класс", "uz": "🗑️ Ha, sinfni o'chirish", "en": "🗑️ Yes, Delete Class"}.get(lang, "Delete")
        else:
            msg_body = {
                "tr": f"⚠️ <b>DİKKAT: {class_name} Sınıfını ve Öğrencilerini Silmek Üzeresiniz!</b>\n────────────────────\nBu sınıfta kayıtlı <b>{st_cnt}</b> öğrenci bulunmaktadır.\n\nSınıfı sildiğinizde sınıftaki tüm öğrencilerin kayıtları, veli bağlantıları, notları ve devamsızlıkları sistemden tamamen silinecektir.\n\nİşlemi onaylıyor musunuz?",
                "ru": f"⚠️ <b>ВНИМАНИЕ: Вы собираетесь удалить класс {class_name} и всех учеников!</b>\n────────────────────\nВ этом классе зарегистрировано <b>{st_cnt}</b> учеников.\n\nПри удалении класса все ученики, их оценки, посещаемость и привязки родителей будут удалены.\n\nПодтверждаете удаление?",
                "uz": f"⚠️ <b>DIQQAT: Siz {class_name} sinfi va barcha o'quvchilarini o'chirmoqchisiz!</b>\n────────────────────\nUshbu sinfda <b>{st_cnt}</b> ta o'quvchi ro'yxatga olingan.\n\nSinf o'chirilganda barcha ma'lumotlar o'chiriladi.\n\nTasdiqlaysizmi?",
                "en": f"⚠️ <b>WARNING: You are about to delete class {class_name} and all students!</b>\n────────────────────\nThere are <b>{st_cnt}</b> students in this class.\n\nDeleting this class will permanently remove all student records, grades, and parent links.\n\nDo you confirm?"
            }.get(lang, f"Delete class {class_name} and all {st_cnt} students?")
            btn_confirm_text = {"tr": f"⚠️ Evet, Sınıfı ve {st_cnt} Öğrenciyi Sil", "ru": f"⚠️ Да, удалить класс и {st_cnt} уч.", "uz": f"⚠️ Ha, sinf va {st_cnt} o'quvchini o'chirish", "en": f"⚠️ Yes, Delete Class & {st_cnt} Students"}.get(lang, "Delete")

        btn_cancel_text = get_text("btn_cancel_action", lang)

        confirm_kb = InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(text=btn_confirm_text, callback_data=f"adm:del_class_confirm:{class_name}")],
            [InlineKeyboardButton(text=btn_cancel_text, callback_data=f"adm:show_class:{class_name}")]
        ])

        full_prompt = f"<b>{bc}</b>\n────────────────────\n{msg_body}"
        await safe_edit_or_answer(query, full_prompt, reply_markup=confirm_kb, parse_mode="HTML")

@router.callback_query(F.data.startswith("adm:del_class_confirm:"))
async def cb_admin_del_class_confirm(query: CallbackQuery):
    try: await query.answer()
    except Exception: pass
    class_name = ":".join(query.data.split(":")[2:])
    async with AsyncSessionLocal() as session:
        ...
        # 1. Sınıftaki tüm öğrencileri bul ve bağlı kayıtlarıyla birlikte sil
        ...
        # 2. Sınıfa ait ders programı, sınavlar, ödevler ve sınıf kaydını sil
        ...
        # 3. Öğretmenlerin atandığı sınıflardan bu sınıfı ayıkla
        teachers = (await session.execute(select(Teacher))).scalars().all()
        for tch in teachers:
            if tch.assigned_classes and tch.assigned_classes != "ALL":
                cls_list = [c.strip() for c in tch.assigned_classes.split(",") if c.strip() and c.strip() != class_name]
                tch.assigned_classes = ",".join(cls_list) if cls_list else "ALL"

        await log_audit(session, query.from_user.id, (user.full_name or "Yönetici"), "SINIF SİLİNDİ", f"Sınıf ve tüm verileri silindi: {class_name} ({len(students)} öğrenci)")
        await session.commit()

        tst = {
            "tr": f"🗑️ {class_name} sınıfı ve tüm kayıtları başarıyla silindi.",
            "ru": f"🗑️ Класс {class_name} и все его данные успешно удалены.",
            "uz": f"🗑️ {class_name} sinfi va barcha ma'lumotlari muvaffaqiyatli o'chirildi.",
            "en": f"🗑️ Class {class_name} and all associated records deleted."
        }.get(lang, f"Class {class_name} deleted.")
...
async def process_student_name(message: Message, state: FSMContext):
    ...
    name_val = message.text.strip() if message.text else ""
    if is_universal_cancel_text(name_val) or name_val.lower() in ("/cancel", "cancel", "iptal", "otmena", "bekor", "отмена"):
        await state.clear()
        try: await message.delete()
        except Exception: pass
...
async def process_student_class(message: Message, state: FSMContext):
    ...
    cls_val = message.text.strip() if message.text else ""
    if is_universal_cancel_text(cls_val) or cls_val.lower() in ("/cancel", "cancel", "iptal", "otmena", "bekor", "отмена"):
        await state.clear()
        try: await message.delete()
        except Exception: pass
        from aiogram.types import CallbackQuery
...
async def process_student_no(message: Message, state: FSMContext):
    async with AsyncSessionLocal() as session:
        ...
        no_val = message.text.strip() if message.text else ""
        if is_universal_cancel_text(no_val) or no_val.lower() in ("/cancel", "cancel", "iptal", "otmena", "bekor", "отмена"):
            await state.clear()
            try: await message.delete()
            except Exception: pass
            from aiogram.types import CallbackQuery
        ...
        lbl_view_cls = {"tr": f"🏫 {class_name} Sınıfını Gör", "ru": f"🏫 Список класса {class_name}", "uz": f"🏫 {class_name} sinfini ko'rish", "en": f"🏫 View {class_name}"}.get(lang, f"🏫 View {class_name}")
        buttons = [
            [
                InlineKeyboardButton(text=lbl_view_cls, callback_data=f"adm:show_class:{class_name}"),
                InlineKeyboardButton(text=get_text("btn_add_another", lang), callback_data=f"adm:add_st_to_cls:{class_name}")
            ],
            [
                InlineKeyboardButton(text=get_text("btn_classes", lang), callback_data="adm:classes")
            ],
...
@router.callback_query(F.data.in_(["cancel_teacher_add", "adm:cancel_teacher"]))
async def cb_cancel_teacher_add(query: CallbackQuery, state: FSMContext | None = None):
    try: await query.answer()
    except Exception: pass
    if state:
        try: await state.clear()
...
async def process_teacher_name(message: Message, state: FSMContext):
    ...
    if is_universal_cancel_text(name_val) or name_val.lower() in ("/cancel", "cancel", "iptal", "otmena", "bekor", "отмена"):
        ...
        await cb_admin_teachers_list(dummy_q, state)
        return
...
async def process_teacher_subject(message: Message, state: FSMContext):
    async with AsyncSessionLocal() as session:
        ...
        subj_val = message.text.strip() if message.text else ""
        if is_universal_cancel_text(subj_val) or subj_val.lower() in ("/cancel", "cancel", "iptal", "otmena", "bekor", "отмена"):
            await state.clear()
            try: await message.delete()
            except Exception: pass
            from aiogram.types import CallbackQuery
...
async def cb_admin_teachers_list(query: CallbackQuery, state: FSMContext | None = None):
    try: await query.answer()
    except Exception: pass
    if state: await state.clear()
    async with AsyncSessionLocal() as session:
...
async def cb_admin_teacher_card(query: CallbackQuery):
    ...
    async with AsyncSessionLocal() as session:
        ...
        lbl_act = {"tr": f"🟢 Aktif (<code>{tch.telegram_id}</code>)", "ru": f"🟢 Активен (<code>{tch.telegram_id}</code>)", "uz": f"🟢 Faol (<code>{tch.telegram_id}</code>)", "en": f"🟢 Active (<code>{tch.telegram_id}</code>)"}.get(lang, f"🟢 Active (<code>{tch.telegram_id}</code>)")
        lbl_inact = {"tr": "⚪ Giriş Yapmadı", "ru": "⚪ Не входил", "uz": "⚪ Kirmagan", "en": "⚪ Not Logged In"}.get(lang, "⚪ Not Logged In")
        ...
        u_tag = f" (@{u_linked.username})" if u_linked and u_linked.username else ""
        text = (
            f"{tc_title}\n"
            "────────────────────\n"
            f"👤 <b>{escape_html(tch.full_name)}</b>{u_tag}\n"
            f"🆔 <code>{tch.telegram_id or '—'}</code> • <b>{lbl_tc_subj}:</b> {escape_html(tch.subject)}\n"
            "────────────────────\n"
...
async def cb_admin_manage_teacher_classes(query: CallbackQuery):
    ...
    async with AsyncSessionLocal() as session:
        ...
        prompt_txt = (
            ...
            + {"tr": "Lütfen öğretmenin sorumlu olacağı sınıfları seçiniz (Dokunarak açıp kapatabilirsiniz):", "ru": "Выберите классы, за которые отвечает учитель (нажимайте для переключения):", "uz": "O'qituvchi mas'ul bo'ladigan sinflarni tanlang (yoqish/o'chirish uchun bosing):", "en": "Select the classes assigned to this teacher (tap to toggle):"}.get(lang, "Select classes:")
        )
        ...
        lbl_save_back = {"tr": "💾 Kaydet & Geri", "ru": "💾 Сохранить и назад", "uz": "💾 Saqlash va orqaga", "en": "💾 Save & Back"}.get(lang, "💾 Save & Back")
        buttons.append([InlineKeyboardButton(text=lbl_save_back, callback_data=f"adm:tch_card:{tch.id}")])
...
async def cb_admin_co_pick_target(query: CallbackQuery):
    ...
    async with AsyncSessionLocal() as session:
        ...
        prompt_co = {"tr": f"👨‍🏫 <b>{escape_html(class_name)}</b> sınıfına hangi hoca ortak olarak atansın?", "ru": f"👨‍🏫 Кого прикрепить к классу <b>{escape_html(class_name)}</b>?", "uz": f"👨‍🏫 <b>{escape_html(class_name)}</b> sinfiga qaysi ustoz hamkor qilinsin?", "en": f"👨‍🏫 Add co-teacher for <b>{escape_html(class_name)}</b>:"}.get(lang, "Select co-teacher:")
...
async def cb_admin_delete_teacher_confirmed(query: CallbackQuery):
    if await is_readonly_mode_active():
        await query.answer(get_text("readonly_mode_active_alert", lang), show_alert=True)
        return
    tch_id = int(query.data.split(":")[2])
    async with AsyncSessionLocal() as session:
        user = await session.get(User, query.from_user.id)
...
async def cb_classes_list(query: CallbackQuery, state: FSMContext | None = None):
    ...
    async with AsyncSessionLocal() as session:
        ...
        lbl_tot_cls = {"tr": f"(Toplam {len(classes)} Sınıf)", "ru": f"(Всего: {len(classes)} кл.)", "uz": f"(Jami: {len(classes)} ta sinf)", "en": f"(Total: {len(classes)} Classes)"}.get(lang, f"({len(classes)})")
        prompt_c = (
            f"<b>{bc_c}</b>\n"
            "────────────────────\n"
            f"🏫 <b>{get_text('btn_classes', lang)}</b> {lbl_tot_cls}\n\n"
            f"{sub_c}"
        )
...
async def cb_show_class_students(query: CallbackQuery):
    ...
    async with AsyncSessionLocal() as session:
        ...
        val_not_assigned = {"tr": "Henüz Atanmadı", "ru": "Еще не назначен", "uz": "Hali biriktirilmagan", "en": "Not Assigned Yet"}.get(lang, "Not Assigned Yet")
        tch_summary = ", ".join(tch_names) if tch_names else val_not_assigned
        ...
        if total_students == 0:
            ...
            buttons = [
                ...
                [InlineKeyboardButton(text=btn_del_cls, callback_data=f"adm:del_class_prompt:{class_name}")],
                get_nav_buttons(lang, back_callback="adm:classes")
            ]
            class_list_title = (
                f"<b>{bc_sc}</b>\n"
...
async def cb_student_card(query: CallbackQuery):
    ...
    async with AsyncSessionLocal() as session:
        ...
        p_label_def = {"tr": "Veli", "ru": "Родитель", "uz": "Ota-ona", "en": "Parent"}.get(lang, "Parent")
        no_parent_txt = {"tr": "Henüz bağlı veli yok", "ru": "Нет привязанных родителей", "uz": "Biriktirilgan ota-ona yo'q", "en": "No linked parents"}.get(lang, "None")
        if parents:
            parent_lines = [f"• {p.full_name or p_label_def} (<code>{p.telegram_id}</code>)" for p in parents]
            p_info = "\n" + "\n".join(parent_lines)
        else:
            p_info = f" <i>{no_parent_txt}</i>"
        ...
        lbl_st_code = {"tr": "Öğrenci Kodu", "ru": "Код ученика", "uz": "O'quvchi kodi", "en": "Student Code"}.get(lang, "Student Code")
        lbl_pr_code = {"tr": "Veli Kodu", "ru": "Код родителя", "uz": "Ota-ona kodi", "en": "Parent Code"}.get(lang, "Parent Code")
        lbl_pr_w = {"tr": "Bağlı Veli", "ru": "Родители", "uz": "Bog'langan ota-ona", "en": "Linked Parents"}.get(lang, "Parents")

        text = (
            f"{t_card_title}\n"
            "────────────────────\n"
            f"👤 <b>{escape_html(st.full_name)}</b>\n"
            f"🏫 <b>{lbl_cls_w}:</b> {escape_html(st.class_name)} │ <b>{lbl_no_w}:</b> <code>{escape_html(st.student_number)}</code>\n"
            "────────────────────\n"
            f"🔑 <b>{lbl_st_code}:</b> <code>{st.student_code}</code> ({st_status})\n"
            f"🔐 <b>{lbl_pr_code}:</b> <code>{st.parent_code}</code> ({pr_status})\n"
            f"👨‍👩‍👧‍👦 <b>{lbl_pr_w}:</b>{p_info}\n"
            "────────────────────"
        )
...
async def cb_admin_unack_notifs(query: CallbackQuery):
    async with AsyncSessionLocal() as session:
        ...
        for un in unacks:
            ...
            lines.append(f"• 👤 <b>{escape_html(p_name)}</b> (<code>{un.user_telegram_id}</code>) — <i>{dt_str}</i>")
...
# (Duplicate cb_admin_risk_radar consolidated to line 16000+)
...
async def cb_admin_export_audit_excel(query: CallbackQuery):
    ...
    await query.answer({"tr": "✅ Excel aktarıldı!", "ru": "✅ Файл отправлен!", "uz": "✅ Fayl yuborildi!", "en": "✅ File sent!"}.get(lang, "✅ File sent!"), show_alert=False)
...
async def cb_admin_remind_all_att(query: CallbackQuery):
    async with AsyncSessionLocal() as session:
        ...
        for t in teachers:
            ...
            remind_m = {
                "tr": f"⚠️ <b>YOKLAMA HATIRLATMASI</b>\n\nSayın Hocam, <b>{', '.join(missing)}</b> sınıflarının sabah yoklaması henüz sisteme girilmemiştir. Lütfen ders yoklamasını alınız.",
                "ru": f"⚠️ <b>НАПОМИНАНИЕ О ПЕРЕКЛИЧКЕ</b>\n\nУважаемый(ая) учитель, перекличка для классов <b>{', '.join(missing)}</b> еще не внесена в систему.",
                "uz": f"⚠️ <b>DAVOMAT ESLATMASI</b>\n\nHurmatli ustoz, <b>{', '.join(missing)}</b> sinflarining tonggi davomati hali tizimga kiritilmagan. Iltimos, davomatni oling.",
                "en": f"⚠️ <b>ATTENDANCE REMINDER</b>\n\nDear Teacher, morning attendance for classes <b>{', '.join(missing)}</b> is still pending. Please take attendance."
...
async def cb_admin_backup_db_now(query: CallbackQuery):
    ...
    try:
        ...
        await query.answer({"tr": "✅ Veritabanı yedeği sohbete gönderildi.", "ru": "✅ Резервная копия базы данных отправлена в чат.", "uz": "✅ Ma'lumotlar bazasi zaxirasi chatga yuborildi.", "en": "✅ Database backup sent to chat."}.get(lang, "✅ Backup sent."), show_alert=False)
    except Exception as e:
...
@router.callback_query(F.data.startswith("adm:status_live"))
async def cb_admin_status_live(query: CallbackQuery):
    user_id = query.from_user.id
    async with AsyncSessionLocal() as session:
        user = await session.get(User, user_id)
        lang = user.language if user and isinstance(getattr(user, "language", None), str) else "tr"
        if not is_admin_user(user, user_id):
            await query.answer(get_text("unauthorized_action", lang), show_alert=True)
            return

    card_text = await generate_status_live_card(lang=lang)
    lbl_refresh = {"tr": "🔄 Canlı Yenile", "ru": "🔄 Обновить", "uz": "🔄 Yangilash", "en": "🔄 Refresh Live"}.get(lang, "🔄 Refresh")
    buttons = [
        [InlineKeyboardButton(text=lbl_refresh, callback_data="adm:status_live:ref")],
        get_nav_buttons(lang, back_callback="adm:cat_settings")
    ]
    await safe_edit_or_answer(query, card_text, reply_markup=InlineKeyboardMarkup(inline_keyboard=buttons), parse_mode="HTML")
    if ":ref" in query.data:
        toast_txt = {"tr": "🔄 Canlı ölçümler güncellendi!", "ru": "🔄 Метрики обновлены!", "uz": "🔄 Ko'rsatkichlar yangilandi!", "en": "🔄 Metrics refreshed!"}.get(lang, "Refreshed!")
        await query.answer(toast_txt, show_alert=False)
    else:
        await query.answer()

@router.message(any_state, Command("status_live", "status", "saglik", "health_live"))
async def cmd_admin_status_live(message: Message, state: FSMContext | None = None):
    if state: await state.clear()
    user_id = message.from_user.id
    async with AsyncSessionLocal() as session:
        user = await session.get(User, user_id)
    ...
    await message.answer(card_text, reply_markup=InlineKeyboardMarkup(inline_keyboard=buttons), parse_mode="HTML")
...
async def cb_cat_settings(event: Message | CallbackQuery, state: FSMContext | None = None):
    ...
    async with AsyncSessionLocal() as session:
        ...
        cat_desc = {"tr": "📌 Okul sistemi, bakım modu, saat dilimi ve sistem araçlarını bu merkezden yönetebilirsiniz:", "ru": "📌 Настройки школы, режима обслуживания, часового пояса и безопасности:", "uz": "📌 Maktab sozlamalari, texnik rejim, vaqt mintaqasi va xavfsizlik sozlamalari:", "en": "📌 School settings, maintenance mode, timezone, and security controls:"}.get(lang, "Select setting:")
        title = f"<b>{bc}</b>\n────────────────────\n{cat_hdr}\n────────────────────\n{cat_desc}\n────────────────────"
...
async def cb_admin_users_hub(query: CallbackQuery):
    ...
    async with AsyncSessionLocal() as session:
        ...
        hub_title = (
            f"<b>{bc_uh}</b>\n"
            "────────────────────\n"
            + {"tr": f"👥 <b>Kullanıcı Rehberi</b> (Sayfa {page + 1}/{total_pages})\nToplam Kayıtlı: <b>{total_users}</b>\n\nDetay veya işlem için dokunun:\n", "ru": f"👥 <b>Справочник пользователей</b> (Стр. {page + 1}/{total_pages})\nВсего зарегистрировано: <b>{total_users}</b>\n\nНажмите для просмотра деталей:\n", "uz": f"👥 <b>Foydalanuvchilar ma'lumotnomasi</b> ({page + 1}/{total_pages}-sahifa)\nJami ro'yxatdan o'tgan: <b>{total_users}</b>\n\nTafsilot yoki amal uchun tanlang:\n", "en": f"👥 <b>User Directory</b> (Page {page + 1}/{total_pages})\nTotal Registered: <b>{total_users}</b>\n\nTap for details or actions:\n"}.get(lang, f"👥 <b>User Directory</b> ({total_users})\n")
        )
...
async def cb_admin_user_card(query: CallbackQuery):
    ...
    async with AsyncSessionLocal() as session:
        ...
        if tg_id in PERMANENT_ADMIN_IDS or target_u.admin_type == "permanent":
            adm_status = {"tr": "Kurucu / Kalıcı Yönetici", "ru": "Главный администратор", "uz": "Bosh administrator", "en": "Permanent Admin"}.get(lang, "Permanent Admin")
            ...
            adm_status = {"tr": f"Geçici Yönetici ({exp_str})", "ru": f"Временный админ ({exp_str})", "uz": f"Vaqtinchalik admin ({exp_str})", "en": f"Temp Admin ({exp_str})"}.get(lang, f"Temp ({exp_str})")
        else:
        ...
        if target_u.is_blacklisted:
            ...
            acc_status = f"⏱️ " + {"tr": f"Kilitli ({rem_m} dk)", "ru": f"Блокировка ({rem_m} мин)", "uz": f"Bloklangan ({rem_m} daq)", "en": f"Locked ({rem_m} min)"}.get(lang, f"Locked ({rem_m}m)")
        else:
            acc_status = "🟢 " + {"tr": "Aktif", "ru": "Активен", "uz": "Faol", "en": "Active"}.get(lang, "Active")
        ...
        usr_tag = f" (@{target_u.username})" if target_u.username else ""
        text = (
            f"{uc_hdr}\n"
            "────────────────────\n"
            f"👤 <b>{escape_html(display_name)}</b>{usr_tag}\n"
            f"🆔 <code>{target_u.telegram_id}</code> • <b>{lbl_u_rol}:</b> {role_label}\n"
            "────────────────────\n"
            f"📱 <b>{lbl_u_phn}:</b> <code>{escape_html(phone_str)}</code>\n"
            f"🌐 <b>{lbl_u_lng}:</b> {target_u.language.upper()}\n"
            f"👑 <b>{lbl_u_adm}:</b> {adm_status}\n"
            f"🛡️ <b>{lbl_u_acc}:</b> {acc_status}\n"
        )
        if extra_info:
            text += "────────────────────\n" + "\n".join(extra_info) + "\n"
        text += "────────────────────"
...
async def cb_admin_user_360(query: CallbackQuery):
    ...
    tg_id = int(parts[2])
    page = int(parts[3]) if len(parts) > 3 and parts[3].isdigit() else 0
    is_ref = len(parts) > 4 and parts[4] == "ref"
    ...
    async with AsyncSessionLocal() as session:
        ...
        # Eğer yönetici kendi profiline bakıyorsa veya yenileye bastıysa canlı aktifliğini doğrudan mühürle
        if tg_id == query.from_user.id:
            target_u.last_seen_at = now
            cur_act = {
                "tr": f"360° İstihbarat Paneli İncelendi ({now_clock})",
                "ru": f"Просмотр 360° досье ({now_clock})",
                "uz": f"360° dosye ko'rildi ({now_clock})",
                "en": f"Viewed 360° Dossier ({now_clock})"
            }.get(lang, "Viewed 360° Dossier")
            target_u.last_action_desc = cur_act
            await session.commit()

        raw_name = target_u.full_name or ""
        clean_name = re.sub(r'[\.\s\-_]+', '', str(raw_name or ''))
        unkn_txt = {"tr": "Bilinmeyen", "ru": "Неизвестно", "uz": "Noma'lum", "en": "Unknown"}.get(lang, "Unknown")
        display_name = raw_name if clean_name else (get_text("permanent_admin_title", lang) if (tg_id in PERMANENT_ADMIN_IDS or target_u.admin_type == "permanent") else unkn_txt)
        usr_display_tag = f"(@{target_u.username})" if target_u.username else ""

        role_labels_all = {
            "admin": {"tr": "Yönetici", "ru": "Администратор", "uz": "Administrator", "en": "Administrator"},
            "teacher": {"tr": "Öğretmen", "ru": "Учитель", "uz": "O'qituvchi", "en": "Teacher"},
            "parent": {"tr": "Veli", "ru": "Родитель", "uz": "Ota-ona", "en": "Parent"},
            "student": {"tr": "Öğrenci", "ru": "Ученик", "uz": "O'quvchi", "en": "Student"},
            "guest": {"tr": "Misafir", "ru": "Гость", "uz": "Mehmon", "en": "Guest"}
        }
        role_label = role_labels_all.get(target_u.role, {}).get(lang, target_u.role)
        lbl_role_prefix = {"tr": "Rol", "ru": "Роль", "uz": "Roli", "en": "Role"}.get(lang, "Role")

        # 1. Canlı Durum & Son Görülme (Sıfır Hata & Anlık Gösterge)
        live_badge = "🟢"
        if target_u.is_bot_blocked:
            live_badge = "🔴"
            live_status = {"tr": "Botu Engelledi / Çıktı", "ru": "Заблокировал бота / Вышел", "uz": "Botni blokladi / Chiqib ketdi", "en": "Blocked Bot / Left"}.get(lang, "Blocked Bot")
        elif target_u.last_seen_at:
            diff_secs = max(0, (now - target_u.last_seen_at).total_seconds())
            if diff_secs < 90:
                live_status = {"tr": "Çevrimiçi (Az önce)", "ru": "В сети (Только что)", "uz": "Onlayn (Hozirgina)", "en": "Online (Just now)"}.get(lang, "Online (Just now)")
            elif diff_secs < 3600:
                m = max(1, int(diff_secs // 60))
                live_status = {"tr": f"Çevrimiçi ({m} dk önce)", "ru": f"В сети ({m} мин. назад)", "uz": f"Onlayn ({m} daq. oldin)", "en": f"Online ({m} min ago)"}.get(lang, f"Online ({m} min ago)")
            elif diff_secs < 86400:
                h = max(1, int(diff_secs // 3600))
                live_badge = "🟡"
                live_status = {"tr": f"Bugün Aktif ({h} sa önce)", "ru": f"Активен сегодня ({h} ч. назад)", "uz": f"Bugun faol ({h} soat oldin)", "en": f"Active today ({h} hr ago)"}.get(lang, f"Active today ({h} hr ago)")
            else:
                d = max(1, int(diff_secs // 86400))
                live_badge = "🔴"
                live_status = {"tr": f"Pasif ({d} gündür girmedi)", "ru": f"Неактивен ({d} дн. без входа)", "uz": f"Nofaol ({d} kundan beri kirmadi)", "en": f"Inactive ({d} days absent)"}.get(lang, f"Inactive ({d} days)")
        else:
            live_badge = "⚪"
            live_status = {"tr": "Henüz etkileşim yok", "ru": "Нет активности", "uz": "Hali faollik yo'q", "en": "No activity yet"}.get(lang, "No activity yet")

        # 2. İtaat & Tepki Hızı (Latency)
        ...
        if lats:
            ...
            if avg_m <= 5:
                itaat_txt = {"tr": f"~{avg_m} dk (A Sınıfı - Anında)", "ru": f"~{avg_m} мин (Класс A - Мгновенно)", "uz": f"~{avg_m} daq (A Sinf - Tezkor)", "en": f"~{avg_m} min (Class A - Instant)"}.get(lang, f"~{avg_m} min")
            elif avg_m <= 30:
                itaat_txt = {"tr": f"~{avg_m} dk (B Sınıfı - Standart)", "ru": f"~{avg_m} мин (Класс B - Стандарт)", "uz": f"~{avg_m} daq (B Sinf - Standart)", "en": f"~{avg_m} min (Class B - Standard)"}.get(lang, f"~{avg_m} min")
            else:
                itaat_txt = {"tr": f"~{avg_m} dk (C Sınıfı - Gecikmeli)", "ru": f"~{avg_m} мин (Класс C - С задержкой)", "uz": f"~{avg_m} daq (C Sinf - Kechikkan)", "en": f"~{avg_m} min (Class C - Delayed)"}.get(lang, f"~{avg_m} min")
        else:
            itaat_txt = {"tr": "Onay kaydı yok", "ru": "Нет записей подтверждений", "uz": "Tasdiq yozuvi yo'q", "en": "No records"}.get(lang, "No records")

        # 3. Güvenlik & Tehdit Skoru (0 - 100)
        score = 100
        if target_u.is_blacklisted:
            score = 0
        else:
        ...
        if score >= 85:
            score_badge = "🛡️"
            score_txt = {"tr": f"%{score} (A Sınıfı)", "ru": f"{score}% (Класс A)", "uz": f"{score}% (A Sinf)", "en": f"{score}% (Class A)"}.get(lang, f"{score}%")
        elif score >= 60:
            score_badge = "🟡"
            score_txt = {"tr": f"%{score} (B Sınıfı)", "ru": f"{score}% (Класс B)", "uz": f"{score}% (B Sinf)", "en": f"{score}% (Class B)"}.get(lang, f"{score}%")
        elif score >= 40:
            score_badge = "🟠"
            score_txt = {"tr": f"%{score} (C Sınıfı)", "ru": f"{score}% (Класс C)", "uz": f"{score}% (C Sinf)", "en": f"{score}% (Class C)"}.get(lang, f"{score}%")
        else:
            score_badge = "🔴"
            score_txt = {"tr": f"%{score} (D Sınıfı - Risk)", "ru": f"{score}% (Класс D - Риск)", "uz": f"{score}% (D Sinf - Xavf)", "en": f"{score}% (Class D - Risk)"}.get(lang, f"{score}%")

        # 4. Biyolojik Saat / Zaman Anomalisi
        night_cnt = target_u.night_activity_count or 0
        if night_cnt > 0:
            rutin_txt = {"tr": f"⚠️ Gece {night_cnt} işlem (01:00-06:00)", "ru": f"⚠️ Ночь: {night_cnt} действ. (01:00-06:00)", "uz": f"⚠️ Tun: {night_cnt} ta amal (01:00-06:00)", "en": f"⚠️ Night: {night_cnt} actions (01:00-06:00)"}.get(lang, f"⚠️ Night: {night_cnt}")
        else:
            rutin_txt = {"tr": "Düzgün Gündüz Rutini", "ru": "Дневной режим (Норма)", "uz": "Kunduzgi tartib (Me'yor)", "en": "Normal Daytime Routine"}.get(lang, "Normal Daytime")

        # 5. Kimlik Değişim Geçmişi
        names_log = []
        ...
        if names_log:
            last_n = names_log[-1]
            c_cnt = len(names_log)
            old_v = last_n.get('old', '')
            new_v = last_n.get('new', '')
            alias_txt = {"tr": f"{c_cnt} Değişiklik ({old_v} ➔ {new_v})", "ru": f"{c_cnt} изм. ({old_v} ➔ {new_v})", "uz": f"{c_cnt} ta o'zgarish ({old_v} ➔ {new_v})", "en": f"{c_cnt} change(s) ({old_v} ➔ {new_v})"}.get(lang, f"{c_cnt} changes")
        else:
            alias_txt = {"tr": "Sabit (Değişiklik yok)", "ru": "Стабильно (Без изменений)", "uz": "O'zgarmas (O'zgarishsiz)", "en": "Constant (No changes)"}.get(lang, "Constant")

        # 6. Tipoloji / İlgi Alanı
        p_map = {}
        ...
        if top_act in ("grade", "exam"):
            persona_txt = {"tr": "🎯 Başarı & Not Odaklı", "ru": "🎯 Оценки и успеваемость", "uz": "🎯 Baholar va natijalar", "en": "🎯 Academic & Grades"}.get(lang, "Academic")
        elif top_act in ("attendance", "excuse"):
            persona_txt = {"tr": "⚠️ Devamsızlık & İzin", "ru": "⚠️ Пропуски и справки", "uz": "⚠️ Davomat va ruxsatlar", "en": "⚠️ Attendance & Excuses"}.get(lang, "Attendance")
        elif top_act == "menu":
            persona_txt = {"tr": "🍲 Yemek & Menü", "ru": "🍲 Меню столовой", "uz": "🍲 Oshxona menyusi", "en": "🍲 Cafeteria Menu"}.get(lang, "Cafeteria")
        elif top_act == "download":
            persona_txt = {"tr": "📑 Evrak & Dosyalar", "ru": "📑 Документы и выгрузки", "uz": "📑 Hujjatlar va fayllar", "en": "📑 Documents & Files"}.get(lang, "Documents")
        else:
            persona_txt = {"tr": "💤 Standart / Seyrek", "ru": "💤 Редкий контакт / Норма", "uz": "💤 Kam muloqot / Me'yor", "en": "💤 Minimal Contact / Normal"}.get(lang, "Normal")

        # 7. Veri Hacmi
        dl_cnt = target_u.download_count or 0
        if dl_cnt >= 10:
            dl_txt = {"tr": f"🚨 Yüksek Hacim ({dl_cnt} dosya)", "ru": f"🚨 Высокий объем ({dl_cnt} файлов)", "uz": f"🚨 Yuqori hajm ({dl_cnt} ta fayl)", "en": f"🚨 High Volume ({dl_cnt} files)"}.get(lang, f"🚨 {dl_cnt} files")
        else:
            dl_txt = {"tr": f"Normal ({dl_cnt} indirme)", "ru": f"Нормально ({dl_cnt} скачиваний)", "uz": f"Oddiy ({dl_cnt} ta yuklash)", "en": f"Normal ({dl_cnt} downloads)"}.get(lang, f"Normal ({dl_cnt})")

        excuse_360_txt = ""
        st_360 = (await session.execute(select(Student).where(Student.student_telegram_id == tg_id))).scalar_one_or_none()
        if st_360:
            st_ex = (await session.execute(select(Attendance).where(Attendance.student_id == st_360.id, Attendance.status == "excused"))).scalars().all()
            if len(st_ex) >= 3:
                mf_cnt = sum(1 for a in st_ex if getattr(a, 'date', None) and hasattr(a.date, 'weekday') and a.date.weekday() in (0, 4))
                if mf_cnt / len(st_ex) >= 0.6:
                    pct = int(round(mf_cnt * 100 / len(st_ex)))
                    excuse_lbl = {
                        "tr": f"\n⚠️ <b>İzin Eğilimi:</b> %{pct} Cuma/Pzt (Hafta sonu uzatma)",
                        "ru": f"\n⚠️ <b>Паттерн пропусков:</b> {pct}% Пт/Пн (Продление выходных)",
                        "uz": f"\n⚠️ <b>Ruxsat moyilligi:</b> {pct}% Juma/Dush (Dam olishni cho'zish)",
                        "en": f"\n⚠️ <b>Absence Pattern:</b> {pct}% Fri/Mon (Weekend extension)"
                    }.get(lang, f"\n⚠️ <b>Pattern:</b> {pct}% Fri/Mon")
                    excuse_360_txt = excuse_lbl

        # 8. Son Hareket
        if target_u.last_action_desc:
            last_act = target_u.last_action_desc
        else:
            last_act = {"tr": "Kayıtlı son işlem yok", "ru": "Нет записей действий", "uz": "Oxirgi amal yozuvi yo'q", "en": "No recorded actions"}.get(lang, "No actions")

        titles = {
            "tr": ("🔍 <b>360° İSTİHBARAT & GÜVENLİK DOSYASI</b>", "Güven Puanı", "Canlı Durum", "İtaat Hızı", "Biyolojik Saat", "Kimlik Kaydı", "Odak Noktası", "Veri Güvenliği", "Son Hareket"),
            "ru": ("🔍 <b>360° ДОСЬЕ БЕЗОПАСНОСТИ И РАЗВЕДКИ</b>", "Индекс доверия", "Онлайн-статус", "Скорость отклика", "Биоритм/Часы", "История имени", "Фокус внимания", "Безопасность данных", "Последнее действие"),
            "uz": ("🔍 <b>360° XAVFSIZLIK VA RAZVEDKA DOSYESI</b>", "Ishonch bali", "Jonli holat", "Itoat tezligi", "Bioritm/Soat", "Ism o'zgarishi", "Asosiy e'tibor", "Ma'lumot xavfsizligi", "Oxirgi amal"),
            "en": ("🔍 <b>360° SECURITY & INTELLIGENCE DOSSIER</b>", "Trust Score", "Live Status", "Response Speed", "Circadian Clock", "Identity Log", "Primary Focus", "Data Security", "Last Action")
        }.get(lang, ("🔍 <b>360° SECURITY DOSSIER</b>", "Trust Score", "Live Status", "Response Speed", "Circadian Clock", "Identity Log", "Primary Focus", "Data Security", "Last Action"))

        t_hdr, t_scr, t_liv, t_lat, t_bio, t_ali, t_tip, t_dat, t_act = titles

        # Ferah, simetrik ve mobil cihazlarda gözü yormayan şık yerleşim
        card_text = (
            f"<b>{t_hdr}</b>\n"
            "────────────────────\n"
            f"👤 <b>{escape_html(display_name)}</b> {usr_display_tag}\n"
            f"🆔 <code>{tg_id}</code> • <b>{lbl_role_prefix}:</b> {role_label}\n"
            "────────────────────\n"
            f"{score_badge} <b>{t_scr}:</b> {score_txt}\n"
            f"{live_badge} <b>{t_liv}:</b> {live_status}\n"
            f"⏱️ <b>{t_lat}:</b> {itaat_txt}\n\n"
            f"🕒 <b>{t_bio}:</b> {rutin_txt}\n"
            f"🎭 <b>{t_ali}:</b> {escape_html(alias_txt)}\n"
            f"🧠 <b>{t_tip}:</b> {persona_txt}\n"
            f"📦 <b>{t_dat}:</b> {dl_txt}{excuse_360_txt}\n"
            "────────────────────\n"
            f"👣 <b>{t_act}:</b>\n"
            f"<i>{escape_html(last_act)}</i>\n"
            "────────────────────"
        )

        btn_refresh = {"tr": "🔄 Anlık Yenile", "ru": "🔄 Обновить досье", "uz": "🔄 Yangilash", "en": "🔄 Refresh Dossier"}.get(lang, "🔄 Refresh")
        btn_kill = {"tr": "⛔ Karantina", "ru": "⛔ В изоляцию", "uz": "⛔ Karantin", "en": "⛔ Quarantine"}.get(lang, "⛔ Quarantine")
        btn_dm = {"tr": "💬 Özel Mesaj", "ru": "💬 Написать", "uz": "💬 Xabar yozish", "en": "💬 Send DM"}.get(lang, "💬 DM")
        btn_prof = {"tr": "👤 Profil", "ru": "👤 Профиль", "uz": "👤 Profil", "en": "👤 Profile"}.get(lang, "👤 Profile")
        btn_back = {"tr": "⬅️ Geri", "ru": "⬅️ Назад", "uz": "⬅️ Orqaga", "en": "⬅️ Back"}.get(lang, "⬅️ Back")

        btn_forensic = {"tr": "📑 Adli Rapor (PDF)", "ru": "📑 Досье (PDF)", "uz": "📑 Sud/Ekspertiza (PDF)", "en": "📑 Forensic PDF"}.get(lang, "📑 Forensic PDF")
        buttons = [
            [InlineKeyboardButton(text=btn_refresh, callback_data=f"adm:user_360:{tg_id}:{page}:ref"), InlineKeyboardButton(text=btn_forensic, callback_data=f"adm:forensic_pdf:{tg_id}")],
        ...
        await safe_edit_or_answer(query, card_text, reply_markup=InlineKeyboardMarkup(inline_keyboard=buttons), parse_mode="HTML")
        if is_ref:
            toast_msg = {"tr": "🔄 Veriler canlı güncellendi!", "ru": "🔄 Досье успешно обновлено!", "uz": "🔄 Ma'lumotlar yangilandi!", "en": "🔄 Dossier refreshed!"}.get(lang, "Refreshed!")
            await query.answer(toast_msg, show_alert=False)
        else:
            await query.answer()
...
@router.callback_query(F.data.startswith("adm:forensic_pdf:"))
async def cb_admin_forensic_pdf(query: CallbackQuery):
    ...
    async with AsyncSessionLocal() as session:
        ...
        pdf_buf = await generate_user_forensic_dossier_pdf(target_id, lang=lang)
        clean_name = re.sub(r'[^\w\s-]', '', target_u.full_name or "User").strip().replace(' ', '_')
        if not clean_name: clean_name = f"User_{target_id}"
        file = BufferedInputFile(pdf_buf.read(), filename=f"Adli_Istihbarat_{clean_name}_{target_id}.pdf")
        caption_txt = f"📑 <b>Adli İstihbarat & Güvenlik Dosyası</b>\n────────────────────\n👤 <b>{escape_html(target_u.full_name or 'Bilinmeyen')}</b> (ID: <code>{target_id}</code>)\n📅 <b>Tarih:</b> {datetime.now().strftime('%d.%m.%Y %H:%M')}\n────────────────────"
        await query.message.answer_document(file, caption=caption_txt, parse_mode="HTML")
        pdf_buf.close()
    await query.answer({"tr": "📑 Adli Rapor Hazırlandı!", "ru": "📑 Досье сформировано!", "uz": "📑 Ekspertiza hujjati tayyorlandi!", "en": "📑 Forensic dossier generated!"}.get(lang, "📑 Dossier generated!"), show_alert=False)
...
async def process_broadcast_text(message: Message, state: FSMContext):
    ...
    async with AsyncSessionLocal() as session:
        ...
        u_lang_map = {
            u.telegram_id: (u.language or "tr")
            for u in (await session.execute(select(User).where(User.telegram_id.in_(target_ids)))).scalars().all()
        }

        for u_id in set(target_ids):
            try:
                u_l = u_lang_map.get(u_id, "tr")
                b_read = {
                    "tr": "👀 Okudum / Bilgim Var",
                    "ru": "👀 Прочитано",
                    "uz": "👀 O'qidim",
                    "en": "👀 Read & Acknowledged"
                }.get(u_l, "👀 Read")

                ann_hdr = {
                    "tr": "📢 <b>OKUL DUYURUSU</b>",
                    "ru": "📢 <b>ОБЪЯВЛЕНИЕ ШКОЛЫ</b>",
                    "uz": "📢 <b>MAKTAB E'LONI</b>",
                    "en": "📢 <b>SCHOOL ANNOUNCEMENT</b>"
                }.get(u_l, "📢 <b>SCHOOL ANNOUNCEMENT</b>")

                read_btn = InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text=b_read, callback_data=f"ack_bc:{notice.id}")]])
                if photo_id:
                    await message.bot.send_photo(chat_id=u_id, photo=photo_id, caption=f"{ann_hdr}\n\n{safe_text}", reply_markup=read_btn, parse_mode="HTML")
                else:
                    await message.bot.send_message(chat_id=u_id, text=f"{ann_hdr}\n\n{safe_text}", reply_markup=read_btn, parse_mode="HTML")
                sent_cnt += 1
                await asyncio.sleep(0.04)
            except Exception:
                pass
...
# --- SINIF VE KADEME TERFİ MASASI (TOPLU & SINIFA GÖRE TERFİ + MEZUNLAR ARŞİVİ) ---
...
async def cb_admin_class_promotion_init(query: CallbackQuery, state: FSMContext | None = None):
    ...
    async with AsyncSessionLocal() as session:
        ...
        total_st = (await session.execute(select(func.count(Student.id)).where(~Student.class_name.startswith("MEZUN")))).scalar() or 0
        active_classes = (await session.execute(select(Student.class_name).where(~Student.class_name.startswith("MEZUN")).distinct())).scalars().all()
        total_cls = len(active_classes)

        bc_pr = {"tr": "🏠 Ana Menü ➔ 👥 Kadro ➔ 🎓 Sınıf Terfi", "ru": "🏠 Главное меню ➔ 👥 Кадры ➔ 🎓 Перевод классов", "uz": "🏠 Asosiy menyu ➔ 👥 Kadro ➔ 🎓 Sinf ko'chirish", "en": "🏠 Main Menu ➔ 👥 Staff ➔ 🎓 Class Promotion"}.get(lang, "🎓 Class Promotion")
        hdr_pr = {"tr": "🎓 <b>SINIF VE KADEME TERFİ YÖNETİMİ</b>", "ru": "🎓 <b>УПРАВЛЕНИЕ ПЕРЕВОДОМ КЛАССОВ</b>", "uz": "🎓 <b>SINF VA BOSQICH KO'CHIRISH BOSHQARUVI</b>", "en": "🎓 <b>CLASS PROMOTION & PROGRESSION DESK</b>"}.get(lang, "🎓 <b>CLASS PROMOTION</b>")
        desc_pr = {
            "tr": f"• Aktif Sınıflar: <b>{total_cls}</b> │ Öğrenciler: <b>{total_st}</b>\n\nLütfen işlem yöntemini seçiniz:",
            "ru": f"• Активных классов: <b>{total_cls}</b> │ Учеников: <b>{total_st}</b>\n\nВыберите способ перевода:",
            "uz": f"• Faol sinflar: <b>{total_cls}</b> │ O'quvchilar: <b>{total_st}</b>\n\nKo'chirish usulini tanlang:",
            "en": f"• Active Classes: <b>{total_cls}</b> │ Students: <b>{total_st}</b>\n\nSelect promotion method:"
        }.get(lang, "Select method:")

        b_bulk = {"tr": "🌐 Toplu Terfi (Tüm Okul)", "ru": "🌐 Массовый перевод (Вся школа)", "uz": "🌐 Ommaviy ko'chirish (Butun maktab)", "en": "🌐 Bulk Promotion (All School)"}.get(lang, "🌐 Bulk Promotion")
        b_single = {"tr": "🏫 Sınıfa Göre Terfi (Sınıf Seç)", "ru": "🏫 Поклассовый перевод (Выбрать класс)", "uz": "🏫 Sinf bo'yicha ko'chirish (Tanlash)", "en": "🏫 Class-by-Class Promotion"}.get(lang, "🏫 Class-by-Class Promotion")
        b_grads = {"tr": "🎓 Mezunlar Arşivini Gör", "ru": "🎓 Архив выпускников", "uz": "🎓 Bitiruvchilar arxivi", "en": "🎓 View Alumni Archive"}.get(lang, "🎓 Alumni Archive")

        buttons = [
            [InlineKeyboardButton(text=b_bulk, callback_data="adm:promo_bulk_prompt")],
            [InlineKeyboardButton(text=b_single, callback_data="adm:promo_cls_list:0")],
            [InlineKeyboardButton(text=b_grads, callback_data="adm:graduates:0")],
            get_nav_buttons(lang, back_callback="adm:cat_staff")
        ]
        card = f"<b>{bc_pr}</b>\n────────────────────\n{hdr_pr}\n────────────────────\n{desc_pr}\n────────────────────"
        await safe_edit_or_answer(query, card, reply_markup=InlineKeyboardMarkup(inline_keyboard=buttons), parse_mode="HTML")
    await query.answer()

@router.callback_query(F.data == "adm:promo_bulk_prompt")
async def cb_admin_promo_bulk_prompt(query: CallbackQuery):
    async with AsyncSessionLocal() as session:
        user = await session.get(User, query.from_user.id)
        lang = user.language if user else "tr"
        if not is_admin_user(user, query.from_user.id):
        ...
        prompt_txt = {
            "tr": (
                "🌐 <b>TÜM OKUL TOPLU SINIF TERFİSİ</b>\n"
                "────────────────────\n"
                f"• Terfi Edilecek Öğrenci Sayısı: <b>{total_st}</b>\n\n"
                "⚠️ <b>Önemli Bilgilendirme:</b>\n"
                "1. Okuldaki tüm sınıflar 1 kademe yükseltilecektir (Örn: <code>9-A ➔ 10-A</code>, <code>11-B ➔ 12-B</code>).\n"
                "2. 12. sınıflar ve son kademedeki öğrenciler otomatik olarak mezun edilerek <b>🎓 Mezunlar Arşivi</b>ne kaydedilecektir.\n"
                "3. İşlem öncesinde tam sistem Excel yedeği sohbete otomatik olarak gönderilecektir.\n\n"
                "İşlemi onaylıyor musunuz?"
            ),
            "ru": (
                "🌐 <b>МАССОВЫЙ ПЕРЕВОД ВСЕЙ ШКОЛЫ</b>\n"
                "────────────────────\n"
                f"• Учеников к переводу: <b>{total_st}</b>\n\n"
                "⚠️ <b>Важное примечание:</b>\n"
                "1. Все классы будут переведены на ступень выше (напр: <code>9-A ➔ 10-A</code>, <code>11-B ➔ 12-B</code>).\n"
                "2. Ученики 12-х классов будут автоматически выпущены и сохранены в <b>🎓 Архиве выпускников</b>.\n"
                "3. Перед выполнением полная копия базы данных (Excel) будет автоматически отправлена вам в чат.\n\n"
                "Подтверждаете выполнение перевода?"
            ),
            "uz": (
                "🌐 <b>BUTUN MAKTAB BO'YICHA OMMAVIY KO'CHIRISH</b>\n"
                "────────────────────\n"
                f"• Ko'chiriladigan o'quvchilar: <b>{total_st}</b>\n\n"
                "⚠️ <b>Muhim eslatma:</b>\n"
                "1. Barcha sinflar 1 pog'ona yuqoriga ko'chiriladi (Masalan: <code>9-A ➔ 10-A</code>, <code>11-B ➔ 12-B</code>).\n"
                "2. 12-sinflar avtomatik ravishda bitiruvchi sifatida <b>🎓 Bitiruvchilar arxivi</b>ga saqlanadi.\n"
                "3. Amaldan oldin to'liq Excel zaxira nusxasi chatga yuboriladi.\n\n"
                "Jarayonni tasdiqlaysizmi?"
            ),
            "en": (
                "🌐 <b>ALL-SCHOOL BULK CLASS PROMOTION</b>\n"
                "────────────────────\n"
                f"• Students to be Promoted: <b>{total_st}</b>\n\n"
                "⚠️ <b>Important Notice:</b>\n"
                "1. All classes will advance to the next grade (e.g. <code>9-A ➔ 10-A</code>, <code>11-B ➔ 12-B</code>).\n"
                "2. Senior 12th-grade students will graduate and be recorded into the <b>🎓 Alumni Archive</b>.\n"
                "3. A full Excel backup will be automatically generated and sent to this chat first.\n\n"
                "Do you confirm this promotion?"
            )
        }.get(lang, "Confirm bulk promotion?")

        b_confirm = {"tr": "✅ Evet, Tüm Okulu Terfi Et", "ru": "✅ Да, перевести всю школу", "uz": "✅ Ha, butun maktabni ko'chirish", "en": "✅ Yes, Promote All School"}.get(lang, "✅ Promote Classes")
        buttons = [
            [InlineKeyboardButton(text=b_confirm, callback_data="adm:class_promotion_confirm")],
            [InlineKeyboardButton(text=get_text("btn_back", lang), callback_data="adm:class_promotion_init")]
        ]
        await safe_edit_or_answer(query, prompt_txt, reply_markup=InlineKeyboardMarkup(inline_keyboard=buttons), parse_mode="HTML")
    await query.answer()

@router.callback_query(F.data.in_(["adm:class_promotion_confirm", "adm:class_promotion_pin_prompt"]))
async def cb_admin_class_promotion_confirm(query: CallbackQuery):
    ...
    async with AsyncSessionLocal() as session:
        ...
        for s in students:
            if s.class_name.startswith("MEZUN"):
                continue
            ...
            if m:
                ...
                if grade_num >= 12:
                    grad = Graduate(
                        ...
                        graduated_class=s.class_name,
                        graduation_year=current_year,
                        parent_phone=None
                    )
                    session.add(grad)
                    s.class_name = f"MEZUN-{s.class_name}"
                    graduated_cnt += 1
                else:
        ...
        succ_card = {
            "tr": (
                "✅ <b>TOPLU SINIF TERFİSİ BAŞARIYLA TAMAMLANDI!</b>\n"
                "────────────────────\n"
                f"• Yükseltilen Öğrenci: <b>{promoted_cnt - graduated_cnt}</b>\n"
                f"• Mezun Olan Öğrenci: <b>{graduated_cnt}</b> (Arşive eklendi)\n"
                f"• Toplam İşlem Gören: <b>{promoted_cnt}</b>\n"
                "────────────────────\n"
                "<i>Öğrenci notları ve devamsızlık kayıtları korunarak sınıflar bir üst kademeye aktarılmıştır.</i>"
            ),
            "ru": (
                "✅ <b>МАССОВЫЙ ПЕРЕВОД КЛАССОВ УСПЕШНО ЗАВЕРШЕН!</b>\n"
                "────────────────────\n"
                f"• Переведено в следующий класс: <b>{promoted_cnt - graduated_cnt}</b>\n"
                f"• Выпущено и заархивировано: <b>{graduated_cnt}</b>\n"
                f"• Всего обработано учеников: <b>{promoted_cnt}</b>\n"
                "────────────────────\n"
                "<i>Классы переведены на ступень выше с сохранением всех архивных оценок и данных.</i>"
            ),
            "uz": (
                "✅ <b>OMMAVIY SINF KO'CHIRISH MUVAFFAQIYATLI YAKUNLANDI!</b>\n"
                "────────────────────\n"
                f"• Yuqori sinfga o'tkazildi: <b>{promoted_cnt - graduated_cnt}</b>\n"
                f"• Bitiruvchilar (Arxivga olindi): <b>{graduated_cnt}</b>\n"
                f"• Jami ko'chirilganlar: <b>{promoted_cnt}</b>\n"
                "────────────────────\n"
                "<i>Barcha baholar va ma'lumotlar saqlangan holda sinflar ko'chirildi.</i>"
            ),
            "en": (
                "✅ <b>BULK CLASS PROMOTION COMPLETED SUCCESSFULLY!</b>\n"
                "────────────────────\n"
                f"• Promoted to Next Grade: <b>{promoted_cnt - graduated_cnt}</b>\n"
                f"• Graduated & Archived: <b>{graduated_cnt}</b>\n"
                f"• Total Processed Students: <b>{promoted_cnt}</b>\n"
                "────────────────────\n"
                "<i>Classes successfully advanced with all historical records preserved.</i>"
            )
        }.get(lang, "Promotion completed successfully.")

        buttons = [
            [InlineKeyboardButton(text={"tr": "🎓 Mezunlar Arşivi", "ru": "🎓 Архив выпускников", "uz": "🎓 Bitiruvchilar arxivi", "en": "🎓 Alumni Archive"}.get(lang, "🎓 Alumni"), callback_data="adm:graduates:0")],
            [InlineKeyboardButton(text=get_text("btn_classes", lang), callback_data="adm:classes")],
            get_nav_buttons(lang, back_callback="adm:class_promotion_init")
        ]
        await safe_edit_or_answer(query, succ_card, reply_markup=InlineKeyboardMarkup(inline_keyboard=buttons), parse_mode="HTML")
    await query.answer()

# --- SINIFA GÖRE TEKİL TERFİ (SEÇİMLİ SINIFA ÖZEL TERFİ) ---
@router.callback_query(F.data.startswith("adm:promo_cls_list:"))
async def cb_admin_promo_class_list(query: CallbackQuery):
    ...
    async with AsyncSessionLocal() as session:
        ...
        all_classes_raw = (await session.execute(select(Student.class_name).where(~Student.class_name.startswith("MEZUN")).distinct())).scalars().all()
        # Sort classes naturally
        def cls_sort_key(c):
            m = re.match(r"^(\d+)(.*)$", str(c).strip())
            return (int(m.group(1)), m.group(2)) if m else (999, str(c))
        
        all_classes = sorted(all_classes_raw, key=cls_sort_key)
        ...
        buttons = []
        if total == 0:
            content = "<i>" + {"tr": "Sistemde terfi edilebilecek aktif sınıf bulunmamaktadır.", "ru": "В системе нет активных классов для перевода.", "uz": "Tizimda ko'chirish uchun faol sinflar topilmadi.", "en": "No active classes found for promotion."}.get(lang, "No classes found.") + "</i>"
        else:
            paged = all_classes[page * PAGE_SIZE : (page + 1) * PAGE_SIZE]
            row = []
            for c in paged:
                cnt = (await session.execute(select(func.count(Student.id)).where(Student.class_name == c))).scalar() or 0
                row.append(InlineKeyboardButton(text=f"🏫 {c} ({cnt})", callback_data=f"adm:promo_card:{c}"))
                if len(row) == 2:
                    buttons.append(row)
                    row = []
            if row: buttons.append(row)
            ...
            if total_pages > 1:
                nav_row = []
                if page > 0:
                    nav_row.append(InlineKeyboardButton(text=get_text("btn_prev", lang), callback_data=f"adm:promo_cls_list:{page-1}"))
                nav_row.append(InlineKeyboardButton(text=f"📄 {page+1}/{total_pages}", callback_data="noop"))
                if page < total_pages - 1:
                    nav_row.append(InlineKeyboardButton(text=get_text("btn_next", lang), callback_data=f"adm:promo_cls_list:{page+1}"))
                buttons.append(nav_row)

        buttons.append([InlineKeyboardButton(text=get_text("btn_back", lang), callback_data="adm:class_promotion_init")])

        hdr_list = {
            "tr": f"🏫 <b>SINIFA GÖRE TERFİ</b> (Toplam {total} Sınıf)\n────────────────────\nTerfi ettirmek veya mezun etmek istediğiniz sınıfı seçiniz:",
            "ru": f"🏫 <b>ПОКЛАССОВЫЙ ПЕРЕВОД</b> (Всего: {total} кл.)\n────────────────────\nВыберите класс, который необходимо перевести или выпустить:",
            "uz": f"🏫 <b>SINF BO'YICHA KO'CHIRISH</b> (Jami: {total} ta sinf)\n────────────────────\nKo'chirmoqchi yoki bitiruvchi qilmoqchi bo'lgan sinfni tanlang:",
            "en": f"🏫 <b>CLASS-BY-CLASS PROMOTION</b> (Total: {total} Classes)\n────────────────────\nSelect the class you want to promote or graduate:"
        }.get(lang, "Select class to promote:")

        await safe_edit_or_answer(query, hdr_list, reply_markup=InlineKeyboardMarkup(inline_keyboard=buttons), parse_mode="HTML")
    await query.answer()

@router.callback_query(F.data.startswith("adm:promo_card:"))
async def cb_admin_promo_class_card(query: CallbackQuery):
    ...
    async with AsyncSessionLocal() as session:
        ...
        students_cnt = (await session.execute(select(func.count(Student.id)).where(Student.class_name == class_name))).scalar() or 0
        m = re.match(r"^(\d+)(.*)$", class_name.strip())
        grade_num = int(m.group(1)) if m else 0
        suffix = m.group(2) if m else ""

        is_senior = (grade_num >= 11)
        is_final_grad = (grade_num >= 12)
        next_cls = f"{grade_num + 1}{suffix}" if not is_final_grad else "MEZUN"

        card_title = {
            "tr": f"🎓 <b>{class_name} SINIFI TERFİ ÖN İZLEME</b>",
            "ru": f"🎓 <b>ПРЕДПРОСМОТР ПЕРЕВОДА КЛАССА {class_name}</b>",
            "uz": f"🎓 <b>{class_name} SINFI KO'CHIRISH TAFSILOTI</b>",
            "en": f"🎓 <b>CLASS {class_name} PROMOTION PREVIEW</b>"
        }.get(lang, f"🎓 Class {class_name} Preview")

        lbl_cur = {"tr": "Mevcut Sınıf", "ru": "Текущий класс", "uz": "Amaldagi sinf", "en": "Current Class"}.get(lang, "Current Class")
        lbl_cnt = {"tr": "Öğrenci Sayısı", "ru": "Количество учеников", "uz": "O'quvchilar soni", "en": "Student Count"}.get(lang, "Students")
        lbl_target = {"tr": "Hedef Kademe", "ru": "Целевой класс", "uz": "Ko'chiriladigan bosqich", "en": "Target Level"}.get(lang, "Target")

        target_display = f"<code>{next_cls}</code>" if not is_final_grad else "🎓 <b>MEZUN (Arşiv)</b>"

        note_promo = {
            "tr": (
                "ℹ️ <i>Bu işlem yalnızca bu sınıftaki öğrencileri etkiler. "
                "İşlem onaylandığında sınıf listesi güncellenir ve geri alınabilmesi için Excel yedeği sohbete iletilir.</i>"
            ),
            "ru": (
                "ℹ️ <i>Это действие затронет только учеников данного класса. "
                "Перед переводом в чат автоматически отправляется копия базы данных в Excel.</i>"
            ),
            "uz": (
                "ℹ️ <i>Ushbu amal faqat ushbu sinfdagi o'quvchilarga taalluqlidir. "
                "Amal tasdiqlanganda sinf ro'yxati yangilanadi va zaxira nusxasi chatga yuboriladi.</i>"
            ),
            "en": (
                "ℹ️ <i>This operation only affects students in this class. "
                "An Excel backup will be generated and sent to this chat before promotion.</i>"
            )
        }.get(lang, "")
        ...
        card_text = (
            f"{card_title}\n"
            "────────────────────\n"
            f"🏫 <b>{lbl_cur}:</b> <code>{class_name}</code>\n"
            f"👥 <b>{lbl_cnt}:</b> <b>{students_cnt}</b>\n"
            f"🎯 <b>{lbl_target}:</b> {target_display}\n"
            "────────────────────\n"
            f"{note_promo}"
        )

        buttons = []
        if not is_final_grad:
            b_next_txt = {
                "tr": f"✅ {class_name} Sınıfını Yükselt (➔ {next_cls})",
                "ru": f"✅ Перевести {class_name} (➔ {next_cls})",
                "uz": f"✅ {class_name} sinfini ko'chirish (➔ {next_cls})",
                "en": f"✅ Promote {class_name} (➔ {next_cls})"
            }.get(lang, f"✅ Promote (➔ {next_cls})")
            buttons.append([InlineKeyboardButton(text=b_next_txt, callback_data=f"adm:promo_do:{class_name}:next")])

        if is_senior:
            b_grad_txt = {
                "tr": f"🎓 {class_name} Sınıfını Mezun Et (Arşive Ekle)",
                "ru": f"🎓 Выпустить класс {class_name} (В архив)",
                "uz": f"🎓 {class_name} sinfini bitiruvchi qilish (Arxivga)",
                "en": f"🎓 Graduate Class {class_name} (Archive)"
            }.get(lang, f"🎓 Graduate {class_name}")
            buttons.append([InlineKeyboardButton(text=b_grad_txt, callback_data=f"adm:promo_do:{class_name}:graduate")])

        buttons.append([InlineKeyboardButton(text=get_text("btn_back", lang), callback_data="adm:promo_cls_list:0")])
        buttons.append(get_nav_buttons(lang, back_callback="adm:class_promotion_init"))

        await safe_edit_or_answer(query, card_text, reply_markup=InlineKeyboardMarkup(inline_keyboard=buttons), parse_mode="HTML")
    await query.answer()

@router.callback_query(F.data.startswith("adm:promo_do:"))
async def cb_admin_promo_class_do(query: CallbackQuery):
    parts = query.data.split(":")
    class_name = parts[2]
    action = parts[3]  # "next" or "graduate"
    ...
    async with AsyncSessionLocal() as session:
        ...
        # 1. Send Excel backup of the school
        try:
            buf = await export_all_school_data_excel()
            today_str = datetime.utcnow().strftime("%d_%m_%Y")
            file = BufferedInputFile(buf.read(), filename=f"Terfi_Yedek_{class_name}_{today_str}.xlsx")
            caption = {"tr": f"📦 <b>{class_name} Sınıfı Terfisi Öncesi Sistem Yedeği</b>", "ru": f"📦 <b>Архив системы перед переводом класса {class_name}</b>", "uz": f"📦 <b>{class_name} sinfi ko'chirilishidan oldingi zaxira</b>", "en": f"📦 <b>Pre-Promotion System Backup for Class {class_name}</b>"}.get(lang, "📦 Backup")
            await bot_obj.send_document(chat_id=target_chat_id, document=file, caption=caption, parse_mode="HTML")
            buf.close()
        except Exception:
            pass
        ...
        if action == "graduate" or (action == "next" and grade_num >= 12):
            # Graduate all students in this class
            for s in students:
                grad = Graduate(
                    full_name=s.full_name,
                    student_number=s.student_number,
                    graduated_class=class_name,
                    graduation_year=current_year,
                    parent_phone=None
                )
                session.add(grad)
                s.class_name = f"MEZUN-{class_name}"
            await session.commit()

            succ_msg = {
                "tr": (
                    f"🎓 <b>{class_name} SINIFI BAŞARIYLA MEZUN EDİLDİ!</b>\n"
                    "────────────────────\n"
                    f"• Mezun Edilen Öğrenci: <b>{len(students)}</b>\n"
                    f"• Mezuniyet Yılı: <b>{current_year}</b>\n"
                    "────────────────────\n"
                    "<i>Öğrenciler başarıyla 'Mezunlar Arşivi'ne taşınmış ve kayıt altına alınmıştır.</i>"
                ),
                "ru": (
                    f"🎓 <b>КЛАСС {class_name} УСПЕШНО ВЫПУЩЕН!</b>\n"
                    "────────────────────\n"
                    f"• Выпущено учеников: <b>{len(students)}</b>\n"
                    f"• Год выпуска: <b>{current_year}</b>\n"
                    "────────────────────\n"
                    "<i>Все выпускники успешно сохранены в Архиве выпускников.</i>"
                ),
                "uz": (
                    f"🎓 <b>{class_name} SINFI MUVAFFAQIYATLI BITIRUVCHI QILINDI!</b>\n"
                    "────────────────────\n"
                    f"• Bitirgan o'quvchilar: <b>{len(students)}</b> ta\n"
                    f"• Bitiruv yili: <b>{current_year}</b>\n"
                    "────────────────────\n"
                    "<i>O'quvchilar 'Bitiruvchilar arxivi'ga muvaffaqiyatli saqlandi.</i>"
                ),
                "en": (
                    f"🎓 <b>CLASS {class_name} GRADUATED SUCCESSFULLY!</b>\n"
                    "────────────────────\n"
                    f"• Graduated Students: <b>{len(students)}</b>\n"
                    f"• Graduation Year: <b>{current_year}</b>\n"
                    "────────────────────\n"
                    "<i>All students have been archived in the Alumni Archive.</i>"
                )
            }.get(lang, f"Class {class_name} graduated.")

        else:
            new_cls = f"{grade_num + 1}{suffix}"
            for s in students:
                s.class_name = new_cls
            
            # Also ensure ClassRoom exists or is updated
            cr_exist = (await session.execute(select(ClassRoom).where(ClassRoom.name == new_cls))).scalar_one_or_none()
            if not cr_exist:
                session.add(ClassRoom(name=new_cls))

            await session.commit()

            succ_msg = {
                "tr": (
                    f"✅ <b>SINIF TERFİSİ TAMAMLANDI!</b>\n"
                    "────────────────────\n"
                    f"🏫 <b>{class_name} ➔ {new_cls}</b>\n"
                    f"• Yükseltilen Öğrenci: <b>{len(students)}</b>\n"
                    "────────────────────\n"
                    f"<i>{class_name} sınıfındaki tüm öğrencilerin seviyesi başarıyla {new_cls} olarak güncellenmiştir.</i>"
                ),
                "ru": (
                    f"✅ <b>ПЕРЕВОД КЛАССА УСПЕШНО ЗАВЕРШЕН!</b>\n"
                    "────────────────────\n"
                    f"🏫 <b>{class_name} ➔ {new_cls}</b>\n"
                    f"• Переведено учеников: <b>{len(students)}</b>\n"
                    "────────────────────\n"
                    f"<i>Все ученики класса {class_name} успешно переведены в {new_cls}.</i>"
                ),
                "uz": (
                    f"✅ <b>SINF KO'CHIRISH YAKUNLANDI!</b>\n"
                    "────────────────────\n"
                    f"🏫 <b>{class_name} ➔ {new_cls}</b>\n"
                    f"• Ko'chirilgan o'quvchilar: <b>{len(students)}</b> ta\n"
                    "────────────────────\n"
                    f"<i>{class_name} sinfidagi barcha o'quvchilar muvaffaqiyatli {new_cls} ga o'tkazildi.</i>"
                ),
                "en": (
                    f"✅ <b>CLASS PROMOTION COMPLETED!</b>\n"
                    "────────────────────\n"
                    f"🏫 <b>{class_name} ➔ {new_cls}</b>\n"
                    f"• Promoted Students: <b>{len(students)}</b>\n"
                    "────────────────────\n"
                    f"<i>All students from class {class_name} have been advanced to {new_cls}.</i>"
                )
            }.get(lang, f"Class {class_name} promoted to {new_cls}.")

        buttons = [
            [InlineKeyboardButton(text={"tr": "🏫 Başka Bir Sınıf Seç", "ru": "🏫 Выбрать другой класс", "uz": "🏫 Boshqa sinfni tanlash", "en": "🏫 Select Another Class"}.get(lang, "Select Another Class"), callback_data="adm:promo_cls_list:0")],
            [InlineKeyboardButton(text={"tr": "🎓 Mezunlar Arşivi", "ru": "🎓 Архив выпускников", "uz": "🎓 Bitiruvchilar arxivi", "en": "🎓 Alumni Archive"}.get(lang, "Alumni Archive"), callback_data="adm:graduates:0")],
            get_nav_buttons(lang, back_callback="adm:class_promotion_init")
        ]
        await safe_edit_or_answer(query, succ_msg, reply_markup=InlineKeyboardMarkup(inline_keyboard=buttons), parse_mode="HTML")
    await query.answer()
...
async def cb_grade_save_final(query: CallbackQuery, state: FSMContext):
    ...
    async with AsyncSessionLocal() as session:
        ...
        for p_id in parents:
            ...
            p_lang = p_u.language if p_u else "tr"
            exam_type_map = {
                "1. Yazılı": {"tr": "1. Yazılı", "ru": "1-я Письменная", "uz": "1-Yozma", "en": "1st Written Exam"},
                "2. Yazılı": {"tr": "2. Yazılı", "ru": "2-я Письменная", "uz": "2-Yozma", "en": "2nd Written Exam"},
                "Sözlü / Performans": {"tr": "Sözlü / Performans", "ru": "Устный опрос", "uz": "Og'zaki", "en": "Oral / Performance"}
            }
            exam_type_loc = exam_type_map.get(exam_type_val, {}).get(p_lang, exam_type_val)
            alert_text = get_text("grade_parent_notification", p_lang, name=escape_html(st.full_name), subject=escape_html(subject_name), exam_type=escape_html(exam_type_loc), score=score_val, badge=badge_val)
...
async def cb_behavior_type_chosen(query: CallbackQuery, state: FSMContext):
    ...
    if b_type == "pos":
        badges = {
            "tr": [("⭐ Derse Aktif Katılım", "⭐"), ("🏆 Üstün Başarı", "🏆"), ("👏 Nezaket & Yardımlaşma", "👏"), ("📖 Kitap Okuma & Gayret", "📖")],
            "ru": [("⭐ Активность на уроке", "⭐"), ("🏆 Высокие успехи", "🏆"), ("👏 Вежливость и помощь", "👏"), ("📖 Чтение и прилежание", "📖")],
            "uz": [("⭐ Darsda faol ishtirok", "⭐"), ("🏆 A'lo natija", "🏆"), ("👏 Odob va yordam", "👏"), ("📖 Kitobxonlik va g'ayrat", "📖")],
            "en": [("⭐ Active Participation", "⭐"), ("🏆 High Achievement", "🏆"), ("👏 Kindness & Respect", "👏"), ("📖 Reading & Effort", "📖")]
        }.get(lang, [("⭐ Active Participation", "⭐"), ("🏆 Achievement", "🏆"), ("👏 Kindness", "👏"), ("📖 Effort", "📖")])
    else:
        badges = {
            "tr": [("⚠️ Derse Geç Kalma", "⚠️"), ("🔴 Kural İhlali", "🔴"), ("📵 Telefon Kuralı", "📵"), ("🗣️ Dersi Kaynatma", "🗣️")],
            "ru": [("⚠️ Опоздание на урок", "⚠️"), ("🔴 Нарушение правил", "🔴"), ("📵 Телефон на уроке", "📵"), ("🗣️ Разговоры на уроке", "🗣️")],
            "uz": [("⚠️ Darsga kechikish", "⚠️"), ("🔴 Tartib buzish", "🔴"), ("📵 Telefondan foydalanish", "📵"), ("🗣️ Darsda gaplashish", "🗣️")],
            "en": [("⚠️ Late to Class", "⚠️"), ("🔴 Rule Violation", "🔴"), ("📵 Phone Usage", "📵"), ("🗣️ Disrupting Class", "🗣️")]
        }.get(lang, [("⚠️ Late to Class", "⚠️"), ("🔴 Rule Violation", "🔴"), ("📵 Phone Usage", "📵"), ("🗣️ Disrupting Class", "🗣️")])
...
async def process_hw_content(message: Message, state: FSMContext):
    ...
    async with AsyncSessionLocal() as session:
        ...
        for t_id in target_ids:
            ...
            dispatch_texts = {
                "tr": (
                    f"📚 <b>ÖDEV PANOSU • {escape_html(class_name)}</b>\n"
                    "────────────────────\n"
                    f"📖 <b>Ders:</b> {escape_html(subj)}\n"
                    "────────────────────\n"
                    f"📝 <b>Ödev Detayı:</b>\n"
                    f"{escape_html(hw_text)}\n"
                    "────────────────────"
                ),
                "ru": (
                    f"📚 <b>ДОСКА ЗАДАНИЙ • {escape_html(class_name)}</b>\n"
                    "────────────────────\n"
                    f"📖 <b>Предмет:</b> {escape_html(subj)}\n"
                    "────────────────────\n"
                    f"📝 <b>Задание:</b>\n"
                ...
                "uz": (
                    f"📚 <b>VAZIFALAR PANELI • {escape_html(class_name)}</b>\n"
                    "────────────────────\n"
                    f"📖 <b>Fan:</b> {escape_html(subj)}\n"
                    "────────────────────\n"
                    f"📝 <b>Vazifa:</b>\n"
                    f"{escape_html(hw_text)}\n"
                    "────────────────────"
                ...
                "en": (
                    f"📚 <b>HOMEWORK BOARD • {escape_html(class_name)}</b>\n"
                    "────────────────────\n"
                    f"📖 <b>Subject:</b> {escape_html(subj)}\n"
                    "────────────────────\n"
                    f"📝 <b>Homework Details:</b>\n"
                    f"{escape_html(hw_text)}\n"
                    "────────────────────"
                )
            }
...
async def process_homework_submission(message: Message, state: FSMContext):
    ...
    async with AsyncSessionLocal() as session:
        ...
        now_hw_str = get_local_now().strftime('%d.%m.%Y %H:%M')
        hw_receipt_txt = {
            "tr": (
                f"📘 <b>Ödev:</b> {escape_html(hw.title)}\n"
                f"👤 <b>Öğrenci:</b> {escape_html(st.full_name)} (No: {st.student_number})\n"
                f"📅 <b>Teslim Zamanı:</b> <code>{now_hw_str}</code>\n"
                f"🆔 <b>Kayıt No:</b> <code>#ODV-{sub.id:04d}</code>\n"
                f"✅ <i>Ödeviniz başarıyla öğretmen incelemesine gönderildi.</i>"
            ),
            "ru": (
                f"📘 <b>Домашнее задание:</b> {escape_html(hw.title)}\n"
                f"👤 <b>Ученик:</b> {escape_html(st.full_name)} (№: {st.student_number})\n"
                f"📅 <b>Время сдачи:</b> <code>{now_hw_str}</code>\n"
                f"🆔 <b>Квитанция №:</b> <code>#ODV-{sub.id:04d}</code>\n"
                f"✅ <i>Ваше задание успешно отправлено на проверку учителю.</i>"
            ),
            "uz": (
                f"📘 <b>Vazifa:</b> {escape_html(hw.title)}\n"
                f"👤 <b>O'quvchi:</b> {escape_html(st.full_name)} (№: {st.student_number})\n"
                f"📅 <b>Topshirilgan vaqt:</b> <code>{now_hw_str}</code>\n"
                f"🆔 <b>Kvitansiya №:</b> <code>#ODV-{sub.id:04d}</code>\n"
                f"✅ <i>Vazifangiz tekshirish uchun o'qituvchiga muvaffaqiyatli yuborildi.</i>"
            ),
            "en": (
                f"📘 <b>Homework:</b> {escape_html(hw.title)}\n"
                f"👤 <b>Student:</b> {escape_html(st.full_name)} (Roll: {st.student_number})\n"
                f"📅 <b>Submission Time:</b> <code>{now_hw_str}</code>\n"
                f"🆔 <b>Receipt ID:</b> <code>#ODV-{sub.id:04d}</code>\n"
                f"✅ <i>Your homework has been submitted for teacher review.</i>"
            )
        }.get(lang, f"Homework submission for {hw.title}")

        hw_title_loc = {
            "tr": f"Ödev Teslim Makbuzu: {hw.title}",
            "ru": f"Квитанция сдачи ДЗ: {hw.title}",
            "uz": f"Vazifa topshirish kvitansiyasi: {hw.title}",
            "en": f"Homework Submission Receipt: {hw.title}"
        }.get(lang, f"Homework Receipt: {hw.title}")
...
async def cb_admin_add_exam_init(query: CallbackQuery, state: FSMContext):
    ...
    cancel_kb = get_inline_cancel_kb(lang, back_callback=f"adm:show_class:{class_name}")
    p_ex_sb = {
        "tr": f"📅 <b>{class_name} Sınav Girişi</b>\n\nLütfen Ders Adını yazınız (Örn: <code>Matematik</code>):",
        "ru": f"📅 <b>Ввод экзамена для класса {class_name}</b>\n\nВведите название предмета (Напр: <code>Математика</code>):",
        "uz": f"📅 <b>{class_name} sinfi imtihon kiritish</b>\n\nFan nomini kiriting (Masalan: <code>Matematika</code>):",
        "en": f"📅 <b>Add Exam for Class {class_name}</b>\n\nEnter Subject Name (e.g. <code>Mathematics</code>):"
...
async def process_exam_subject(message: Message, state: FSMContext):
    ...
    async with AsyncSessionLocal() as session:
        ...
        lang = user.language if user else "tr"
    p_ex_dt = {
        "tr": "📅 Lütfen sınav tarihini ve saatini yazınız (Örn: <code>2026-06-15 09:30</code>):",
        "ru": "📅 Введите дату и время экзамена (Напр: <code>2026-06-15 09:30</code>):",
        "uz": "📅 Imtihon sanasi va vaqtini kiriting (Masalan: <code>2026-06-15 09:30</code>):",
        "en": "📅 Please enter exam date and time (e.g. <code>2026-06-15 09:30</code>):"
...
async def process_exam_date(message: Message, state: FSMContext):
    ...
    async with AsyncSessionLocal() as session:
        ...
        done_ex = {
            "tr": f"✅ <b>{class_name}</b> sınıfı için <b>{subj}</b> sınav tarihi kaydedildi: <code>{exam_d} {exam_t}</code>",
            "ru": f"✅ Для класса <b>{class_name}</b> сохранен экзамен по предмету <b>{subj}</b>: <code>{exam_d} {exam_t}</code>",
            "uz": f"✅ <b>{class_name}</b> sinfi uchun <b>{subj}</b> fanidan imtihon sanasi saqlandi: <code>{exam_d} {exam_t}</code>",
            "en": f"✅ Exam for class <b>{class_name}</b> ({subj}) saved: <code>{exam_d} {exam_t}</code>"
        }.get(lang, "Exam saved.")
...
async def cb_admin_emergency_guard(query: CallbackQuery, state: FSMContext):
...
async def process_emergency_text(message: Message, state: FSMContext):
    ...
    async with AsyncSessionLocal() as session:
        ...
        sent_cnt = 0
        for p in parents:
            p_l = p.language or "tr"
            em_hdr = {"tr": "🚨 <b>ACİL DURUM / KIRMIZI ALARM</b> 🚨", "ru": "🚨 <b>ЭКСТРЕННОЕ ОПОВЕЩЕНИЕ / КРАСНЫЙ СИГНАЛ</b> 🚨", "uz": "🚨 <b>FAVQULODDA XABAR / QIZIL ALARM</b> 🚨", "en": "🚨 <b>EMERGENCY ALERT / RED ALARM</b> 🚨"}.get(p_l, "🚨 <b>EMERGENCY ALERT</b> 🚨")
            em_hint = {"tr": "⚠️ Lütfen aşağıdaki butona basarak bildirim aldığınızı onaylayınız:", "ru": "⚠️ Пожалуйста, нажмите кнопку ниже для подтверждения получения:", "uz": "⚠️ Iltimos, xabarni olganingizni tasdiqlash uchun quyidagi tugmani bosing:", "en": "⚠️ Please confirm receipt by pressing the button below:"}.get(p_l, "⚠️ Please confirm receipt:")
            em_msg = f"{em_hdr}\n\n{escape_html(em_text)}\n\n{em_hint}"
        ...
        em_rep = {
            "tr": f"🚨 Acil durum alarmı <b>{sent_cnt}</b> veliye sesli bildirimle iletildi!",
            "ru": f"🚨 Экстренное оповещение отправлено <b>{sent_cnt}</b> родителям со звуковым сигналом!",
            "uz": f"🚨 Favqulodda xabar <b>{sent_cnt}</b> ta ota-onaga ovozli signal bilan yetkazildi!",
            "en": f"🚨 Emergency alert dispatched to <b>{sent_cnt}</b> parents with high-priority audio!"
        }.get(lang, "Alert sent.")
...
async def cb_view_report_card(query: CallbackQuery, state: FSMContext):
    ...
    async with AsyncSessionLocal() as session:
        ...
        for s_name, scores in subj_map.items():
            ...
            lbl_avg_s = {"tr": "Ort", "ru": "Ср", "uz": "O'rt", "en": "Avg"}.get(lang, "Avg")
            subj_lines.append(f"📚 <b>{escape_html(s_name)}</b> ({lbl_avg_s}: <b>{s_avg}</b> {badge})\n   └ <code>[{scores_str}]</code>")
        ...
        no_gr_txt = {"tr": "<i>Henüz ders notu girilmemiş.</i>", "ru": "<i>Оценки пока не выставлены.</i>", "uz": "<i>Hali baholar kiritilmagan.</i>", "en": "<i>No grades recorded yet.</i>"}.get(lang, "<i>No grades yet.</i>")
        grades_block = "\n".join(subj_lines) if subj_lines else no_gr_txt
        ...
        text = (
            f"<b>{rc_title}</b>\n"
            "────────────────────\n"
            f"🧑‍🎓 <b>{escape_html(st.full_name)}</b> ({escape_html(st.class_name)})\n"
            ...
            f"📊 <code>{gpa_bar}</code> {honor_badge}\n"
            "────────────────────"
        )
...
async def cb_download_pdf_report(query: CallbackQuery):
    ...
    async with AsyncSessionLocal() as session:
        ...
        v_list = DOWNLOAD_VELOCITY.setdefault(query.from_user.id, [])
        v_list[:] = [t for t in v_list if (now_dl - t).total_seconds() < 180]
        v_list.append(now_dl)
        if len(v_list) > 5:
            asyncio.create_task(alert_admins_security_breach(
                bot=query.bot or bot,
                user_id=query.from_user.id,
                user_name=query.from_user.full_name or "Kullanıcı",
                event_title="Toplu Veri İndirme Şüphesi (Bulk Exfiltration Risk)",
                details=f"Kullanıcı son 3 dakika içinde {len(v_list)} adet evrak/karne indirdi."
            ))
...
async def cb_parent_excuse_send(query: CallbackQuery):
    ...
    async with AsyncSessionLocal() as session:
        ...
        exc_title_loc = {
            "tr": f"Mazeret İzni: {st.full_name}",
            "ru": f"Заявление об отсутствии: {st.full_name}",
            "uz": f"Ruxsatnoma arizasi: {st.full_name}",
            "en": f"Absence Excuse: {st.full_name}"
        }.get(lang, f"Absence Excuse: {st.full_name}")

        exc_content_loc = {
            "tr": f"Öğrenci: {st.full_name} ({st.class_name})\nTarih: {today_d.strftime('%d.%m.%Y')}\nDurum: {reason_txt}\nOnaylayan: Veli Beyanı (Dijital İzin)",
            "ru": f"Ученик: {st.full_name} ({st.class_name})\nДата: {today_d.strftime('%d.%m.%Y')}\nПричина: {reason_txt}\nПодтверждение: Заявление родителя",
            "uz": f"O'quvchi: {st.full_name} ({st.class_name})\nSana: {today_d.strftime('%d.%m.%Y')}\nHolat: {reason_txt}\nTasdiqlovchi: Ota-ona arizasi",
            "en": f"Student: {st.full_name} ({st.class_name})\nDate: {today_d.strftime('%d.%m.%Y')}\nReason: {reason_txt}\nApprover: Parent Digital Statement"
        }.get(lang, f"Student: {st.full_name}\nDate: {today_d}\nReason: {reason_txt}")
        ...
        await query.answer({"tr": "✅ Mazeret onaylandı!", "ru": "✅ Заявка принята!", "uz": "✅ Ruxsatnoma qabul qilindi!", "en": "✅ Excuse accepted!"}.get(lang, "✅ Accepted!"), show_alert=False)
...
async def cb_parent_report(query: CallbackQuery):
    await cb_view_report_card(query, None)

@router.callback_query(F.data.in_(["adm:change_pin_init", "chgpin:cancel", "pinkey:cancel"]))
async def cb_admin_pin_disabled_stub(query: CallbackQuery):
    try: await query.answer()
    except Exception: pass
    await cb_cat_settings(query, None)

@router.callback_query(F.data.startswith("pinkey:") | F.data.startswith("chgpin:"))
async def cb_admin_pin_ignore(query: CallbackQuery):
    try: await query.answer()
    except Exception: pass
...
def match_reply_button(text: str) -> str | None:
    ...
    # 1. Birebir lokalizasyon eşleşmesi (tüm dillerdeki buton etiketleri)
    ...
    # 2. Emojisiz / Temizlenmiş tam metin eşleşmesi
    clean_norm = re.sub(r'^[^\w]+|[^\w]+$', '', clean_text).strip().lower()
    for key, action in REPLY_BUTTON_ACTIONS.items():
        for lang_code in LOCALES:
            btn_txt = LOCALES[lang_code].get(key, "")
            if btn_txt:
                norm_btn = re.sub(r'^[^\w]+|[^\w]+$', '', btn_txt).strip().lower()
                if norm_btn and norm_btn == clean_norm:
                    return action

    # 3. Bilinen standart buton tam adları (Asla kısmi alt dize araması yapılmaz!)
    exact_known_labels = {
        "ana menu": "act_main_menu",
        "ana menü": "act_main_menu",
        "baş menü": "act_main_menu",
        "главное меню": "act_main_menu",
        "главное": "act_main_menu",
        "main menu": "act_main_menu",
        "asosiy menyu": "act_main_menu",
        "menu": "act_main_menu",
        "menü": "act_main_menu",
        "kadro ve ogrenciler": "act_cat_staff",
        "kadro & ogrenci": "act_cat_staff",
        "kadro & öğrenci": "act_cat_staff",
        "персонал и дети": "act_cat_staff",
        "ученики и учителя": "act_cat_staff",
        "staff & students": "act_cat_staff",
        "xodimlar va o'quvchilar": "act_cat_staff",
        "raporlar & denetim": "act_cat_tools_reports",
        "отчеты и контроль": "act_cat_tools_reports",
        "idari araclar": "act_cat_tools_reports",
        "инструменты": "act_cat_tools_reports",
        "onay masasi": "act_cat_requests",
        "центр одобрений": "act_cat_requests",
        "sistem & ayarlar": "act_cat_settings",
        "настройки": "act_cat_settings",
        "настройки системы": "act_cat_settings",
        "cikis yap": "act_logout",
        "çıkış yap": "act_logout",
        "выйти": "act_logout",
        "log out": "act_logout",
        "chiqish": "act_logout",
    }
    return exact_known_labels.get(clean_norm, None)
...
async def cb_dl_view(query: CallbackQuery):
    ...
    async with AsyncSessionLocal() as session:
        ...
        hdr_dl = {"tr": "📁 <b>DİJİTAL EVRAK DETAYI</b>", "ru": "📁 <b>ДЕТАЛИ ДОКУМЕНТА</b>", "uz": "📁 <b>RAQAMLI HUJJAT TAFSILOTI</b>", "en": "📁 <b>DIGITAL DOCUMENT DETAILS</b>"}.get(lang, "📁 <b>DOCUMENT DETAILS</b>")
        lbl_dl_doc = {"tr": "Evrak", "ru": "Документ", "uz": "Hujjat", "en": "Document"}.get(lang, "Document")
        lbl_dl_date = {"tr": "Kayıt Tarihi", "ru": "Дата сохранения", "uz": "Saqlangan sana", "en": "Recorded Date"}.get(lang, "Date")
        lbl_dl_id = {"tr": "Kayıt No", "ru": "Номер записи", "uz": "Qayd raqami", "en": "Record ID"}.get(lang, "Record ID")

        detail_html = (
            f"{hdr_dl}\n"
            "────────────────────\n"
            f"📄 <b>{lbl_dl_doc}:</b> {escape_html(rec.title)}\n"
            f"📅 <b>{lbl_dl_date}:</b> <code>{d_str}</code>\n"
            f"🆔 <b>{lbl_dl_id}:</b> <code>#EVR-{rec.id:04d}</code>\n"
            "────────────────────\n\n"
            f"{rec.full_content}"
        )
...
async def cb_dl_file(query: CallbackQuery):
    ...
    async with AsyncSessionLocal() as session:
        ...
        try:
            ...
            await query.answer({"tr": "✅ Dosya iletildi.", "ru": "✅ Файл отправлен.", "uz": "✅ Fayl yuborildi.", "en": "✅ File sent."}.get(lang, "✅ File sent."))
        except Exception as e:
            await query.answer({"tr": "❌ Dosya gönderilemedi.", "ru": "❌ Не удалось отправить файл.", "uz": "❌ Faylni yuborib bolmadi.", "en": "❌ Failed to send file."}.get(lang, "❌ Failed to send file."), show_alert=True)
...
async def cb_lost_item_view(query: CallbackQuery):
    ...
    async with AsyncSessionLocal() as session:
        ...
        hdr_li = {"tr": "📦 <b>KAYIP EŞYA DETAYI</b>", "ru": "📦 <b>ДЕТАЛИ НАЙДЕННОЙ ВЕЩИ</b>", "uz": "📦 <b>TOPILGAN BUYUM TAFSILOTI</b>", "en": "📦 <b>LOST & FOUND ITEM DETAILS</b>"}.get(lang, "📦 <b>LOST ITEM DETAILS</b>")
        lbl_li_item = {"tr": "Eşya", "ru": "Предмет", "uz": "Buyum", "en": "Item"}.get(lang, "Item")
        lbl_li_loc = {"tr": "Bulunduğu Yer", "ru": "Место находки", "uz": "Topilgan joy", "en": "Location Found"}.get(lang, "Location")
        lbl_li_date = {"tr": "Tarih", "ru": "Дата", "uz": "Sana", "en": "Date"}.get(lang, "Date")
        lbl_li_desc = {"tr": "Açıklama", "ru": "Описание", "uz": "Tavsif", "en": "Description"}.get(lang, "Description")
        lbl_li_stat = {"tr": "Durum", "ru": "Статус", "uz": "Holat", "en": "Status"}.get(lang, "Status")
        val_li_stat = {"tr": "🟢 TESLİM BEKLİYOR", "ru": "🟢 ОЖИДАЕТ ВЫДАЧИ", "uz": "🟢 TOPSHIRILISHI KUTILMOQDA", "en": "🟢 AWAITING COLLECTION"}.get(lang, "🟢 PENDING")
        val_none = {"tr": "Belirtilmedi", "ru": "Не указано", "uz": "Ko'rsatilmadi", "en": "Not specified"}.get(lang, "-")
        hint_li = {"tr": "Bu eşya size aitse aşağıdaki butona tıklayarak idareye bildirim gönderebilirsiniz.", "ru": "Если эта вещь принадлежит вам, нажмите кнопку ниже для обращения к администрации.", "uz": "Agar bu buyum sizga tegishli bo'lsa, quyidagi tugmani bosib ma'muriyatga xabar bering.", "en": "If this item belongs to you, tap the button below to submit a claim."}.get(lang, "")

        card_text = (
            f"{hdr_li}\n"
            "────────────────────\n"
            f"• <b>{lbl_li_item}:</b> {escape_html(item.title)}\n"
            f"• <b>{lbl_li_loc}:</b> {escape_html(item.location_found)}\n"
            f"• <b>{lbl_li_date}:</b> <i>{d_str}</i>\n"
            f"• <b>{lbl_li_desc}:</b> {escape_html(item.description or val_none)}\n"
            f"• <b>{lbl_li_stat}:</b> {val_li_stat}\n\n"
            f"<i>{hint_li}</i>"
        )
        buttons = [
...
async def smart_text_auth_router(message: Message, state: FSMContext):
    ...
    async with AsyncSessionLocal() as session:
        ...
        if is_code or user.role == "guest":
            ...
            # Kullanıcının yazdığı mesajları sessizce silme!
            pass
...
async def cb_admin_bot_block_monitor(query: CallbackQuery):
    ...
    async with AsyncSessionLocal() as session:
        ...
        lbl_stats_hdr = {"tr": "📊 <b>İletişim Sağlığı ve İstatistikler:</b>", "ru": "📊 <b>Состояние связи и статистика:</b>", "uz": "📊 <b>Aloqa holati va statistikasi:</b>", "en": "📊 <b>Delivery Health & Statistics:</b>"}.get(lang, "📊 <b>Statistics:</b>")
        lbl_tot_u = {"tr": "Toplam Kayıtlı", "ru": "Всего пользователей", "uz": "Jami foydalanuvchilar", "en": "Total Registered"}.get(lang, "Total")
        lbl_act_u = {"tr": "Aktif Ulaşılabilir", "ru": "Доступны для связи", "uz": "Faol aloqadagilar", "en": "Reachable Active"}.get(lang, "Reachable")
        lbl_blk_u = {"tr": "Botu Engelleyen", "ru": "Заблокировали бота", "uz": "Botni bloklaganlar", "en": "Blocked the Bot"}.get(lang, "Blocked")
        lbl_unit_u = {"tr": "Kullanıcı", "ru": "чел.", "uz": "ta", "en": "users"}.get(lang, "users")
        lbl_blk_note = {"tr": "⚠️ <i>Botu engelleyen kullanıcılara acil durumlarda sesli zorla bildirim gönderebilirsiniz.</i>", "ru": "⚠️ <i>Пользователям, заблокировавшим бота, можно отправить экстренное звуковое оповещение.</i>", "uz": "⚠️ <i>Botni bloklaganlarga shoshilinch holatlarda ovozli xabar yuborish mumkin.</i>", "en": "⚠️ <i>High-priority force audio alerts can be dispatched to blocked users in emergencies.</i>"}.get(lang, "")
        ...
        stat_box = (
            ...
            "────────────────────\n"
            f"{lbl_stats_hdr}\n"
            f"• 👥 <b>{lbl_tot_u}:</b> {total_users} {lbl_unit_u}\n"
            f"• 🟢 <b>{lbl_act_u}:</b> {active_reach} {lbl_unit_u}\n"
            f"• 🚫 <b>{lbl_blk_u}:</b> {total_blocked} {lbl_unit_u}\n\n"
            f"{lbl_blk_note}\n"
            "────────────────────"
        )
...
async def cb_force_audio_alert_init(query: CallbackQuery):
    async with AsyncSessionLocal() as session:
        ...
        hdr = {
            "tr": "🚨 <b>ACİL DURUM BİLDİRİM MASASI (YÜKSEK ÖNCELİK)</b>\n────────────────────\nBu işlem botu sessize alan veya bildirimleri kapatan kullanıcılara Telegram'ın <code>disable_notification=False</code> ve mesaj sabitleme parametreleri ile <b>yüksek öncelikli sesli ve görsel alarm</b> gönderir.\n\nLütfen hedef kitleyi seçiniz:",
            ...
            "uz": "🚨 <b>SHOSHILINCH XABAR (YUQORI DARAJALI)</b>\n────────────────────\nFoydalanuvchilarga ovozli bildirishnoma va qadab qo'yish orqali yuqori darajadagi shoshilinch xabar yuboradi.\n\nAuditoriyani tanlang:",
            "en": "🚨 <b>EMERGENCY ALERT (HIGH PRIORITY)</b>\n────────────────────\nDispatches high-priority alert with audio notification and chat priority signal.\n\nSelect target audience:"
...
async def process_force_audio_alert_text(message: Message, state: FSMContext):
    ...
    async with AsyncSessionLocal() as session:
        ...
        # Acil Durum kaydı aç (onaylayanları ve toplam sayıyı admine canlı raporlamak için)
        em_alert = EmergencyAlert(message_text=raw_text, created_by=user_id, created_at=datetime.utcnow())
        session.add(em_alert)
        await session.commit()
        await session.refresh(em_alert)
        ...
        for tid in target_ids:
            ...
            ack_kb = InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text=ack_btn_txt, callback_data=f"ack_fa:{user_id}:{em_alert.id}")]])
            ...
            try:
                ...
                # Ekranın En Üstüne Sabitle (Sabitleme sinyali ile sesli uyarı)
                if sent_msg:
                    try:
...
async def cb_ack_force_alert(query: CallbackQuery):
    parts = query.data.split(":")
    admin_id = int(parts[1]) if len(parts) > 1 and parts[1].isdigit() else None
    alert_id = int(parts[2]) if len(parts) > 2 and parts[2].isdigit() else None

    async with AsyncSessionLocal() as session:
        u = await session.get(User, query.from_user.id)
        u_lang = u.language if u else "tr"

        total_acked = 1
        if alert_id:
            existing_ack = (await session.execute(
                select(EmergencyAck).where(EmergencyAck.alert_id == alert_id, EmergencyAck.user_telegram_id == query.from_user.id)
            )).scalar_one_or_none()
            if not existing_ack:
                session.add(EmergencyAck(alert_id=alert_id, user_telegram_id=query.from_user.id, acknowledged_at=datetime.utcnow()))
                await session.commit()
            
            total_acked = (await session.execute(
                select(func.count(EmergencyAck.id)).where(EmergencyAck.alert_id == alert_id)
            )).scalar() or 1

        tst = {
            "tr": "✅ Acil durum onayınız kaydedildi.",
            "ru": "✅ Ваше подтверждение принято.",
            "uz": "✅ Tasdig'ingiz qabul qilindi.",
            "en": "✅ Alert confirmed."
        }.get(u_lang, "✅ Confirmed.")
        await query.answer(tst, show_alert=True)
        ...
        # Gönderen yöneticiye anlık bilgi ilet: Kim onayladı ve toplam kaç kişi oldu!
        if admin_id:
            admin_user = await session.get(User, admin_id)
            adm_lang = admin_user.language if admin_user else "tr"
            u_name = u.full_name if u else (query.from_user.full_name or "Kullanıcı")
            u_role = {
                "teacher": {"tr": "Öğretmen", "ru": "Учитель", "uz": "O'qituvchi", "en": "Teacher"},
                "parent": {"tr": "Veli", "ru": "Родитель", "uz": "Ota-ona", "en": "Parent"},
                "student": {"tr": "Öğrenci", "ru": "Ученик", "uz": "O'quvchi", "en": "Student"}
            }.get(u.role if u else "", {}).get(adm_lang, "Kullanıcı")

            notif_txt = {
                "tr": f"🚨🔊 <b>ACİL ÇAĞRI ONAYLANDI</b>\n────────────────────\n👤 <b>{escape_html(u_name)}</b> ({u_role})\n⏱️ <i>Acil çağrıyı okudu ve onayladı.</i>\n\n📊 <b>Toplam Onaylayan:</b> <b>{total_acked}</b> kişi",
                "ru": f"🚨🔊 <b>ЭКСТРЕННЫЙ ВЫЗОВ ПОДТВЕРЖДЕН</b>\n────────────────────\n👤 <b>{escape_html(u_name)}</b> ({u_role})\n⏱️ <i>Прочитал и подтвердил экстренное сообщение.</i>\n\n📊 <b>Всего подтвердили:</b> <b>{total_acked}</b> чел.",
                "uz": f"🚨🔊 <b>SHOSHILINCH CHAQIRUV TASDIQLANDI</b>\n────────────────────\n👤 <b>{escape_html(u_name)}</b> ({u_role})\n⏱️ <i>Shoshilinch xabarni o'qidi va tasdiqladi.</i>\n\n📊 <b>Jami tasdiqlaganlar:</b> <b>{total_acked}</b> kishi",
                "en": f"🚨🔊 <b>EMERGENCY ALERT ACKNOWLEDGED</b>\n────────────────────\n👤 <b>{escape_html(u_name)}</b> ({u_role})\n⏱️ <i>Read and acknowledged the emergency alert.</i>\n\n📊 <b>Total Acknowledged:</b> <b>{total_acked}</b> users"
            }.get(adm_lang, "Alert acknowledged.")

            try:
                await query.bot.send_message(chat_id=admin_id, text=notif_txt, parse_mode="HTML")
            except Exception: pass
...
async def cb_view_proposal_attachment(query: CallbackQuery):
    ...
    async with AsyncSessionLocal() as session:
        ...
        att_desc = {"tr": "Ekli Belge / Tutanak", "ru": "Прикрепленный документ / Протокол", "uz": "Biriktirilgan hujjat / Bayonnoma", "en": "Attached Document / Minutes"}.get(lang, "Attached Document")
        caption = f"📎 <b>{escape_html(prop.title)}</b> — {att_desc}"
        try:
            ...
            err_att_fail = {"tr": f"⚠️ Belge gönderilemedi: {str(e)[:50]}", "ru": f"⚠️ Ошибка отправки документа: {str(e)[:50]}", "uz": f"⚠️ Hujjat yuborilmadi: {str(e)[:50]}", "en": f"⚠️ Could not send document: {str(e)[:50]}"}.get(lang, "⚠️ Error sending document.")
            await query.answer(err_att_fail, show_alert=True)
...
async def cb_open_proposal_for_vote(query: CallbackQuery):
    ...
    async with AsyncSessionLocal() as session:
        ...
        if not prop or prop.status != "active":
            await query.answer({"tr": "⚠️ Bu teklif artık aktif değil.", "ru": "⚠️ Это предложение уже не активно.", "uz": "⚠️ Ushbu taklif endi faol emas.", "en": "⚠️ This proposal is no longer active."}.get(lang, "⚠️ Inactive proposal."), show_alert=True)
            return
        ...
        lbl_deadline = {"tr": "Son Oy", "ru": "Срок", "uz": "Muddati", "en": "Deadline"}.get(lang, "Deadline")
        deadline_line = f"⏳ <b>{lbl_deadline}:</b> <code>{prop.deadline_at.strftime('%d.%m.%Y %H:%M')}</code>\n" if prop.deadline_at else ""
        anon_line = {"tr": "🕵️ <b>Gizli Oylama (Anonim)</b>\n", "ru": "🕵️ <b>Анонимное голосование</b>\n", "uz": "🕵️ <b>Yashirin ovoz berish</b>\n", "en": "🕵️ <b>Anonymous Ballot</b>\n"}.get(lang, "🕵️ <b>Anonymous</b>\n") if prop.is_anonymous else {"tr": "👁️ <b>Açık Oylama</b>\n", "ru": "👁️ <b>Открытое голосование</b>\n", "uz": "👁️ <b>Ochiq ovoz berish</b>\n", "en": "👁️ <b>Open Ballot</b>\n"}.get(lang, "👁️ <b>Open Ballot</b>\n")

        lbl_desc_p = {"tr": "Açıklama", "ru": "Описание", "uz": "Tavsif", "en": "Description"}.get(lang, "Description")
        lbl_cur_st = {"tr": "Mevcut Durum", "ru": "Текущий результат", "uz": "Joriy holat", "en": "Current Tally"}.get(lang, "Tally")
        lbl_vote_hint = {"tr": "Lütfen aşağıdaki butonlarla oyunuzu kullanınız:", "ru": "Пожалуйста, проголосуйте с помощью кнопок ниже:", "uz": "Iltimos, quyidagi tugmalar orqali ovoz bering:", "en": "Please cast your vote using the buttons below:"}.get(lang, "Cast your vote:")
        ...
        p_card = (
            ...
            f"📊 <b>{lbl_cur_st}:</b> ✅ {agree_cnt} | ❌ {disagree_cnt}\n"
            "────────────────────\n"
            f"<i>{lbl_vote_hint}</i>"
        )
...
async def cb_cat_tools_reports(event: Message | CallbackQuery, state: FSMContext | None = None):
    ...
    async with AsyncSessionLocal() as session:
        ...
        btn_risk = {"tr": "🚨 Erken Risk Radarı", "ru": "🚨 Радар риска", "uz": "🚨 Xavf radari", "en": "🚨 At-Risk Radar"}.get(lang, "🚨 Risk Radar")
        btn_watch = {"tr": "👁️ İzleme Masası", "ru": "👁️ Список наблюдения", "uz": "👁️ Kuzatuv stoli", "en": "👁️ Watchlist"}.get(lang, "👁️ Watchlist")
        btn_diseng = {"tr": "💤 İletişimsiz Veliler", "ru": "💤 Неактивные родители", "uz": "💤 Nofaol ota-onalar", "en": "💤 Disengaged Parents"}.get(lang, "💤 Disengaged Parents")
        btn_heat = {"tr": "📊 Isı Haritası", "ru": "📊 Тепловая карта", "uz": "📊 Faollik xaritasi", "en": "📊 Activity Heatmap"}.get(lang, "📊 Heatmap")
        ...
        buttons = [
            ...
            [InlineKeyboardButton(text=btn_risk, callback_data="adm:risk_radar"), InlineKeyboardButton(text=btn_watch, callback_data="adm:watchlist")],
            [InlineKeyboardButton(text=btn_diseng, callback_data="adm:disengaged_parents"), InlineKeyboardButton(text=btn_heat, callback_data="adm:heatmap")],
        ]
        buttons.append(get_nav_buttons(lang, back_callback="adm:dashboard"))
        await safe_edit_or_answer(event, desc, reply_markup=InlineKeyboardMarkup(inline_keyboard=buttons), parse_mode="HTML")
    if isinstance(event, CallbackQuery):
...
async def cb_admin_risk_radar(query: CallbackQuery):
    ...
    async with AsyncSessionLocal() as session:
        ...
        risk_list = []
        for s in students:
            att_records = (await session.execute(select(Attendance).where(Attendance.student_id == s.id))).scalars().all()
            absent_cnt = sum(1 for a in att_records if a.status == "absent")
            grades = (await session.execute(select(Grade).where(Grade.student_id == s.id))).scalars().all()
            avg_gpa = sum(g.score for g in grades) / len(grades) if grades else 100.0

            parents = (await session.execute(select(User).join(ParentStudent, ParentStudent.parent_telegram_id == User.telegram_id).where(ParentStudent.student_id == s.id))).scalars().all()

            risk_score = 0
            reasons = []
            
            lbl_days_abs = {"tr": f"{absent_cnt} gün devamsızlık", "ru": f"{absent_cnt} дн. пропусков", "uz": f"{absent_cnt} kun dars qoldirgan", "en": f"{absent_cnt} days absent"}.get(lang, f"{absent_cnt} days absent")
            lbl_low_gpa = {"tr": f"Düşük ortalama ({avg_gpa:.1f})", "ru": f"Низкий балл ({avg_gpa:.1f})", "uz": f"Past o'zlashtirish ({avg_gpa:.1f})", "en": f"Low GPA ({avg_gpa:.1f})"}.get(lang, f"Low GPA ({avg_gpa:.1f})")
            lbl_risk_gpa = {"tr": f"Riskli ortalama ({avg_gpa:.1f})", "ru": f"Рискованный балл ({avg_gpa:.1f})", "uz": f"Xavfli o'rtacha ({avg_gpa:.1f})", "en": f"Borderline GPA ({avg_gpa:.1f})"}.get(lang, f"Borderline GPA ({avg_gpa:.1f})")
            lbl_no_par = {"tr": "Bağlı veli yok", "ru": "Нет привязанных родителей", "uz": "Bog'langan ota-ona yo'q", "en": "No linked parents"}.get(lang, "No parents")
            lbl_inact_par = {"tr": "Veli ilgisiz (>30 gün)", "ru": "Родитель неактивен (>30 дн.)", "uz": "Ota-ona nofaol (>30 kun)", "en": "Parent inactive (>30 days)"}.get(lang, "Inactive parent")

            if absent_cnt >= 8:
                risk_score += 50
                reasons.append(lbl_days_abs)
            elif absent_cnt >= 5:
                risk_score += 30
                reasons.append(lbl_days_abs)
            ...
            if avg_gpa < 50:
                risk_score += 40
                reasons.append(lbl_low_gpa)
            elif avg_gpa < 65:
                risk_score += 25
                reasons.append(lbl_risk_gpa)

            if not parents:
                risk_score += 20
                reasons.append(lbl_no_par)
            elif all((not p.last_seen_at or (datetime.utcnow() - p.last_seen_at).days > 30) for p in parents):
                risk_score += 15
                reasons.append(lbl_inact_par)

            risk_score = min(100, risk_score)
            if risk_score >= 40:
                risk_list.append((s, risk_score, reasons))

        risk_list.sort(key=lambda x: x[1], reverse=True)

        hdr = {
            "tr": "🚨 <b>AKADEMİK RİSK VE ERKEN UYARI RADARI</b>",
            "ru": "🚨 <b>РАДАР АКАДЕМИЧЕСКОГО РИСКА</b>",
            "uz": "🚨 <b>AKADEMIK XAVF VA ERTA OGOHLANTIRISH RADARI</b>",
            "en": "🚨 <b>AT-RISK STUDENT EARLY WARNING RADAR</b>"
        }.get(lang, "🚨 <b>AT-RISK RADAR</b>")

        desc = {
            "tr": "Devamsızlık artışı, not düşüşü ve veli ilgisizliği analiz edilerek risk altındaki öğrenciler tespit edilmiştir:",
            "ru": "Ученики группы риска на основе пропусков, снижения успеваемости и пассивности родителей:",
            "uz": "Davomat, baholar pasayishi va ota-onalar faolligi asosida xavf guruhidagi o'quvchilar:",
            "en": "At-risk students identified through attendance spikes, grade drops, and parent inactivity:"
        }.get(lang, "At-risk students:")
        ...
        if not risk_list:
            text = (
                f"{hdr}\n"
                "────────────────────\n"
                f"<i>" + {"tr": "Tebrikler! Şu anda okulda yüksek risk altında öğrenci bulunmamaktadır.", "ru": "В школе нет учеников группы высокого риска.", "uz": "Hozirda yuqori xavf guruhidagi o'quvchilar yo'q.", "en": "No students currently in the high-risk category."}.get(lang, "No at-risk students.") + "</i>\n"
                "────────────────────"
            )
        else:
            lines = [f"{hdr}\n────────────────────\n{desc}\n"]
            lbl_r_word = {"tr": "Risk", "ru": "Риск", "uz": "Xavf", "en": "Risk"}.get(lang, "Risk")
            for s, r_score, r_reasons in risk_list[:8]:
                badge = "🔴" if r_score >= 70 else "🟡"
                r_txt = ", ".join(r_reasons[:2])
                lines.append(f"{badge} <b>{escape_html(s.full_name)}</b> ({escape_html(s.class_name)}) • <b>{lbl_r_word}: %{r_score}</b>\n   ↳ <i>{r_txt}</i>")
                btn_txt = f"{badge} {s.full_name} (%{r_score})"
                buttons.append([InlineKeyboardButton(text=btn_txt, callback_data=f"adm:st_card:{s.id}:0")])
            lines.append("────────────────────")
            text = "\n".join(lines)
...
@router.callback_query(F.data == "adm:disengaged_parents")
async def cb_admin_disengaged_parents(query: CallbackQuery):
    user_id = query.from_user.id
    async with AsyncSessionLocal() as session:
        user = await session.get(User, user_id)
        lang = user.language if user else "tr"
        ...
        parents = (await session.execute(
            select(User)
            .where(User.role == "parent", (User.last_seen_at == None) | (User.last_seen_at < thirty_days_ago))
            .limit(10)
        )).scalars().all()

        hdr = {
            "tr": "💤 <b>İLETİŞİMSİZ / PASİF VELİ İNDEKSİ</b>",
            "ru": "💤 <b>СПИСОК НЕАКТИВНЫХ РОДИТЕЛЕЙ</b>",
            "uz": "💤 <b>NOFAOL / ALOQASIZ OTA-ONALAR</b>",
            "en": "💤 <b>DISENGAGED PARENTS INDEX</b>"
        }.get(lang, "💤 <b>DISENGAGED PARENTS</b>")
        ...
        if not parents:
            text = (
                f"{hdr}\n"
                "────────────────────\n"
                f"<i>" + {"tr": "Tüm veliler son 30 gün içinde sistemle aktif iletişim kurmuştur.", "ru": "Все родители проявляли активность за последние 30 дней.", "uz": "Barcha ota-onalar so'nggi 30 kunda faol bo'lgan.", "en": "All parents have interacted within the last 30 days."}.get(lang, "All parents active.") + "</i>\n"
                "────────────────────"
            )
        else:
            lines = [f"{hdr}\n────────────────────\n" + {"tr": "Son 30 gündür botu açmayan ve bildirimleri kontrol etmeyen veliler:", "ru": "Родители без активности более 30 дней:", "uz": "30 kundan ortiq faollik ko'rsatmagan ota-onalar:", "en": "Parents inactive for more than 30 days:"}.get(lang, "Inactive parents:") + "\n"]
            val_no_kids = {"tr": "Öğrenci Bağı Yok", "ru": "Нет учеников", "uz": "O'quvchi biriktirilmagan", "en": "No Linked Students"}.get(lang, "-")
            val_never = {"tr": "Hiç girmedi", "ru": "Никогда", "uz": "Hech kirmagan", "en": "Never"}.get(lang, "-")
            lbl_p_def = {"tr": "Veli", "ru": "Родитель", "uz": "Ota-ona", "en": "Parent"}.get(lang, "Parent")
            lbl_st_tag = {"tr": "Öğrenci", "ru": "Ученик", "uz": "O'quvchi", "en": "Student"}.get(lang, "Student")
            lbl_inact_tag = {"tr": "Pasif", "ru": "Неактивен", "uz": "Nofaol", "en": "Inactive"}.get(lang, "Inactive")
            lbl_d_unit = {"tr": "gün", "ru": "дн.", "uz": "kun", "en": "days"}.get(lang, "d")
            ...
            for p in parents:
                ...
                k_info = ", ".join([f"{k.full_name} ({k.class_name})" for k in kids]) if kids else val_no_kids
                days_absent = f"{(now - p.last_seen_at).days} {lbl_d_unit}" if p.last_seen_at else val_never
                p_display = p.full_name or lbl_p_def
                lines.append(f"⚪ <b>{escape_html(p_display)}</b> (<code>{p.telegram_id}</code>)\n   ↳ {lbl_st_tag}: {escape_html(k_info)} • {lbl_inact_tag}: {days_absent}")
                buttons.append([InlineKeyboardButton(text=f"👤 {p_display} ({days_absent})", callback_data=f"adm:user_card:{p.telegram_id}:0")])
            lines.append("────────────────────")
            text = "\n".join(lines)
...
@router.callback_query(F.data == "adm:heatmap")
async def cb_admin_activity_heatmap(query: CallbackQuery):
    user_id = query.from_user.id
    async with AsyncSessionLocal() as session:
        user = await session.get(User, user_id)
        lang = user.language if user else "tr"
        ...
        users = (await session.execute(select(User.last_seen_at).where(User.last_seen_at.isnot(None)))).scalars().all()
        bins = {"sabah": 0, "ogle": 0, "ikindi": 0, "aksam": 0, "gece": 0}
        for dt in users:
            h = (dt.hour + TIMEZONE_OFFSET) % 24
            if 8 <= h < 12: bins["sabah"] += 1
            elif 12 <= h < 14: bins["ogle"] += 1
            elif 14 <= h < 18: bins["ikindi"] += 1
            elif 18 <= h < 22: bins["aksam"] += 1
            else: bins["gece"] += 1

        total = max(1, sum(bins.values()))
        max_v = max(1, max(bins.values()))

        def get_bar(val):
            return render_progress_bar(int(round(val * 10 / max_v)), 10)

        hdr = {
            "tr": "📊 <b>CANLI KULLANICI ETKİLEŞİM ISI HARİTASI</b>",
            "ru": "📊 <b>ТЕПЛОВАЯ КАРТА АКТИВНОСТИ ПОЛЬЗОВАТЕЛЕЙ</b>",
            "uz": "📊 <b>FOYDALANUVCHILAR FAOLLIGI ISSIQLIK XARITASI</b>",
            "en": "📊 <b>LIVE USER ACTIVITY HEATMAP</b>"
        }.get(lang, "📊 <b>ACTIVITY HEATMAP</b>")

        labels = {
            "tr": ("🌅 Sabah (08:00 - 12:00)", "☀️ Öğle (12:00 - 14:00)", "🌇 İkindi (14:00 - 18:00)", "🌙 Akşam (18:00 - 22:00)", "🦉 Gece (22:00 - 08:00)"),
            "ru": ("🌅 Утро (08:00 - 12:00)", "☀️ Обед (12:00 - 14:00)", "🌇 День (14:00 - 18:00)", "🌙 Вечер (18:00 - 22:00)", "🦉 Ночь (22:00 - 08:00)"),
            "uz": ("🌅 Tong (08:00 - 12:00)", "☀️ Tush (12:00 - 14:00)", "🌇 Peshin (14:00 - 18:00)", "🌙 Oqshom (18:00 - 22:00)", "🦉 Tun (22:00 - 08:00)"),
            "en": ("🌅 Morning (08:00 - 12:00)", "☀️ Noon (12:00 - 14:00)", "🌇 Afternoon (14:00 - 18:00)", "🌙 Evening (18:00 - 22:00)", "🦉 Night (22:00 - 08:00)")
        }.get(lang, ("Morning", "Noon", "Afternoon", "Evening", "Night"))

        pct_s = round(bins["sabah"] * 100 / total, 1)
        pct_o = round(bins["ogle"] * 100 / total, 1)
        pct_i = round(bins["ikindi"] * 100 / total, 1)
        pct_a = round(bins["aksam"] * 100 / total, 1)
        pct_g = round(bins["gece"] * 100 / total, 1)

        lbl_ops = {"tr": "işlem", "ru": "действий", "uz": "amal", "en": "actions"}.get(lang, "actions")
        lbl_tot_act = {"tr": "Toplam Kayıtlı Aktivite", "ru": "Всего зафиксировано активности", "uz": "Jami qayd etilgan faollik", "en": "Total Recorded Activity"}.get(lang, "Total Activity")

        text = (
            f"{hdr}\n"
            "────────────────────\n"
            f"{labels[0]}: <b>%{pct_s}</b>\n<code>{get_bar(bins['sabah'])}</code> ({bins['sabah']} {lbl_ops})\n\n"
            f"{labels[1]}: <b>%{pct_o}</b>\n<code>{get_bar(bins['ogle'])}</code> ({bins['ogle']} {lbl_ops})\n\n"
            f"{labels[2]}: <b>%{pct_i}</b>\n<code>{get_bar(bins['ikindi'])}</code> ({bins['ikindi']} {lbl_ops})\n\n"
            f"{labels[3]}: <b>%{pct_a}</b>\n<code>{get_bar(bins['aksam'])}</code> ({bins['aksam']} {lbl_ops})\n\n"
            f"{labels[4]}: <b>%{pct_g}</b>\n<code>{get_bar(bins['gece'])}</code> ({bins['gece']} {lbl_ops})\n"
            "────────────────────\n"
            f"👥 <b>{lbl_tot_act}:</b> {sum(bins.values())} {lbl_ops}\n"
            "────────────────────"
        )
        buttons = [get_nav_buttons(lang, back_callback="adm:cat_tools_reports")]
        await safe_edit_or_answer(query, text, reply_markup=InlineKeyboardMarkup(inline_keyboard=buttons), parse_mode="HTML")
...
@router.callback_query(F.data == "adm:watchlist")
async def cb_admin_watchlist_hub(query: CallbackQuery):
    user_id = query.from_user.id
    async with AsyncSessionLocal() as session:
        user = await session.get(User, user_id)
        lang = user.language if user else "tr"
        ...
        stmt = select(User).where(
            (User.is_blacklisted == True) |
            (User.night_activity_count > 0) |
            (User.download_count >= 5) |
            (User.is_bot_blocked == True)
        ).limit(15)
        suspicious = (await session.execute(stmt)).scalars().all()

        hdr = {
            "tr": "👁️ <b>ŞÜPHELİ HESAPLAR İZLEME MASASI (WATCHLIST)</b>",
            "ru": "👁️ <b>СПИСОК НАБЛЮДЕНИЯ ЗА ПОДОЗРИТЕЛЬНЫМИ</b>",
            "uz": "👁️ <b>GUMONLI HISOB-KITOBLAR KUZATUV STOLI</b>",
            "en": "👁️ <b>SECURITY WATCHLIST HUB</b>"
        }.get(lang, "👁️ <b>WATCHLIST</b>")
        ...
        if not suspicious:
            text = (
                f"{hdr}\n"
                "────────────────────\n"
                f"<i>" + {"tr": "Şu anda şüpheli hareket sergileyen veya radar listesine giren kullanıcı bulunmamaktadır.", "ru": "В настоящее время нет подозрительных пользователей в списке.", "uz": "Hozirda shubhali foydalanuvchilar mavjud emas.", "en": "No suspicious users currently on the watchlist."}.get(lang, "No suspicious users.") + "</i>\n"
                "────────────────────"
            )
        else:
            lines = [f"{hdr}\n────────────────────\n" + {"tr": "Güvenlik radarına takılan (gece işlemi, engelleme, yüksek indirme) hesaplar:", "ru": "Пользователи, попавшие под радар безопасности:", "uz": "Xavfsizlik radariga tushgan foydalanuvchilar:", "en": "Accounts flagged by security radar:"}.get(lang, "Flagged accounts:") + "\n"]
            lbl_banned = {"tr": "Yasaklı", "ru": "Заблокирован", "uz": "Bloklangan", "en": "Banned"}.get(lang, "Banned")
            lbl_blocked_bot = {"tr": "Botu Sildi", "ru": "Удалил бота", "uz": "Botni o'chirdi", "en": "Removed Bot"}.get(lang, "Blocked Bot")
            lbl_night = {"tr": "Gece", "ru": "Ночь", "uz": "Tun", "en": "Night"}.get(lang, "Night")
            lbl_downloads = {"tr": "İndirme", "ru": "Загрузок", "uz": "Yuklab olish", "en": "Downloads"}.get(lang, "Downloads")
            lbl_u_def = {"tr": "Kullanıcı", "ru": "Пользователь", "uz": "Foydalanuvchi", "en": "User"}.get(lang, "User")

            for u in suspicious:
                flags = []
                if u.is_blacklisted: flags.append(lbl_banned)
                if u.is_bot_blocked: flags.append(lbl_blocked_bot)
                if u.night_activity_count > 0: flags.append(f"{u.night_activity_count} {lbl_night}")
                if u.download_count >= 5: flags.append(f"{u.download_count} {lbl_downloads}")
                f_str = ", ".join(flags)

                badge = "🔴" if u.is_blacklisted or u.is_bot_blocked else "🟡"
                u_name = u.full_name or lbl_u_def
                lines.append(f"{badge} <b>{escape_html(u_name)}</b> (<code>{u.telegram_id}</code>)\n   ↳ <i>{f_str}</i>")
                buttons.append([InlineKeyboardButton(text=f"{badge} {u_name} ({f_str[:16]})", callback_data=f"adm:user_360:{u.telegram_id}:0")])
            lines.append("────────────────────")
            text = "\n".join(lines)
    ...
    async with AsyncSessionLocal() as session:
        ...
        if tab == "notices":
            ...
            if not sched or not sched.schedule_data:
                tab_content = f"<i>{cls_name} " + {"tr": "sınıfına ait haftalık ders programı henüz yüklenmemiştir.", "ru": "расписание уроков еще не добавлено.", "uz": "sinfi uchun dars jadvali hali kiritilmagan.", "en": "class schedule has not been uploaded yet."}.get(lang, "schedule not added.") + "</i>"
            else:
                sch_title_loc = {"tr": f"<b>{cls_name} Haftalık Ders Programı:</b>", "ru": f"<b>Расписание уроков класса {cls_name}:</b>", "uz": f"<b>{cls_name} sinfi haftalik dars jadvali:</b>", "en": f"<b>Class {cls_name} Weekly Timetable:</b>"}.get(lang, f"<b>Class {cls_name} Timetable:</b>")
                tab_content = f"{sch_title_loc}\n\n{escape_html(sched.schedule_data)}"
        elif tab == "cafe":
            ...
            if not menu:
                tab_content = "<i>" + {"tr": "Bugün için kayıtlı yemekhane menüsü bulunmamaktadır.", "ru": "На сегодня меню в столовой отсутствует.", "uz": "Bugun uchun oshxona menyusi kiritilmagan.", "en": "No cafeteria menu registered for today."}.get(lang, "No menu.") + "</i>"
            else:
                cafe_title_loc = {"tr": f"🍽️ <b>Günün Menüsü ({today_str}):</b>", "ru": f"🍽️ <b>Меню столовой на сегодня ({today_str}):</b>", "uz": f"🍽️ <b>Kunlik oshxona menyusi ({today_str}):</b>", "en": f"🍽️ <b>Today's Cafeteria Menu ({today_str}):</b>"}.get(lang, f"🍽️ <b>Menu ({today_str}):</b>")
                cal_lbl = {"tr": "Kalori", "ru": "Калорийность", "uz": "Kaloriya", "en": "Calories"}.get(lang, "Calories")
                none_cal = {"tr": "Belirtilmedi", "ru": "Не указана", "uz": "Ko'rsatilmadi", "en": "Not specified"}.get(lang, "-")
                tab_content = f"{cafe_title_loc}\n\n{escape_html(menu.items_list)}\n\n🔥 <i>{cal_lbl}: {menu.calories or none_cal} kcal</i>"
        elif tab == "exams":
            tab_title = get_text("btn_exam_schedule", lang)
            exams = (await session.execute(select(ExamSchedule).where(ExamSchedule.class_name == cls_name).order_by(ExamSchedule.exam_date))).scalars().all()
            if not exams:
                tab_content = f"<i>{cls_name} " + {"tr": "sınıfı için planlanmış sınav takvimi bulunmamaktadır.", "ru": "расписание экзаменов для класса отсутствует.", "uz": "sinfi uchun imtihonlar jadvali kiritilmagan.", "en": "no exam schedule scheduled for this class."}.get(lang, "no exams.") + "</i>"
            else:
                exam_hdr_loc = {"tr": f"📝 <b>{cls_name} Sınav Takvimi:</b>", "ru": f"📝 <b>Расписание экзаменов класса {cls_name}:</b>", "uz": f"📝 <b>{cls_name} sinfi imtihonlar jadvali:</b>", "en": f"📝 <b>Class {cls_name} Exam Schedule:</b>"}.get(lang, f"📝 <b>Class {cls_name} Exams:</b>")
                lines = [f"{exam_hdr_loc}\n"]
                for e in exams:
        ...
        main_card = (
            ...
            f"{tab_content}\n"
            "────────────────────\n"
            "<i>" + {"tr": "Yukarıdaki sekmelere dokunarak bölümler arasında anında geçiş yapabilirsiniz.", "ru": "Нажимайте на вкладки выше для быстрого переключения разделов.", "uz": "Bo'limlar o'rtasida o'tish uchun yuqoridagi tugmalardan foydalaning.", "en": "Tap the tabs above to switch between sections."}.get(lang, "") + "</i>"
        )
...
async def cb_admin_scan_bot_blocks(query: CallbackQuery):
    await query.answer({"tr": "🔍 Kullanıcı bağlantı kontrolleri taranıyor, lütfen bekleyiniz...", "ru": "🔍 Сканирование доступности пользователей, пожалуйста подождите...", "uz": "🔍 Foydalanuvchilar bilan aloqa tekshirilmoqda, kuting...", "en": "🔍 Scanning user connectivity, please wait..."}.get(lang, "Scanning..."), show_alert=False)
    async with AsyncSessionLocal() as session:
        user = await session.get(User, query.from_user.id)
        lang = user.language if user else "tr"
        if not is_admin_user(user, query.from_user.id):
...
async def process_admin_rename_class_name(message: Message, state: FSMContext):
    ...
    async with AsyncSessionLocal() as session:
        ...
        if not raw_name or len(raw_name) < 1 or len(raw_name) > 15 or not re.match(r'^[A-Z0-9\-_]+$', raw_name):
            err_invalid = {
                "tr": "⚠️ Geçersiz sınıf adı! Lütfen harf ve rakamlardan oluşan geçerli bir sınıf adı giriniz (Örn: <code>10-A</code>):",
                "ru": "⚠️ Некорректное название класса! Введите правильное имя из букв и цифр (Напр: <code>10-A</code>):",
                "uz": "⚠️ Noto'g'ri sinf nomi! Harf va raqamlardan iborat to'g'ri nom kiriting (Masalan: <code>10-A</code>):",
                "en": "⚠️ Invalid class name! Please enter a valid alphanumeric name (e.g. <code>10-A</code>):"
            }.get(lang, "⚠️ Invalid class name:")
            await message.answer(err_invalid, parse_mode="HTML")
            return
        ...
        if (existing and existing.name != old_name) or (st_existing and st_existing.class_name != old_name):
            err_dup = {
                "tr": f"⚠️ <b>{raw_name}</b> sınıf adı sistemde zaten kayıtlı! Lütfen farklı bir isim giriniz:",
                "ru": f"⚠️ Класс <b>{raw_name}</b> уже зарегистрирован! Пожалуйста, укажите другое имя:",
                "uz": f"⚠️ <b>{raw_name}</b> sinf nomi tizimda allaqachon mavjud! Boshqa nom kiriting:",
                "en": f"⚠️ Class <b>{raw_name}</b> already exists! Please enter a different name:"
            }.get(lang, f"⚠️ Class {raw_name} already exists!")
            await message.answer(err_dup, parse_mode="HTML")
            return
...
async def cb_admin_graduates_archive(query: CallbackQuery):
    ...
    async with AsyncSessionLocal() as session:
        ...
        bc_ga = {"tr": "🏠 Ana Menü ➔ 👥 Kadro ➔ 🎓 Mezunlar Arşivi", "ru": "🏠 Главное меню ➔ 👥 Кадры ➔ 🎓 Архив выпускников", "uz": "🏠 Asosiy menyu ➔ 👥 Kadro ➔ 🎓 Bitiruvchilar arxivi", "en": "🏠 Main Menu ➔ 👥 Staff ➔ 🎓 Alumni Archive"}.get(lang, "🎓 Alumni Archive")
        title_ga = {"tr": f"🎓 <b>MEZUNLAR ARŞİVİ</b> (Toplam: <b>{total}</b> Mezun)", "ru": f"🎓 <b>АРХИВ ВЫПУСКНИКОВ</b> (Всего: <b>{total}</b> вып.)", "uz": f"🎓 <b>BITIRUVCHILAR ARXIVI</b> (Jami: <b>{total}</b> bitiruvchi)", "en": f"🎓 <b>GRADUATES & ALUMNI ARCHIVE</b> (Total: <b>{total}</b> Graduates)"}.get(lang, f"🎓 <b>GRADUATES ARCHIVE</b> ({total})")

        lbl_year = {"tr": "Yıl", "ru": "Год", "uz": "Yil", "en": "Year"}.get(lang, "Year")
        lbl_cls = {"tr": "Sınıf", "ru": "Класс", "uz": "Sinf", "en": "Class"}.get(lang, "Class")
        lbl_no = {"tr": "No", "ru": "№", "uz": "№", "en": "No"}.get(lang, "No")

        buttons = []
        if total == 0:
            content = "<i>" + {"tr": "Sistemde henüz arşivlenmiş mezun öğrenci bulunmamaktadır.", "ru": "В архиве пока нет выпускников.", "uz": "Arxivda hali bitiruvchilar mavjud emas.", "en": "No archived graduates found in the system."}.get(lang, "No graduates.") + "</i>"
        else:
            paged = all_grads[page * PAGE_SIZE : (page + 1) * PAGE_SIZE]
            lines = []
            for idx, g in enumerate(paged, start=page * PAGE_SIZE + 1):
                # Clean, mobile-friendly card layout that never overflows screen
                lines.append(
                    f"<b>{idx}. {escape_html(g.full_name)}</b>\n"
                    f"   • {lbl_cls}: <code>{escape_html(g.graduated_class)}</code> | {lbl_no}: <code>{escape_html(g.student_number)}</code> | {lbl_year}: <b>{g.graduation_year}</b>"
                )
            content = "\n".join(lines)
            ...
            # Excel download button for graduates
            btn_dl_excel = {"tr": "📥 Mezunlar Listesi (Excel)", "ru": "📥 Список выпускников (Excel)", "uz": "📥 Bitiruvchilar ro'yxati (Excel)", "en": "📥 Export Alumni List (Excel)"}.get(lang, "📥 Export Alumni (Excel)")
            buttons.append([InlineKeyboardButton(text=btn_dl_excel, callback_data="adm:export_graduates_excel")])

        b_promo_hub = {"tr": "🎓 Sınıf Terfi Masası", "ru": "🎓 Перевод классов", "uz": "🎓 Sinf ko'chirish", "en": "🎓 Class Promotion"}.get(lang, "🎓 Promotion")
        buttons.append([InlineKeyboardButton(text=b_promo_hub, callback_data="adm:class_promotion_init")])
        buttons.append(get_nav_buttons(lang, back_callback="adm:cat_staff"))

        footer_ga = {"tr": "ℹ️ <i>Sınıf terfi işlemlerinde son sınıftan mezun olan öğrenciler otomatik olarak buraya arşivlenir.</i>", "ru": "ℹ️ <i>При переводе классов выпускники старших классов автоматически архивируются в этом разделе.</i>", "uz": "ℹ️ <i>Sinf ko'chirish amallarida bitiruvchilar avtomatik ravishda ushbu arxivga kiritiladi.</i>", "en": "ℹ️ <i>Graduating students are automatically archived into this directory.</i>"}.get(lang, "")
...
@router.callback_query(F.data == "adm:export_graduates_excel")
async def cb_admin_export_graduates_excel(query: CallbackQuery):
    async with AsyncSessionLocal() as session:
        user = await session.get(User, query.from_user.id)
        lang = user.language if user else "tr"
        if not is_admin_user(user, query.from_user.id):
        ...
        all_grads = (await session.execute(select(Graduate).order_by(Graduate.graduation_year.desc(), Graduate.full_name))).scalars().all()
        if not all_grads:
            no_g_tst = {"tr": "İndirilecek mezun kaydı bulunamadı.", "ru": "Нет записей выпускников для скачивания.", "uz": "Yuklab olish uchun bitiruvchilar topilmadi.", "en": "No graduate records found to download."}.get(lang, "No graduates found.")
            await query.answer(no_g_tst, show_alert=True)
            return
        ...
        headers = [
            {"tr": "ID", "ru": "ID", "uz": "ID", "en": "ID"}.get(lang, "ID"),
            {"tr": "Ad Soyad", "ru": "ФИО", "uz": "F.I.O", "en": "Full Name"}.get(lang, "Name"),
            {"tr": "Mezun Olduğu Sınıf", "ru": "Класс выпуска", "uz": "Bitirgan sinfi", "en": "Graduated Class"}.get(lang, "Class"),
            {"tr": "Okul Numarası", "ru": "Номер в школе", "uz": "Maktab raqami", "en": "Student Number"}.get(lang, "Number"),
            {"tr": "Mezuniyet Yılı", "ru": "Год выпуска", "uz": "Bitirgan yili", "en": "Graduation Year"}.get(lang, "Year"),
            {"tr": "Kayıt Tarihi", "ru": "Дата архивации", "uz": "Arxivlangan sana", "en": "Archived Date"}.get(lang, "Date")
        ]
        ws.append(headers)
        ...
        for g in all_grads:
            d_str = (g.created_at + timedelta(hours=TIMEZONE_OFFSET)).strftime("%d.%m.%Y %H:%M") if g.created_at else "-"
            ws.append([g.id, g.full_name, g.graduated_class, g.student_number, g.graduation_year, d_str])
        ...
        file = BufferedInputFile(bio.read(), filename=f"Mezunlar_Arsivi_{datetime.utcnow().strftime('%Y%m%d')}.xlsx")
        cap = {
            "tr": f"🎓 <b>Mezunlar Arşivi Resmi Excel Listesi</b> (Toplam {len(all_grads)} Mezun)",
            "ru": f"🎓 <b>Официальный список выпускников (Excel)</b> (Всего: {len(all_grads)} вып.)",
            "uz": f"🎓 <b>Bitiruvchilar rasmiy ro'yxati (Excel)</b> (Jami: {len(all_grads)} ta)",
            "en": f"🎓 <b>Official Graduates & Alumni Excel Sheet</b> (Total: {len(all_grads)} Graduates)"
        }.get(lang, "Graduates Excel")

        target_chat_id = query.message.chat.id if query.message else query.from_user.id
        bot_obj = query.bot if getattr(query, 'bot', None) else (query.message.bot if query.message else bot)
        await bot_obj.send_document(chat_id=target_chat_id, document=file, caption=cap, parse_mode="HTML")
        bio.close()
        await query.answer({"tr": "✅ Excel aktarıldı!", "ru": "✅ Файл отправлен!", "uz": "✅ Fayl yuborildi!", "en": "✅ File sent!"}.get(lang, "✅ Sent!"), show_alert=False)
...
async def cb_admin_remind_votes(query: CallbackQuery):
    ...
    async with AsyncSessionLocal() as session:
        ...
        prop = await session.get(Proposal, prop_id)
        if not prop or prop.status != "active":
            await query.answer({"tr": "⚠️ Bu oylama aktif değil!", "ru": "⚠️ Это голосование уже не активно!", "uz": "⚠️ Ushbu ovoz berish faol emas!", "en": "⚠️ This ballot is not active!"}.get(lang, "⚠️ Inactive ballot!"), show_alert=True)
            return
        ...
        sent_c = 0
        u_lang_map = {u.telegram_id: (u.language or "tr") for u in (await session.execute(select(User).where(User.telegram_id.in_(non_voters)))).scalars().all()}
        for uid in non_voters:
            u_l = u_lang_map.get(uid, "tr")
            remind_txt = {
                "tr": (
                ...
                "ru": (
                    f"🗳️ <b>НАПОМИНАНИЕ О ГОЛОСОВАНИИ</b>\n"
                    "────────────────────\n"
                    f"У вас есть непроголосованное обсуждение / предложение:\n\n"
                    f"📌 <b>{escape_html(prop.title)}</b>\n\n"
                    "<i>Пожалуйста, нажмите кнопку ниже, чтобы выразить свое мнение.</i>"
                ),
                "uz": (
                    f"🗳️ <b>OVOZ BERISH ESLATMASI</b>\n"
                    "────────────────────\n"
                    f"Siz hali ovoz bermagan taklif / muhokama mavjud:\n\n"
                    f"📌 <b>{escape_html(prop.title)}</b>\n\n"
                    "<i>Iltimos, quyidagi tugmani bosib o'z fikringizni bildiring.</i>"
                ),
                "en": (
                    f"🗳️ <b>BALLOT REMINDER</b>\n"
                    "────────────────────\n"
                    f"You have a pending ballot / proposal to vote on:\n\n"
                    f"📌 <b>{escape_html(prop.title)}</b>\n\n"
                    "<i>Please tap the button below to cast your vote.</i>"
                )
            }.get(u_l, f"Vote reminder: {prop.title}")

            b_open_v = {"tr": "🗳️ Oylamayı Aç ve Oy Ver", "ru": "🗳️ Открыть и проголосовать", "uz": "🗳️ Ovoz berishga o'tish", "en": "🗳️ Open & Vote"}.get(u_l, "🗳️ Open & Vote")
...
dp = Dispatcher()
dp.callback_query.outer_middleware(AdvancedBotSecurityMiddleware())
dp.callback_query.outer_middleware(AutoCallbackAnswerMiddleware())
dp.message.outer_middleware(AdvancedBotSecurityMiddleware())
...
async def global_error_shield(event, exception):
    ...
    # Self-Healing: Kullanıcı botu sildiyse veya engellediyse veritabanını anında onar
    if "blocked by the user" in err_lower or "user is deactivated" in err_lower:
        u_id = None
        ...
        if u_id:
            try:
                async with AsyncSessionLocal() as session:
                    u_db = await session.get(User, u_id)
                    if u_db and not u_db.is_bot_blocked:
                        u_db.is_bot_blocked = True
                        u_db.blocked_bot_at = datetime.utcnow()
                        await session.commit()
                        logger.info(f"Self-Healing: Kullanıcı {u_id} veritabanında engelli olarak otomatik mühürlendi.")
            except Exception:
                pass
        return True

    # Self-Healing: Silinecek mesaj bulunamadıysa yerel önbellek kayıtlarını onar
    if "message to delete not found" in err_lower or "message can't be deleted" in err_lower:
        c_id = getattr(getattr(event, "chat", None), "id", None)
        if c_id:
            LAST_MENU_MSG_ID.pop(c_id, None)
            if c_id in ACTIVE_CHAT_MESSAGES:
                ACTIVE_CHAT_MESSAGES[c_id].clear()
        return True
    ...
    if any(ig in err_lower for ig in ignorable):
        return True
...
async def background_health_and_ttl_cleanup_loop():
    while True:
        try:
            ...
            # Self-Healing: Bellek Sızıntısı Kalkanı (Bounded Dictionary Auto-Pruning)
            prune_bounded_cache(USER_REQUEST_LOG, 1000)
            prune_bounded_cache(USER_LAST_CLICK, 1000)
            prune_bounded_cache(CALLBACK_DEBOUNCE, 1000)
            prune_bounded_cache(USER_CLIENT_IP_MAP, 1000)
            prune_bounded_cache(ACTIVE_CHAT_MESSAGES, 500)
            prune_bounded_cache(LAST_MENU_MSG_ID, 500)
            prune_bounded_cache(KEYBOARD_ANCHOR_MSG_ID, 500)
...
async def lifespan(app: FastAPI):
    ...
    # 4/6: Self-Healing Watchdog Supervisor ile Arka Plan Görevleri Canlı Koruma Altında
    global TASK_SUPERVISOR_INSTANCE
    task_supervisor = TaskSupervisor(bot_obj=bot)
    TASK_SUPERVISOR_INSTANCE = task_supervisor
    task_supervisor.register("attendance_worker", background_attendance_loop)
    task_supervisor.register("keep_alive_pinger", background_keep_alive_pinger)
    task_supervisor.register("evening_briefing", background_evening_briefing_loop)
    task_supervisor.register("morning_briefing", background_morning_briefing_loop)
    task_supervisor.register("friday_backup", background_friday_backup_loop)
    task_supervisor.register("health_and_ttl_cleanup", background_health_and_ttl_cleanup_loop)
    task_supervisor.register("profile_telemetry_poller", background_profile_telemetry_poller, bot)
    supervisor_task = asyncio.create_task(task_supervisor.run_supervision_loop(), name="task_supervisor_master")

    print("--> [4/6] Self-Healing Task Supervisor & Arka plan işçileri aktif.")
    print("--> [5/6] Cuma 18:00 Otomatik Yönetici Excel Veri Yedeği aktif.")
    print("--> [6/6] SİSTEM CANLI VE TÜM SELF-HEALING KALKANLARI HAZIR.")
    print("=" * 60)
    yield
    if polling_task:
        polling_task.cancel()
...
async def telegram_webhook(request: Request):
    ...
    try:
        ...
        # 4.1. Kaynak Ağ Denetimi (Client IP Extraction)
        client_ip = None
        ...
        if u_id and client_ip:
            USER_CLIENT_IP_MAP[u_id] = client_ip
        if u_id:
            import time
            now_t = time.time()
