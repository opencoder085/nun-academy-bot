...
async def cb_admin_user_360(query: CallbackQuery):
    ...
    async with AsyncSessionLocal() as session:
        ...
        none_txt = {"tr": "Yok", "ru": "Нет", "uz": "Yo'q", "en": "None"}.get(lang, "None")
        usr_display = f"@{target_u.username}" if target_u.username else none_txt
        ...
        # 1. Canlı Durum & Son Görülme (100% Dinamik)
        ...
        # 2. İtaat & Tepki Hızı (100% Dinamik)
        ...
        if lats:
            ...
            if avg_m <= 5:
                itaat_txt = {"tr": f"~{avg_m} dk (A Sınıfı - Anında Onay)", "ru": f"~{avg_m} мин (Класс A - Мгновенно)", "uz": f"~{avg_m} daq (A Sinf - Tezkor)", "en": f"~{avg_m} min (Class A - Instant)"}.get(lang, f"~{avg_m} min")
            elif avg_m <= 30:
                itaat_txt = {"tr": f"~{avg_m} dk (B Sınıfı - Standart)", "ru": f"~{avg_m} мин (Класс B - Стандартно)", "uz": f"~{avg_m} daq (B Sinf - Standart)", "en": f"~{avg_m} min (Class B - Standard)"}.get(lang, f"~{avg_m} min")
            else:
            ...
            itaat_txt = {"tr": "Henüz onay kaydı yok", "ru": "Нет записей подтверждений", "uz": "Tasdiqlash yozuvlari yo'q", "en": "No confirmation records"}.get(lang, "No records")

        # 3. Güvenlik & Tehdit Skoru (100% Dinamik)
        score = 100
        if target_u.is_blacklisted:
            score = 0
        else:
        ...
        if score >= 85:
            score_badge = "🛡️"
            score_txt = {"tr": f"%{score} (A Sınıfı - Çok Güvenilir)", "ru": f"{score}% (Класс A - Высокое доверие)", "uz": f"{score}% (A Sinf - Yuqori ishonch)", "en": f"{score}% (Class A - Highly Trusted)"}.get(lang, f"{score}%")
        elif score >= 60:
            score_badge = "🟡"
            score_txt = {"tr": f"%{score} (B Sınıfı - Standart)", "ru": f"{score}% (Класс B - Стандартный)", "uz": f"{score}% (B Sinf - Standart)", "en": f"{score}% (Class B - Standard)"}.get(lang, f"{score}%")
        elif score >= 40:
            score_badge = "🟠"
            score_txt = {"tr": f"%{score} (C Sınıfı - Takipte)", "ru": f"{score}% (Класс C - Под наблюдением)", "uz": f"{score}% (C Sinf - Nazorat ostida)", "en": f"{score}% (Class C - Monitored)"}.get(lang, f"{score}%")
        else:
            score_badge = "🔴"
            score_txt = {"tr": f"%{score} (D Sınıfı - Yüksek Risk)", "ru": f"{score}% (Класс D - Высокий риск)", "uz": f"{score}% (D Sinf - Yuqori xavf)", "en": f"{score}% (Class D - High Risk)"}.get(lang, f"{score}%")

        # 4. Biyolojik Saat / Zaman Anomalisi (100% Dinamik)
        night_cnt = target_u.night_activity_count or 0
        if night_cnt > 0:
            rutin_txt = {"tr": f"⚠️ Gece {night_cnt} işlem (01:00-06:00)", "ru": f"⚠️ Ночная активность: {night_cnt} действ. (01:00-06:00)", "uz": f"⚠️ Tungi faollik: {night_cnt} ta amal (01:00-06:00)", "en": f"⚠️ Night activity: {night_cnt} actions (01:00-06:00)"}.get(lang, f"⚠️ Night: {night_cnt}")
        else:
            rutin_txt = {"tr": "Gündüz Rutini (Anomali Yok)", "ru": "Дневной режим (Без аномалий)", "uz": "Kunduzgi tartib (Anomaliya yo'q)", "en": "Daytime Routine (No anomaly)"}.get(lang, "Daytime Routine")

        # 5. Kimlik Değişim Geçmişi (100% Dinamik)
        names_log = []
        ...
        if names_log:
            ...
            alias_txt = {"tr": "Sabit (İsim/kullanıcı adı değişmedi)", "ru": "Стабильно (Имя не менялось)", "uz": "O'zgarmas (Ism o'zgarmagan)", "en": "Constant (No changes)"}.get(lang, "Constant")

        # 6. Tipoloji / İlgi Alanı (100% Dinamik)
        p_map = {}
        ...
        if top_act in ("grade", "exam"):
            persona_txt = {"tr": "🎯 Başarı & Not Odaklı", "ru": "🎯 Оценки и успеваемость", "uz": "🎯 Baholar va natijalar", "en": "🎯 Grade & Academic Focused"}.get(lang, "Academic")
        elif top_act in ("attendance", "excuse"):
            persona_txt = {"tr": "⚠️ Devamsızlık & İzin Odaklı", "ru": "⚠️ Пропуски и справки", "uz": "⚠️ Davomat va ruxsatlar", "en": "⚠️ Attendance & Excuse Focused"}.get(lang, "Attendance")
        elif top_act == "menu":
            persona_txt = {"tr": "🍲 Yemek & Menü Odaklı", "ru": "🍲 Меню столовой", "uz": "🍲 Oshxona menyusi", "en": "🍲 Cafeteria Menu Focused"}.get(lang, "Cafeteria")
        elif top_act == "download":
            persona_txt = {"tr": "📑 Evrak & Dosya Odaklı", "ru": "📑 Документы и выгрузки", "uz": "📑 Hujjatlar va fayllar", "en": "📑 Documents & Files Focused"}.get(lang, "Documents")
        else:
            persona_txt = {"tr": "💤 Standart / Asgari Temas", "ru": "💤 Редкий контакт / Стандарт", "uz": "💤 Kam muloqot / Standart", "en": "💤 Minimal Contact / Standard"}.get(lang, "Standard")

        # 7. Veri Hacmi (100% Dinamik)
        dl_cnt = target_u.download_count or 0
        if dl_cnt >= 10:
            dl_txt = {"tr": f"🚨 Yüksek Hacim ({dl_cnt} indirme)", "ru": f"🚨 Высокий объем ({dl_cnt} скачиваний)", "uz": f"🚨 Yuqori hajm ({dl_cnt} ta yuklash)", "en": f"🚨 High Volume ({dl_cnt} downloads)"}.get(lang, f"🚨 {dl_cnt} downloads")
        else:
        ...
        card_text = (
            ...
            f"{lbl_role_prefix}: <b>{role_label}</b>\n"
            "━━━━━━━━━━━━━━━━━━━━\n"
        ...
        # Butonlar: Kısa, taşma yapmayan ve yenileme özellikli
        ...
        buttons = [
            [InlineKeyboardButton(text=btn_refresh, callback_data=f"adm:user_360:{tg_id}:{page}")],
